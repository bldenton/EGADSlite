...
#ifdef __CUDACC__
__HOST_AND_DEVICE__ int EGlite_evaluate( const egObject *geom, const double *param,
                                     double *results );
#else
...
EGlite_arcBasis(qFill *q, int nx, int ny, int *sideptr[], double *abasis[2])
{
...
__HOST_AND_DEVICE__ static void
EGlite_getside(int iuv, double t, int len, int *side, double *uvx,
...
EGlite_dQuadTFI(int *elen, double *uv, int npts, double *uvx)
{
  ...
  for (j = i = 0; i < 4; i++) {
    len = elen[i] + 1;
    sideptr[i] = (int *) EGlite_alloc(len*sizeof(int));
    if (sideptr[i] == NULL) {
      for (k = 0; k < i; k++) EGlite_free(sideptr[k]);
      return -1;
    }
    if (i >= 2) {
      for (k = len-1; k > 0; k--, j++) {
  ...
  ul  = 2*cipt[3];
  for (k = len; k < npts; k++) {
    EGlite_getside(0, uvx[2*k  ], elen[3]+1, sideptr[3], uvx, uv, uvi[0]);
    EGlite_getside(0, uvx[2*k  ], elen[1]+1, sideptr[1], uvx, uv, uvi[1]);
    EGlite_getside(1, uvx[2*k+1], elen[0]+1, sideptr[0], uvx, uv, uvj[0]);
    EGlite_getside(1, uvx[2*k+1], elen[2]+1, sideptr[2], uvx, uv, uvj[1]);
  ...
  for (i = 0; i < 4; i++) EGlite_free(sideptr[i]);
...
__HOST_AND_DEVICE__ static int
EGlite_getVertCnt(qFill *q, int len, int blocks[][6])
{
  int k, cnt;
...
__HOST_AND_DEVICE__ static void
EGlite_setQuads(qFill *q, int len, int blocks[][6], int *sideptr[])
{
...
__HOST_AND_DEVICE__ static void
EGlite_smoothQuads(const egObject *face, qFill *q, int len, int npass)
{
  ...
  for (pass = 0; pass < npass; pass++) {
    ...
    /* get xyz */
    for (j = 0; j < q->nvert; j++) {
      status = EGlite_evaluate(face, verts[j].uv, results);
      if (status != EGADS_SUCCESS) {
        if (q->outLevel > 0)
          printf(" EGADS Info: EGlite_evaluate = %d (EGlite_smoothQuad)!\n", 
                 status);
        return;
      }
...
__HOST_AND_DEVICE__ static int
EGlite_triTemplate(/*@unused@*/ long tID, const egObject *face, qFill *q, int nsp,
               int *indices, int *elens, double *uv, int *npts, double **uvs)
{
  ...
  if (q->fillT == 0) {
    len       = EGlite_getVertCnt(q, 3, blocks);
    q->vpatch = (int *) EGlite_alloc(len*sizeof(int));
  ...
  len      = MAX(elens[0], len);
  q->quads = (Quads *) EGlite_alloc(len*len*sizeof(Quads));
  if (q->quads == NULL) {
    if (q->fillT == 0) EGlite_free(q->vpatch);
    return -1;
  }
  q->verts = (Node *) EGlite_alloc((len+1)*(len+1)*sizeof(Node));
  if (q->verts == NULL) {
    EGlite_free(q->quads);
    if (q->fillT == 0) EGlite_free(q->vpatch);
    return -1;
...
*/
  ...
  EGlite_setQuads(q, 3, blocks, sideptr);

  /* free up our integrated sides */
  for (i = 0; i < 9; i++) EGlite_free(sideptr[i]);

  /* get the actual storage that we return the data with */

  uvb = (double *) EGlite_alloc(2*q->nvert*sizeof(double));
  if (uvb == NULL) {
    EGlite_free(q->verts);
    EGlite_free(q->quads);
    if (q->fillT == 0) EGlite_free(q->vpatch);
  ...
  len = MAX(elens[1], elens[2]);
  len = MAX(elens[0], len);
  EGlite_smoothQuads(face, q, len*len, nsp);
  ...
  if (q->fillT != 0) {
    q->ntris = 2*q->nquad;
    q->tris  = (int *) EGlite_alloc(3*q->ntris*sizeof(int));
    if (q->tris == NULL) {
      EGlite_free(uvb);
      EGlite_free(q->verts);
      EGlite_free(q->quads);
...
__HOST_AND_DEVICE__ static int
EGlite_quadFillG(const egObject *face, qFill *q, int nsp, int *indices, int *elens,
             double *uv, int *npts, double **uvs)
{
  ...
  if (q->fillT == 0) {
    len       = EGlite_getVertCnt(q, 17, blocks);
    q->vpatch = (int *) EGlite_alloc(len*sizeof(int));
    if (q->vpatch == NULL) return -1;
  }
  ...
  EGlite_setQuads(q, 17, blocks, sideptr);

  /* free up our integrated sides */
  for (i = 0; i < 42; i++) EGlite_free(sideptr[i]);
  ...
  len = elens[1]*elens[2];
  EGlite_smoothQuads(face, q, len, nsp);
  ...
  if (q->fillT != 0) {
    ...
    if (extra != -1) len += 3;
    q->ntris = len/3;
    q->tris  = (int *) EGlite_alloc(len*sizeof(int));
    if (q->tris == NULL) {
      EGlite_free(uvb);
      EGlite_free(q->verts);
      EGlite_free(q->quads);
...
__HOST_AND_DEVICE__ static int
EGlite_quadFillQ(const egObject *face, qFill *q, int nsp, int *indices, int *elens,
             double *uv, int *npts, double **uvs)
{
  int    i, j, k, len, N, M, P, Q, extra = -1;
  ...
  if (q->fillT == 0) {
    len       = EGlite_getVertCnt(q, 12, blocks);
    q->vpatch = (int *) EGlite_alloc(len*sizeof(int));
    if (q->vpatch == NULL) return -1;
  }
  ...
  EGlite_setQuads(q, 12, blocks, sideptr);

  /* free up our integrated sides */
  for (i = 0; i < 31; i++) EGlite_free(sideptr[i]);
...
__HOST_AND_DEVICE__ static int
EGlite_quadFillP(const egObject *face, qFill *q, int nsp, int *indices, int *elens,
             double *uv, int *npts, double **uvs)
{
  int    i, j, k, len, N, M, P, Q, extra = -1;
  ...
  if (q->fillT == 0) {
    len       = EGlite_getVertCnt(q, 13, blocks);
    q->vpatch = (int *) EGlite_alloc(len*sizeof(int));
    if (q->vpatch == NULL) return -1;
  }
  ...
  EGlite_setQuads(q, 13, blocks, sideptr);

  /* free up our integrated sides */
  for (i = 0; i < 33; i++) EGlite_free(sideptr[i]);
...
__HOST_AND_DEVICE__ static int
EGlite_quadFillT(qFill *q, int *elens, double *uv, int *npts, double **uvs)
{
  ...
  len      = nx*ny;
  q->quads = (Quads *) EGlite_alloc(len*sizeof(Quads));
  if (q->quads == NULL) {
    if (q->fillT == 0) EGlite_free(q->vpatch);
    return -1;
  }
  len      = (nx+1)*(ny+1) + 1;
  q->verts = verts = (Node *) EGlite_alloc(len*sizeof(Node));
  if (q->verts == NULL) {
    EGlite_free(q->quads);
    if (q->fillT == 0) EGlite_free(q->vpatch);
    return -1;
  ...
  abasis[0] = (double *) EGlite_alloc((nx+1)*(ny+1)*sizeof(double));
  if (abasis[0] == NULL) {
    for (i = 0; i < 4; i++) EGlite_free(sideptr[i]);
    EGlite_free(q->verts);
    EGlite_free(q->quads);
  ...
  abasis[1] = (double *) EGlite_alloc((nx+1)*(ny+1)*sizeof(double));
  if (abasis[1] == NULL) {
    EGlite_free(abasis[0]);
    for (i = 0; i < 4; i++) EGlite_free(sideptr[i]);
    EGlite_free(q->verts);
    EGlite_free(q->quads);
    if (q->fillT == 0) EGlite_free(q->vpatch);
  ...
  EGlite_arcBasis(q, nx, ny, sideptr, abasis);
...
EGlite_quadFill(const egObject *face, double *parms, int *elens, double *uv, 
            int *npts, double **uvs, int *npat, int *pats, int **vpats)
{
  ...
  *uvs     = NULL;
  *vpats   = NULL;
  outLevel = q.outLevel = EGlite_outLevel(face);
  q.flip   = 1.0;
  q.fillT  = 0;
  q.ntris  = 0;
  ...
  if ((elens[0] == elens[2]) && (elens[1] == elens[3])) {
    ret = EGlite_quadFillT(&q, elens, uv, npts, uvs);
  ...
  indices = (int *) EGlite_alloc(len*sizeof(int));
  if (indices == NULL) return -1;
  uvx = (double *) EGlite_alloc(4*len*sizeof(double));
  if (uvx == NULL) {
    EGlite_free(indices);
    return -1;
  }
  ...
  if (P == 0) {
    ret = EGlite_quadFillQ(face, &q, nsp, indices, lens, uvx, npts, uvs);
  } else if (Q == 0) {
    ret = EGlite_quadFillP(face, &q, nsp, indices, lens, uvx, npts, uvs);
  } else {
    ret = EGlite_quadFillG(face, &q, nsp, indices, lens, uvx, npts, uvs);
  }
  for (i = 0; i < len; i++) {
    j         = len+i+1;
    uv[2*i  ] = uvx[2*j  ];
  ...
  if ((ret == 0) && (unmap == 0) && (align == 0) && (*uvs != NULL)) {
    ret = EGlite_dQuadTFI(elens, uv, *npts, *uvs);
    if (ret != 0) {
      EGlite_free(*uvs);
      EGlite_free(q.quads);
      EGlite_free(q.vpatch);
      *npts = 0;
      *uvs  = NULL;
    }
  }
...
__HOST_AND_DEVICE__ int
EGlite_quad2tris(long tID, const egObject *face, double *parms, int *elens,
             ...
             int *tfi)
{
  ...
  *tris    = NULL;
  ...
  EGlite_free(uvx);
  EGlite_free(indices);
  if (q.quads != NULL) EGlite_free(q.quads);
  if (ret != 0) {
    if (outLevel > 1)
  ...
  if ((q.tris == NULL) || (*uvs == NULL)) {
    ...
    if (q.tris != NULL) EGlite_free(q.tris);
    return -99;
  }
...
__HOST_AND_DEVICE__ static int
EGlite_quadfil3(long tID, const egObject *face, double *parms, int *elens,
            double *uv, int *npts, double **uvs, int *ntris, int **tris)
{
  ...
  *tris  = NULL;
  ...
  len += elens[ret];
  uvx = (double *) EGlite_alloc(2*len*sizeof(double));
  ...
  ret = EGlite_quad2tris(tID, face, parms, lens, uvx, npts, uvs, ntris, tris, &i);
  EGlite_free(uvx);
  if (ret != 0) return ret;
...
__HOST_AND_DEVICE__ static int
EGlite_quadfil3as4(long tID, const egObject *face, double *parms, int *elens,
               double *uv, int *npts, double **uvs, int *ntris, int **tris)
{
...
#ifdef DEBUG
  printf("EGlite_quadfil3as4: Filling 3-sided Face as if it were 4-sided\n");
  printf("EGlite_quadfil3as4: %d %d %d\n", elens[0], elens[1], elens[2]);
#endif
...
#ifdef DEBUG
  printf("EGlite_quadfil3as4: Quad with %d %d %d %d\n",
         lens[0], lens[1], lens[2], lens[3]);
#endif
  
  /* fill the area */
  
  ret = EGlite_quad2tris(tID, face, parms, lens, uv, npts, uvs, ntris, tris, &i);
  if (ret != 0) return ret;
...
__HOST_AND_DEVICE__ int
EGlite_quad2tris3(long tID, const egObject *face, double *parms, int *elens,
              double *uv, int *npts, double **uvs, int *ntris, int **tris,
              int *flag)
{
  ...
  *flag = 1;
  ...
  ret   = EGlite_quadfil3(tID, face, parms, elens, uv, npts, uvs, ntris, tris);
  if (ret == -8) {
    *npts = 1;
    ret   = EGlite_quadfil3(tID, face, parms, elens, uv, npts, uvs, ntris, tris);
    if (ret == -8) ret = -6;
  ...
  if (ret != 0) {
    ...
    *npts      = 0;
    ...
    indices    = (int *) EGlite_alloc((n+1)*sizeof(int));
    if (indices == NULL) return -1;
    for (i = 0; i < n; i++) indices[i] = i;
    indices[n] = 0;
    q.outLevel = EGlite_outLevel(face);
    q.flip     = 1.0;
    q.fillT    = 1;
    q.ntris    = 0;
    q.quads    = NULL;
    nsp        = parms[2]+0.1;
    ret = EGlite_triTemplate(tID, face, &q, nsp, indices, elens, uv, npts, uvs);
    /* make tris & free up q */
    EGlite_free(indices);
    if (q.quads != NULL) EGlite_free(q.quads);
    if (ret != 0) {
      if (q.outLevel > 1)
        printf("%lX EGADS Info: EGlite_triTemplate = %d!\n", tID, ret);
    } else {
      if ((q.tris == NULL) || (*uvs == NULL)) {
        if (*uvs   != NULL) EGlite_free(*uvs);
        if (q.tris != NULL) EGlite_free(q.tris);
  ...
  if (ret != 0) {
    *npts = 0;
    ret   = EGlite_quadfil3as4(tID, face, parms, elens, uv, npts, uvs, ntris, tris);
  }
