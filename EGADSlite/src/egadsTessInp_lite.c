...
__PROTO_H_AND_D__ void EGlite_cleanupTess( egTessel *btess );
__PROTO_H_AND_D__ void EGlite_cleanupTessMaps( egTessel *btess );
__PROTO_H_AND_D__ void EGlite_makeConnect( int k1, int k2, int *tri, int *kedge,
                                       int *ntable, connect *etable, int face );
__PROTO_H_AND_D__ int  EGlite_fillArea( int ncontours, const int *cntr,
...
__PROTO_H_AND_D__ int  EGlite_objectBodyTopo( const egObject *body, int oclass,
...
__PROTO_H_AND_D__ int  EGlite_invEvaluateGuess( const egObject *geom, double *xyz,
                                            double *param, double *result );
__PROTO_H_AND_D__ int  EGlite_getEdgeUV( const egObject *face, const egObject *edge,
                                     int sense, double t, double *result );
__PROTO_H_AND_D__ int  EGlite_getEdgeUVeval( const egObject *face,
                                         const egObject *topo, int sense,
                                         double t, double *result);
__PROTO_H_AND_D__ int  EGlite_getTessEdge( const egObject *tess, int indx, int *len,
                                       const double **xyz, const double **t );
__PROTO_H_AND_D__ int  EGlite_getTessFace( const egObject *tess, int indx, int *len,
...
EGlite_openTessBody(egObject *tess)
{
  egTessel *btess;
  ...
  context = EGlite_context(tess);
  if (tess->topObj != context) {
    printf(" EGADS Error: Tessellation in Model (EGlite_openTessBody)!\n");
    return EGADS_REFERCE;
  }
  btess = (egTessel *) tess->blind;
  if (btess == NULL) {
    printf(" EGADS Error: NULL Blind Object (EGlite_openTessBody)!\n");
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    printf(" EGADS Error: NULL Source Object (EGlite_openTessBody)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    printf(" EGADS Error: Source Not an Object (EGlite_openTessBody)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    printf(" EGADS Error: Source Not Body (EGlite_openTessBody)!\n");
...
#ifndef LITE
  EGlite_attributeDel(tess, ".mixed");
  EGlite_attributeDel(tess, ".tessType");
#endif
...
__HOST_AND_DEVICE__ int
EGlite_initTessBody(egObject *object, egObject **tess)
{
  ...
  *tess = NULL;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Blind Malloc (EGlite_initTessBody)!\n");
    EGlite_free(faces);
    EGlite_free(edges);
    return EGADS_MALLOC;
  }
  ...
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Alloc %d Edges (EGlite_initTessBody)!\n", nedge);
    EGlite_free(faces);
    EGlite_free(edges);
    EGlite_free(btess);
    return EGADS_MALLOC;
  ...
  EGlite_free(edges);
  for (j = 0; j < nedge; j++) {
    stat = EGlite_getTopology(btess->tess1d[j].obj, &geom, &oclass, &mtype, limits,
                          &nnode, &nodes, &senses);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_getTopology = %d (EGlite_initTessBody)!\n", stat);
      EGlite_free(faces);
      EGlite_cleanupTess(btess);
      EGlite_free(btess);
      return EGADS_MALLOC;
    }
    stat = EGlite_indexBodyTopo(object, nodes[0]);
    if (stat <= EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_indexBodyTopo0 = %d (EGlite_initTessBody)!\n",
               stat);
      EGlite_free(faces);
      EGlite_cleanupTess(btess);
      EGlite_free(btess);
    ...
    if (nnode == 1) continue;
    stat = EGlite_indexBodyTopo(object, nodes[1]);
    if (stat < EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_indexBodyTopo1 = %d (EGlite_initTessBody)!\n",
               stat);
      EGlite_free(faces);
      EGlite_cleanupTess(btess);
      EGlite_free(btess);
  ...
  if (nface != 0) {
    ...
    for (i = 0; i < nface; i++) {
      ...
      for (j = 0; j < nloop; j++) {
        ...
        if (stat != EGADS_SUCCESS) continue;
        for (k = 0; k < ndum; k++) {
          n = EGlite_indexBodyTopo(object, dum[k]);
            ...
            if (btess->tess1d[n-1].faces[0].nface != 0) {
              if (btess->tess1d[n-1].faces[0].nface == 1) {
                ...
                if (btess->tess1d[n-1].faces[0].faces == NULL) {
                  if (outLevel > 0)
                    printf(" EGADS Error: Alloc (-) Edge %d (EGlite_initTessBody)!\n",
                           n);
                  EGlite_free(faces);
                  EGlite_cleanupTess(btess);
                  EGlite_free(btess);
                ...
                if (finds == NULL) {
                  if (outLevel > 0)
                    printf(" EGADS Error: ReAlloc (-) Edge %d (EGlite_initTessBody)!\n",
                           n);
                  EGlite_free(faces);
                  EGlite_cleanupTess(btess);
                  EGlite_free(btess);
    ...
    if (btess->tess2d == NULL) {
      printf(" EGADS Error: Alloc %d Faces (EGlite_initTessBody)!\n", nface);
      EGlite_cleanupTess(btess);
      EGlite_free(btess);
      return EGADS_MALLOC;
    }
...
__HOST_AND_DEVICE__ int
EGlite_computeTessMap(egTessel *btess, int outLevel)
{
  ...
  if ((btess->nEdge == 1) && (btess->tess1d[0].obj->mtype == DEGENERATE) &&
    ...
    btess->xyzs = (double *) EGlite_alloc(3*sizeof(double));
    if (btess->xyzs == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Allocation of 1 Nodes (EGlite_computeTessMap)!\n");
      return EGADS_MALLOC;
    }
    ...
    btess->tess1d[0].global = (int *) EGlite_alloc(2*sizeof(int));
    if (btess->tess1d[0].global == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Allocation of 1 Global (EGlite_computeTessMap)!\n");
      return EGADS_MALLOC;
    }
    btess->tess1d[0].global[0] = 1;
    btess->tess1d[0].global[1] = 1;

    btess->globals = (int *) EGlite_alloc(2*sizeof(int));
  ...
  inode = (int *) EGlite_alloc(nNode*sizeof(int));
  if (inode == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Allocation of %d Nodes (EGlite_computeTessMap)!\n",
             nNode);
    return EGADS_MALLOC;
  }
  btess->xyzs = (double *) EGlite_alloc(3*nNode*sizeof(double));
  if (btess->xyzs == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Allocation Of %d Nodes (EGlite_computeTessMap)!\n",
             nNode);
  ...
  for (i = 0; i < btess->nEdge; i++) {
    ...
    inode[j]++;
    btess->tess1d[i].global = (int *) EGlite_alloc(k*sizeof(int));
    if (btess->tess1d[i].global == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: %d Allocation of %d Global (EGlite_computeTessMap)!\n",
               i+1, k);
      EGlite_cleanupTessMaps(btess);
      EGlite_free(inode);
      return EGADS_MALLOC;
  ...
  if (btess->nFace == 0) {
    ...
    btess->globals = (int *) EGlite_alloc(2*npts*sizeof(int));
    if (btess->globals == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Allocation of %d Globals (EGlite_computeTessMap)!\n",
               npts);
      EGlite_cleanupTessMaps(btess);
      EGlite_free(inode);
      return EGADS_MALLOC;
  ...
  for (k = i = 0; i < btess->nFace; i++) {
    ...
    if (n == 0) continue;
    btess->tess2d[i].global = (int *) EGlite_alloc(n*sizeof(int));
    if (btess->tess2d[i].global == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: %d Allocation of %d Globals (EGlite_computeTessMap)!\n",
               i+1, n);
      EGlite_cleanupTessMaps(btess);
      EGlite_free(inode);
      return EGADS_MALLOC;
  ...
  btess->globals = (int *) EGlite_alloc(2*k*sizeof(int));
  if (btess->globals == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Allocation of %d Globals (EGlite_computeTessMap)!\n",
             k);
...
__HOST_AND_DEVICE__ int
EGlite_statusTessBody(egObject *tess, egObject **body, int *state, int *npts)
{
  ...
  *state = *npts = 0;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_statusTessBody)!\n");
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_statusTessBody)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_statusTessBody)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_statusTessBody)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Edge Tessellations (EGlite_statusTessBody)!\n");
    return EGADS_NODATA;
  }
  if ((btess->tess2d == NULL) && (btess->nFace != 0)) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_statusTessBody)!\n");
    return EGADS_NODATA;
  }
  *body  = obj;
  *state = btess->done;
  *npts  = btess->nGlobal;
  ...
  if (btess->done == 0) {
    ...
    for (i = 0; i < btess->nEdge; i++) {
      ...
      if ((btess->tess1d[i].obj->mtype == DEGENERATE) &&
        ...
        /* allocate the data */
        len  = 2;
        xyzs = (double *) EGlite_alloc(3*len*sizeof(double));
        if (xyzs == NULL) {
          if (outLevel > 0)
            printf(" EGADS Warning: Allocating %d Coordinates (EGlite_statusTessBody)!\n",
                   len);
          continue;
        }
        ts = (double *) EGlite_alloc(len*sizeof(double));
        if (ts == NULL) {
          if (outLevel > 0)
            printf(" EGADS Warning: Allocating %d Parameters (EGlite_statusTessBody)!\n",
                   len);
          EGlite_free(xyzs);
          continue;
        }
        stat = EGlite_getTopology(btess->tess1d[i].obj, &ref, &oclass, &mtype,
                              ts, &j, &nodes, &senses);
        if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Warning: EGlite_getTopo Degen %d = %d (EGlite_statusTessBody)!\n",
                   i, stat);
          EGlite_free(xyzs);
          EGlite_free(ts);
          continue;
        }
        stat = EGlite_getTopology(nodes[0], &ref, &oclass, &mtype, xyzs, &j, &objs,
                              &senses);
        if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Warning: EGlite_getTopo Node %d = %d (EGlite_statusTessBody)!\n",
                   i, stat);
          EGlite_free(xyzs);
          EGlite_free(ts);
          continue;
        ...
        /* set the data */
        if (btess->tess1d[i].xyz != NULL) EGlite_free(btess->tess1d[i].xyz);
        if (btess->tess1d[i].t   != NULL) EGlite_free(btess->tess1d[i].t);
        xyzs[3]               = xyzs[0];
        xyzs[4]               = xyzs[1];
        xyzs[5]               = xyzs[2];
    ...
    stat = EGlite_attributeRet(tess, ".mixed", &atype, &alen, &ints, &reals, &str);
    if (stat == EGADS_SUCCESS) {
      if ((alen != btess->nFace) || (atype != ATTRINT)) {
#ifndef LITE
        stat = EGlite_attributeDel(tess, ".mixed");
        if ((stat != EGADS_SUCCESS) && (outLevel > 0))
          printf(" EGADS Error: Deleting Attribute %d (EGlite_statusTessBody)!\n",
                 stat);
#endif
      } else {
        for (j = i = 0; i < btess->nFace; i++) {
          ...
          if (2*ints[i] >  btess->tess2d[i].ntris) {
            printf(" EGADS Error: %d 2*nQuads (%d) > nTris (%d) (EGlite_statusTessBody)!\n",
        ...
        if (j == -1) {
          ...
          if (stat != EGADS_SUCCESS)
            if (outLevel > 0)
              printf(" EGADS Warning: EGlite_attributeAdd = %d (EGlite_statusTessBody)!\n",
                     stat);
#endif
        } else {
#ifndef LITE
  ...
  stat = EGlite_computeTessMap(btess, outLevel);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_computeTessMap = %d (EGlite_statusTessBody)!\n",
              stat);
    return stat;
  }
  *npts = btess->nGlobal;
...
__HOST_AND_DEVICE__ int
EGlite_setTessEdge(const egObject *tess, int index, int len, const double *xyz,
               const double *t)
{
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_setTessEdge)!\n");
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_setTessEdge)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_setTessEdge)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_setTessEdge)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Edge Tessellations (EGlite_setTessEdge)!\n");
    return EGADS_NODATA;
  }
  if (btess->done == 1) {
    if (outLevel > 0)
      printf(" EGADS Error: Complete Tessellation (EGlite_setTessEdge)!\n");
    return EGADS_EXISTS;
  }
  if ((index < 1) || (index > btess->nEdge)) {
    if (outLevel > 0)
      printf(" EGADS Error: Index = %d [1-%d] (EGlite_setTessEdge)!\n",
             index, btess->nEdge);
  ...
  for (i = 1; i < len; i++) {
    ...
    printf(" EGADS Error: ts not in order %d %lf  %d %lf (EGlite_setTessEdge)!\n",
           i-1, t[i-1], i, t[i]);
  ...
  if (obj->oclass == EBODY) {
    stat = EGlite_getBodyTopos(obj, btess->tess1d[index-1].obj, EFACE, &n, &objs);
  } else {
    stat = EGlite_getBodyTopos(obj, btess->tess1d[index-1].obj,  FACE, &n, &objs);
  }
  if (stat  != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: Edge %d - EGlite_getBodyTopos = %d (EGlite_setTessEdge)!\n",
             index, stat);
    return stat;
  }
  if ((n != 0) && (objs != NULL))
    for (i = 0; i < n; i++) {
      j = EGlite_indexBodyTopo(obj, objs[i]);
      if (j <= EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: Edge %d - indexTopoBody %d = %d (EGlite_setTessEdge)!\n",
                 index, i+1, j);
        EGlite_free(objs);
        return j;
      }
      if (btess->tess2d[j-1].npts != 0) {
        if (outLevel > 0)
          printf(" EGADS Error: Edge %d - Face %d set (EGlite_setTessEdge)!\n",
                 index, j);
        EGlite_free(objs);
        return EGADS_EXISTS;
      }
    }
  if (objs != NULL) EGlite_free(objs);

  /* get the bounding information */
  stat = EGlite_getTopology(btess->tess1d[index-1].obj, &geom, &oclass, &mtype,
                        trange, &nnode, &nodes, &senses);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_getTopology = %d for Edge %d (EGlite_setTessEdge)!\n",
             stat, index);
    return stat;
  }

  stat = EGlite_getTopology(nodes[0], &geom, &oclass, &mtype, xyz0, &i, &objs,
                        &senses);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_getTopology = %d - Edge %d/0 (EGlite_setTessEdge)!\n",
             stat, index);
    return stat;
  }
  j = nnode - 1;
  stat = EGlite_getTopology(nodes[j], &geom, &oclass, &mtype, xyz1, &i, &objs,
                        &senses);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_getTopology = %d - Edge %d/1 (EGlite_setTessEdge)!\n",
             stat, index);
    return stat;
  }
  ...
  if (xyzs == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Allocating %d Coordinates (EGlite_setTessEdge)!\n",
             len);
    return EGADS_MALLOC;
  }
  ts = (double *) EGlite_alloc(len*sizeof(double));
  if (ts == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Allocating %d Parameters (EGlite_setTessEdge)!\n",
             len);
    EGlite_free(xyzs);
    return EGADS_MALLOC;
  }
  ...
  if ((xyz[0] != xyz0[0]) || (xyz[1] != xyz0[1]) || (xyz[2] != xyz0[2])) {
    printf(" EGADS Warning: Edge %d- Node %d  XYZ misMatch (EGlite_setTessEdge)!\n",
           index, EGlite_indexBodyTopo(obj, nodes[0]));
  }
  ts[0]   = trange[0];
  xyzs[0] = xyz0[0];
  xyzs[1] = xyz0[1];
  ...
  if ((xyz[3*len-3] != xyz1[0]) || (xyz[3*len-2] != xyz1[1]) ||
      (xyz[3*len-1] != xyz1[2])) {
    printf(" EGADS Warning: Edge %d+ Node %d  XYZ misMatch (EGlite_setTessEdge)!\n",
           index, EGlite_indexBodyTopo(obj, nodes[j]));
  }
  ts[len-1]     = trange[1];
  xyzs[3*len-3] = xyz1[0];
  xyzs[3*len-2] = xyz1[1];
  ...
  /* set the data */
  if (btess->tess1d[index-1].xyz != NULL) EGlite_free(btess->tess1d[index-1].xyz);
  if (btess->tess1d[index-1].t   != NULL) EGlite_free(btess->tess1d[index-1].t);
  ...
  if (n > 0) {
    if (btess->tess1d[index-1].faces[0].tric == NULL)
      btess->tess1d[index-1].faces[0].tric = (int *)
                                             EGlite_alloc((n*(len-1))*sizeof(int));
    if (btess->tess1d[index-1].faces[0].tric == NULL) {
      if (outLevel > 0)
        printf(" EGADS Warning: Alloc %d Tric- Edge %d (EGlite_setTessEdge)!\n",
               len, index);
    } else {
      for (i = 0; i < len-1; i++)
        for (k = 0; k < n; k++)
              ...
              int nseg, triSeg *segs)
{
  ...
  ntab = (int *) EGlite_alloc(nverts*sizeof(int));
  if (ntab == NULL) {
    printf(" EGADS Error: Vert Table Malloc (EGlite_setTessFace)!\n");
    return EGADS_MALLOC;
  }
  etab = (connect *) EGlite_alloc(ntri*3*sizeof(connect));
  if (etab == NULL) {
    printf(" EGADS Error: Edge Table Malloc (EGlite_setTessFace)!\n");
    EGlite_free(ntab);
    return EGADS_MALLOC;
  }
  ...
  for (j = 0; j < ntri;  j++) {
    EGlite_makeConnect(tris[3*j+1], tris[3*j+2], &tric[3*j  ], &nside,ntab,etab, f);
    EGlite_makeConnect(tris[3*j  ], tris[3*j+2], &tric[3*j+1], &nside,ntab,etab, f);
    EGlite_makeConnect(tris[3*j  ], tris[3*j+1], &tric[3*j+2], &nside,ntab,etab, f);
  }

  for (j = 0; j < nseg; j++)
    EGlite_makeConnect(segs[j].indices[0], segs[j].indices[1], &segs[j].neighbor,
                   &nside, ntab, etab, f);
...
__HOST_AND_DEVICE__ int
EGlite_setTessFace(const egObject *tess, int index, int len, const double *xyz,
               const double *uv, int ntri, const int *tris)
{
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_setTessFace)!\n");
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_setTessFace)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_setTessFace)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_setTessFace)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess2d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_setTessFace)!\n");
    return EGADS_NODATA;
  }
  if (btess->done == 1) {
    if (outLevel > 0)
      printf(" EGADS Error: Complete Tessellation (EGlite_setTessFace)!\n");
    return EGADS_EXISTS;
  }
  if ((index < 1) || (index > btess->nFace)) {
    if (outLevel > 0)
      printf(" EGADS Error: Index = %d [1-%d] (EGlite_setTessFace)!\n",
             index, btess->nFace);
    return EGADS_INDEXERR;
  }
  ...
  for (i = 0; i < ntri; i++) {
    for (j = 0; j < 3; j++) {
      if ((tris[3*i+j] < 1) || (tris[3*i+j] > len)) {
        printf(" EGADS Error: Face %d - tris %d/%d = %d [1-%d] (EGlite_setTessFace)!\n",
    ...
    if ((tris[3*i  ] == tris[3*i+1]) || (tris[3*i  ] == tris[3*i+2]) ||
      ...
      printf(" EGADS Error: Face %d - tris %d is degenerate = %d %d %d (EGlite_setTessFace)!\n",
  ...
  /* get our Face object */
  if (obj->oclass == EBODY) {
    stat = EGlite_getBodyTopos(obj, NULL, EFACE, &i, &faces);
  } else {
    stat = EGlite_getBodyTopos(obj, NULL,  FACE, &i, &faces);
  }
  if (stat  != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - EGlite_getBodyTopos Faces = %d (EGlite_setTessFace)!\n",
             index, stat);
    return stat;
  }
  face = faces[index-1];
  ...
  if (obj->oclass == EBODY) {
    stat = EGlite_getBodyTopos(obj, face, EEDGE, &nedge, &edges);
  } else {
    stat = EGlite_getBodyTopos(obj, face,  EDGE, &nedge, &edges);
  }
  if (stat  != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - EGlite_getBodyTopos Edges = %d (EGlite_setTessFace)!\n",
             index, stat);
    return stat;
  }
  for (i = 0; i < nedge; i++) {
    if (edges[i] == NULL) continue;
    iedge = EGlite_indexBodyTopo(obj, edges[i]);
    if (iedge <= 0) {
      if (outLevel > 0)
        printf(" EGADS Error: Face %d - EGlite_indexTopoBody Edge %d = %d (EGlite_setTessFace)!\n",
               index, i+1, iedge);
      return iedge;
    }
    ...
    printf(" EGADS Error: Face %d - No Tessellation for Edge %d (EGlite_setTessFace)!\n",
           index, iedge);
    EGlite_free(edges);
    return EGADS_NOTFOUND;
  }
  ...
  /* setup ptype/pindex */
  table = (int *) EGlite_alloc(3*len*sizeof(int));
  if (table == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - Allocating %d Vert Table (EGlite_setTessFace)!\n",
             index, len);
    EGlite_free(edges);
    return EGADS_MALLOC;
  }
  ...
  if (stat  != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - EGlite_getTopology = %d (EGlite_setTessFace)!\n",
             index, stat);
    EGlite_free(table);
    return stat;
  }
  ...
  for (ntot = i = 0; i < nloop; i++) {
    ...
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: Face %d - EGlite_getTopology Loop %d = %d (EGlite_setTessFace)!\n",
               index, i+1, stat);
      EGlite_free(table);
      return stat;
    }
    for (j = 0; j < nedge; j++) {
      ...
      iedge = EGlite_indexBodyTopo(obj, edges[j]);
  ...
  segs  = (triSeg *) EGlite_alloc(ntot*sizeof(triSeg));
  if (segs == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - Allocating %d Segs (EGlite_setTessFace)!\n",
             index, ntot);
    EGlite_free(table);
    return EGADS_MALLOC;
  }
  uvs = (double *) EGlite_alloc((2*ntot+2)*sizeof(double));
  if (uvs == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - Allocating %d uvs (EGlite_setTessFace)!\n",
             index, ntot);
    EGlite_free(segs);
    EGlite_free(table);
    return EGADS_MALLOC;
  ...
  frlps = (int *) EGlite_alloc(nloop*sizeof(int));
  if (frlps == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - Allocating %d Loops (EGlite_setTessFace)!\n",
             index, nloop);
    EGlite_free(uvs);
    EGlite_free(segs);
    EGlite_free(table);
  ...
  for (np = i = 0; i < nloop; i++) {
    ...
    for (j = 0; j < nedge; j++, n += ori*lor) {
      if (edges[n] == NULL) continue;
      iedge = EGlite_indexBodyTopo(obj, edges[n]);
      ...
      stat = EGlite_getTopology(edges[n], &geom, &oclass, &mtype, trange, &nd,
                            &nds, &sns);
      if (stat != EGADS_SUCCESS) {
        printf(" EGADS Error: EGlite_getTopology = %d  for Face = %d, Edge = %d\n",
               stat, index, iedge);
        EGlite_free(frlps);
        EGlite_free(uvs);
        EGlite_free(segs);
      ...
      for (m = 0; m < nedge; m++) {
        ...
        if (iedge == EGlite_indexBodyTopo(obj, edges[m])) {
          uvm[0] = uvm[1] = -1.0;
          uvp[0] = uvp[1] =  1.0;
          ...
          if ((uvm[0] == uvp[0]) && (uvm[1] == uvp[1]) && (intEdg == NULL)) {
            ...
            intEdg = (double *)
                     EGlite_alloc(4*btess->tess1d[iedge-1].npts*sizeof(double));
            if (intEdg == NULL) {
            ...
            for (m = 0; m < btess->tess1d[iedge-1].npts; m++) {
              stat = EGlite_getEdgeUV(face, edges[n], senses[n]*lor,
      ...
      if (sen == 1) {
        for (m = 0; m < btess->tess1d[iedge-1].npts-1; m++, np++) {
          ...
          uvp[0] = uvp[1] = 0.0;
          stat   = EGlite_getEdgeUV(face, edges[n], senses[n],
                                btess->tess1d[iedge-1].t[m], uvp);
          if (stat != EGADS_SUCCESS)
            printf(" EGADS Internal: Face %d - EdgeUV+ = %d (EGlite_setTessFace)!\n",
                   index, stat);
          stat = findPoint(range, uvp, pt, pi, len, table, uv, uvx);
          if (stat < EGADS_SUCCESS) {
            printf(" EGADS Error: Face %d - FindPt+ %d/%d = %d (EGlite_setTessFace)!\n",
                   index, pt, pi, stat);
  ...
  fast.segs  = NULL;
  fast.front = NULL;
  frame = (int *) EGlite_alloc(3*ntrix*sizeof(int));
  if (frame == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - Allocating %d frame (EGlite_setTessFace)!\n",
             index, ntrix);
    EGlite_free(frlps);
    EGlite_free(uvs);
    EGlite_free(segs);
  ...
  n = EGlite_fillArea(nloop, frlps, uvs, frame, &nf8, 0, &fast);
  ...
  if (n != ntrix) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      n = EGlite_fillArea(nloop, frlps, uvs, frame, &nf8, 1, &fast);
  ...
  EGlite_free(uvs);
  if (n != ntrix) {
    printf(" EGADS Error: Face %d - Can't Triangulate Frame (EGlite_setTessFace)!\n",
           index);
    EGlite_free(frame);
    EGlite_free(frlps);
    EGlite_free(segs);
  ...
  trix = (int *) EGlite_alloc(3*ntri*sizeof(int));
  tric = (int *) EGlite_alloc(3*ntri*sizeof(int));
  if ((trix == NULL) || (tric == NULL)) {
    if (trix != NULL) EGlite_free(trix);
    if (tric != NULL) EGlite_free(tric);
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - Allocating %d tris (EGlite_setTessFace)!\n",
             index, ntri);
    EGlite_free(frame);
    EGlite_free(frlps);
    EGlite_free(segs);
  ...
  ptype  = (int *)    EGlite_alloc(  len*sizeof(int));
  pindex = (int *)    EGlite_alloc(  len*sizeof(int));
  uvs    = (double *) EGlite_alloc(2*len*sizeof(double));
  xyzs   = (double *) EGlite_alloc(3*len*sizeof(double));
  if ((ptype == NULL) || (pindex == NULL) || (xyzs == NULL) || (uvs == NULL)) {
    if (ptype  != NULL) EGlite_free(ptype);
    if (pindex != NULL) EGlite_free(pindex);
    if (uvs    != NULL) EGlite_free(uvs);
    if (xyzs   != NULL) EGlite_free(xyzs);
    if (outLevel > 0)
      printf(" EGADS Error: Face %d - Allocating %d verts (EGlite_setTessFace)!\n",
             index, len);
    EGlite_free(trix);
    EGlite_free(tric);
    EGlite_free(frame);
  ...
  if (btess->tess2d[index-1].xyz    != NULL)
    EGlite_free(btess->tess2d[index-1].xyz);
  if (btess->tess2d[index-1].uv     != NULL)
    EGlite_free(btess->tess2d[index-1].uv);
  if (btess->tess2d[index-1].ptype  != NULL)
    EGlite_free(btess->tess2d[index-1].ptype);
  if (btess->tess2d[index-1].pindex != NULL)
    EGlite_free(btess->tess2d[index-1].pindex);
  if (btess->tess2d[index-1].bary   != NULL)
    EGlite_free(btess->tess2d[index-1].bary);
  if (btess->tess2d[index-1].frame  != NULL)
    EGlite_free(btess->tess2d[index-1].frame);
  if (btess->tess2d[index-1].frlps  != NULL)
    EGlite_free(btess->tess2d[index-1].frlps);
  if (btess->tess2d[index-1].tris   != NULL)
    EGlite_free(btess->tess2d[index-1].tris);
  if (btess->tess2d[index-1].tric   != NULL)
    EGlite_free(btess->tess2d[index-1].tric);
...
__HOST_AND_DEVICE__ int
EGlite_localToGlobal(const egObject *tess, int index, int local, int *global)
{
  int      stat, n;
  egTessel *btess;
  egObject *node, **edges;
  ...
  *global = 0;
  ...
  if (btess->globals == NULL) {
    stat = EGlite_computeTessMap(btess, EGlite_outLevel(tess));
    if (stat != EGADS_SUCCESS) {
      printf(" EGADS Error: EGlite_computeTessMap = %d (EGlite_localToGlobal)!\n", stat);
      return stat;
    }
  }
  ...
  if (index < 0) {
    ...
    *global = btess->tess2d[ index-1].global[local-1];
    ...
    /* Node */
    stat = EGlite_objectBodyTopo(btess->src, NODE, local, &node);
    if (stat != EGADS_SUCCESS) {
      printf(" EGADS Error: EGlite_objectBodyTopo = %d (EGlite_localToGlobal)!\n", stat);
      return stat;
    }
    if (btess->src->oclass == EBODY) {
      stat = EGlite_getBodyTopos(btess->src, node, EEDGE, &n, &edges);
    } else {
      stat = EGlite_getBodyTopos(btess->src, node,  EDGE, &n, &edges);
    }
    if (stat != EGADS_SUCCESS) {
      printf(" EGADS Error: EGlite_getBodyTopos = %d (EGlite_localToGlobal)!\n", stat);
      return stat;
    }
    if (n == 0) {
      printf(" EGADS Error: No Edges for Node (EGlite_localToGlobal)!\n");
      return EGADS_TOPOERR;
    }
    stat = EGlite_indexBodyTopo(btess->src, edges[0]);
    if (stat <= EGADS_SUCCESS) {
      printf(" EGADS Error: EGlite_indexBodyTopo = %d (EGlite_localToGlobal)!\n", stat);
      return stat;
    }
    EGlite_free(edges);
...
__HOST_AND_DEVICE__ int
EGlite_getGlobal(const egObject *tess, int global, int *ptype, int *pindex,
             /*@null@*/ double *xyz)
{
  int      i, j, stat;
  egTessel *btess;
  ...
  if (btess->globals == NULL) {
    stat = EGlite_computeTessMap(btess, EGlite_outLevel(tess));
    if (stat != EGADS_SUCCESS) {
      printf(" EGADS Error: EGlite_computeTessMap = %d (EGlite_getGlobal)!\n", stat);
      return stat;
    }
  }
...
__HOST_AND_DEVICE__ static int
EGlite_findMidSide(int i1, int i2, int *table, midside *mid)
{
  int index, last;
...
EGlite_getInterior(const ego face, double *xyz, double *uv)
{
  int    count, stat, diverge = 0;
  double a00, a10, a11, b0, b1, det, dist, ldist, frac = 1.0;
  double dx[3], du[2], range[4], result[18];

  stat = EGlite_getRange(face, range, &count);
  if (stat != EGADS_SUCCESS) {
    printf(" EGADS Warning: EGlite_getRange = %d (EGlite_getInterior)!\n", stat);
    return;
  }
  if ((uv[0] < range[0]) || (uv[0] > range[1]) || (uv[1] < range[2]) ||
      (uv[1] > range[3])) {
    printf(" EGADS Warning: Out of Range (EGlite_getInterior)!\n");
    return;
  }
  ...
  for (count = 0; count < 15; count++) {
    ...
    if (stat != EGADS_SUCCESS) {
      printf(" EGADS Warning: EGlite_evaluate = %d (EGlite_getInterior)!\n", stat);
      return;
    }
    dx[0] = result[0] - xyz[0];
    dx[1] = result[1] - xyz[1];
    ...
    det = a00*a11 - a10*a10;
    if (det == 0.0) {
      printf(" EGADS Warning: det = zero (EGlite_getInterior)!\n");
      return;
    }
    det   = 1.0/det;
    ...
    if (count != 0)
      if (ldist < dist) {
        printf(" EGlite_getInterior -- diverge %d  %le %le!\n",
               count+1, du[0], du[1]);
/*      return;  */
        diverge++;
      }
    ...
    uv[1] += frac*du[1];
    if (frac != 1.0) {
      if (diverge != 0) printf(" EGlite_getInterior: boundary exit %d\n", diverge);
      return;
    }
  }

  printf(" EGlite_getInterior: not converged %d!\n", diverge);
}


__HOST_AND_DEVICE__ int
EGlite_baryInsert(ego face, double w1, double w2, double w3, const double *uvA,
              const double *uvB, const double *uvC, double *uvOUT)
{
  ...
  stat    = EGlite_evaluate(face, uvA , pA);
  stat   += EGlite_evaluate(face, uvB , pB);
  stat   += EGlite_evaluate(face, uvC , pC);
  if (stat != EGADS_SUCCESS) {
    printf(" EGlite_baryInsert: EGlite_evaluate triangle vertices stat %d !!\n", stat);
    return stat;
  }

#ifdef DEBUG
  EGlite_evaluate(face, uvIT , pIT);
  OA[0] = pA[0] - pIT[0];
  OA[1] = pA[1] - pIT[1];
  OA[2] = pA[2] - pIT[2];
  ...
  for (it = 0; it < nT; it++) {
    ...
    for (i = 0; i < nS; i++) {
      ...
      stat = EGlite_evaluate(face, uv, pIT);
      if (stat != EGADS_SUCCESS) {
        printf(" EGlite_baryInsert: EGlite_evaluate = %d\n", stat);
        s /= 2.0;
        continue;
      }
      ...
      if ((it > 0) &&
        ...
        if ((DOT(crA,crIT) < 0.0) || (DOT(crB,crIT) < 0.0) || (DOT(crC,crIT) < 0.0)) {
          if (fold == 1) {
            ...
            printf(" EGlite_baryInsert: Initial Fold!\n");
#ifdef DEBUGM
          ...
          stat = EGlite_invEvaluateGuess(face, pIT, uvIT, J);

          /* check for success and in range */
          EGlite_getRange(face, range, &periodic);
          if ((stat != EGADS_SUCCESS) ||
              (uvIT[0] < range[0]) || (uvIT[0] > range[1]) ||
              (uvIT[1] < range[2]) || (uvIT[1] > range[3])) {

            if (stat != EGADS_SUCCESS) {
              printf(" EGlite_baryInsert: EGlite_invEvaluateGuess = %d\n", stat);
            } else {
              printf(" EGlite_baryInsert: EGlite_invEvaluateGuess out of range!\n");
            }

            /* one last hope for a better initial guess */
            stat = EGlite_invEvaluate(face, pIT, uvIT, J);
            if (stat != EGADS_SUCCESS) {
              printf(" EGlite_baryInsert: EGlite_invEvaluate = %d\n", stat);
              return EGADS_TESSTATE;
            }
          }
          fold = 1;
    ...
    if (i == nS) printf(" EGlite_baryInsert: LineSearch Failed!\n");
  ...
  if (rsdnrm >= tol && delnrm >= tol) {
    printf(" EGlite_baryInsert: not converged -- residual %1.2le delta %1.2le (%1.2le)\n", rsdnrm, delnrm, tol);
  }
...
*/
__HOST_AND_DEVICE__ void
EGlite_minArc4(const ego face, double fact1, double fact2, const double *uv0,
           ...
           double *uvOUT)
{
  ...
  stat = EGlite_getRange(face, range, &it);
  if (stat != EGADS_SUCCESS) {
    printf(" EGlite_minArc4: EGlite_getRange = %d!\n", stat);
    return;
  }
  ...
  uvOUT[1] = uvIT[1];
  uvIT[2]  = uvIT[3] = 0.0;
  i        = EGlite_evaluate(face, uv0, p0);
  i       += EGlite_evaluate(face, uv1, p1);
  i       += EGlite_evaluate(face, uv2, p2);
  i       += EGlite_evaluate(face, uv3, p3);
  i       += EGlite_evaluate(face, uvIT, pIT);
#ifdef DEBUG
  printf("%lf %lf %lf 1\n",  p0[0],  p0[1],  p0[2]);
  printf("%lf %lf %lf 2\n",  p1[0],  p1[1],  p1[2]);
  printf("%lf %lf %lf 3\n",  p2[0],  p2[1],  p2[2]);
...
#endif
  if (i != EGADS_SUCCESS) {
    printf(" EGlite_minArc4: EGlite_evaluate %d!\n", i);
    return;
  }
  ...
  for (it   = 0; it < nT; it++) {
    ...
    if (uvIT[0] < range[0] || uvIT[0] > range[1] ||
      ...
      stat    = EGlite_invEvaluateGuess(face, pIT, uvIT, pOUT);
      if (stat != EGADS_SUCCESS ||
          uvIT[0] < range[0] || uvIT[0] > range[1] ||
          uvIT[1] < range[2] || uvIT[1] > range[3]) {
        stat      = EGlite_invEvaluate(face, pIT, uvIT, pOUT);
        if (stat != EGADS_SUCCESS ||
            uvIT[0] < range[0] || uvIT[0] > range[1] ||
        ...
        printf(" EGlite_minArc4: Out of UVbox!\n");
        return;
      }
    }
...
#endif
    if (i != EGADS_SUCCESS) printf(" EGlite_minArc4: EGlite_evaluate = %d!\n", i);
    if (i != EGADS_SUCCESS || x2 < EPS10) break;
  }
  if (it == nT)
    printf(" EGlite_minArc4: Not Converged!\n");
  ...
  if (fabs(l[0] / lt02 - fact1       )  > 1.0e-08 ||
    ...
    printf(" EGlite_minArc4: Did NOT work!!!!!\n");
    return;
  }
#endif
...
__HOST_AND_DEVICE__ void
EGlite_getEdgepoint(const ego edge, double w, double tm, double tp, double *tOUT)
{
  ...
  tIT   = tp * w + tm * (1.0 - w);
  *tOUT = tIT;
  stat  = EGlite_evaluate(edge, &tm,  pm);
  stat += EGlite_evaluate(edge, &tp,  pp);
  stat += EGlite_evaluate(edge, &tIT, pIT);
  if (stat != EGADS_SUCCESS) {
    printf(" EGlite_getEdgePoint before Newton: EGlite_evaluate error sum = %d\n ",
           stat);
    return;
  }
  ...
  for (it = 0 ; it < nT; it++) {
    ...
    for (i = 0; i < nS; i++) {
      t    = tIT + s * dt;
      stat = EGlite_evaluate(edge, &t, pIT);
      if (stat != EGADS_SUCCESS) {
        printf(" EGlite_getEdgepoint: EGlite_evaluate = %d\n", stat);
        s /= 2.0;
        continue;
      }
      if (t < tm || t > tp ) {
        if (it > 0) {
          ...
          if (i == 0) {
            ...
            stat      = EGlite_invEvaluateGuess(edge, pIT, &tIT, xyz);
            if (stat != EGADS_SUCCESS ||
                tIT < tm || tIT > tp ) {
              printf(" EGlite_getEdgepoint: EGlite_invEvaluateGuess %d out-of-range!\n",
                     stat);
              /* we are hopelessly out of range... */
              return;
            }
            ...
            /* already tried better guess with inverse evaluate... */
            printf(" EGlite_getEdgepoint: Initial out-of-range!\n");
            return;
          }
        }
      }
    ...
    tIT = t;
    if (i == nS) printf(" EGlite_getEdgepoint: LineSearch Failed!\n");
  ...
  if (res >= nEPS && fabs(dt) >= nEPS)
    printf(" EGlite_getEdgepoint: not converged -- residual %1.2le delta %1.2le (%1.2le)!\n",
           res, fabs(dt), nEPS);
  
  /* found a solution out of range -- report it (for now) */
  if (tIT < tm || tIT > tp )
    printf(" EGlite_getEdgepoint: solution out-of-range!\n");
...
*/
__HOST_AND_DEVICE__ void
EGlite_getSidepoint(const ego face, double fact, const double *uvm,
                ...
                /*@null@*/ const double *uvr, double *uvOUT)
{
  ...
  stat = EGlite_getRange(face, range, &it);
  if (stat != EGADS_SUCCESS) {
    printf(" EGlite_getSidepoint: EGlite_egetRange = %d!\n ", stat);
    return;
  }
  /* Initial guess uv  */
  ...
  uvOUT[0] = uvIT[0];
  uvOUT[1] = uvIT[1];
  stat     = EGlite_evaluate(face, uvm, pM);
  stat    += EGlite_evaluate(face, uvp, pP);
  if (uvl != NULL) stat += EGlite_evaluate(face, uvl, pL);
  if (uvr != NULL) stat += EGlite_evaluate(face, uvr, pR);
  if (stat != EGADS_SUCCESS) {
    printf(" EGlite_getSidepoint: EGlite_evaluate = %d!\n ", stat);
    return;
  }
#ifdef DEBUG
  EGlite_evaluate(face, uvIT, pIT);
  ...
  for (it = 0; it < nT; it++) {
    ...
    for (i = 0; i < nS; i++) {
      ...
      if ((it > 0) &&
        ...
        if (uv[0] < range[0] || uv[0] > range[1] ||
          ...
          if (fold == 2) {
            /* already tried better guess with inverse evaluate... */
            printf(" EGlite_getSidepoint: Initial out-of-range!\n");
            return;
          }
          ...
          stat   = EGlite_invEvaluate(face, pIT, uvOUT, xyz);
          if (stat != EGADS_SUCCESS ||
              uvOUT[0] < range[0] || uvOUT[0] > range[1] ||
              uvOUT[1] < range[2] || uvOUT[1] > range[3]) {
            printf(" EGlite_getSidepoint: EGlite_invEvaluate Out-of-Range %d!\n", stat);
            /* we are hopelessly out of range... */
            return;
          }
          fold = 2;
    ...
    if (i == nS) {
      printf(" EGlite_getSidepoint: LineSearch Failed (%lf %lf)!\n",
             uvIT[0], uvIT[1]);
      pIT[0] = fact*pP[0] + (1.0-fact)*pM[0];
      pIT[1] = fact*pP[1] + (1.0-fact)*pM[1];
      pIT[2] = fact*pP[2] + (1.0-fact)*pM[2];
      ...
      if (stat != EGADS_SUCCESS ||
          uvOUT[0] < range[0] || uvOUT[0] > range[1] ||
          uvOUT[1] < range[2] || uvOUT[1] > range[3]) {
        printf(" EGlite_getSidepoint: EGlite_invEvaluate %d out-of-range!\n", stat);
      }
      return;
    }
...
#ifdef DEBUG
  if (i != EGADS_SUCCESS) printf("EGlite_evaluate %d !!\n", i);
  ...
  if (rsdnrm >= EPS10 && delnrm >= EPS10)
    printf(" EGlite_getSidepoint: not converged -- residual %1.2le delta %1.2le (%1.2le)!\n",
           rsdnrm, delnrm, EPS10);
  
  /* found a solution out of range -- report it (for now) */
  if (uvIT[0] < range[0] || uvIT[0] > range[1] ||
      uvIT[1] < range[2] || uvIT[1] > range[3])
    printf(" EGlite_getSidepoint: solution out-of-range!\n");

  if (foldL || foldR) {
    printf(" EGlite_getSidepoint: solution with fold!\n");
#ifdef DEBUGM
    printf("VARIABLES=X, Y, Z, U, V\n");
    ...
    pIT[1] = fact*pP[1] + (1.0-fact)*pM[1];
    pIT[2] = fact*pP[2] + (1.0-fact)*pM[2];
    stat   = EGlite_invEvaluate(face, pIT, uvIT, xyz);
    if (stat != EGADS_SUCCESS ||
        uvIT[0] < range[0] || uvIT[0] > range[1] ||
        uvIT[1] < range[2] || uvIT[1] > range[3]) {
      printf(" EGlite_getSidepoint: EGlite_invEvaluate out-of-range %d!\n", stat);
...
EGlite_quadThread(void *struc)
{
  ...
  for (;;) {
    ...
    if (qthread->mutex != NULL) EMP_LITE_LockSet(qthread->mutex);
    ...
    if (qthread->mutex != NULL) EMP_LITE_LockRelease(qthread->mutex);
    if (index >= qthread->end) break;

    /* do the work */
    stat = EGlite_meshRegularization(qthread->bodydata->qm[index]);
    if (stat != EGADS_SUCCESS)
      printf(" EGADS Warning: EGlite_fullMeshRegularization %d = %d (EGlite_quadTess)!\n",
             index+1, stat);
  }

  /* exhausted all work -- exit */
  if (ID != qthread->master) EMP_LITE_ThreadExit();
}


__HOST_AND_DEVICE__ int
EGlite_quadTess(const egObject *tess, egObject **quadTess)
{
  ...
  *quadTess = NULL;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_quadTess)!\n");
    return EGADS_NOTFOUND;
  }
  if (btess->done == 0) {
    if (outLevel > 0)
      printf(" EGADS Error: Tessellation is open (EGlite_quadTess)!\n");
    return EGADS_TESSTATE;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_quadTess)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_quadTess)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_quadTess)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Edge Tessellations (EGlite_quadTess)!\n");
    return EGADS_NODATA;
  }
  if ((btess->tess2d == NULL) && (btess->nFace != 0)) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_quadTess)!\n");
    return EGADS_NODATA;
  }

  /* initialize the new tessellation object */
  stat = EGlite_initTessBody(obj, &newTess);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_initTessBody = %d (EGlite_quadTess)!\n", stat);
    return stat;
  }

  if (obj->oclass == EBODY) {
    stat = EGlite_getBodyTopos(obj, NULL, EEDGE, &nedges, &edges);
  } else {
    stat = EGlite_getBodyTopos(obj, NULL,  EDGE, &nedges, &edges);
  }
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_getBodyTopos E = %d (EGlite_quadTess)!\n", stat);
  ...
  for (j = i = 0; i < nedges; i++) {
    ...
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_getTessEdge %d = %d (EGlite_quadTess)!\n",
               i+1, stat);
      EGlite_free(edges);
      EGlite_deleteObject(newTess);
      return stat;
    ...
    if (npts == 0) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_getTessEdge %d -- no points (EGlite_quadTess)!\n",
               i+1);
      EGlite_free(edges);
      EGlite_deleteObject(newTess);
      return EGADS_INDEXERR;
  ...
  coords = (double *) EGlite_alloc(4*(2*j-1)*sizeof(double));
  if (coords == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc for %d points (EGlite_quadTess)!\n", j);
    EGlite_free(edges);
    EGlite_deleteObject(newTess);
    return EGADS_MALLOC;
  }
...
/*    parms[2*j+1] = 0.5*(ts[j] + ts[j+1]);  */
      EGlite_getEdgepoint(edges[i], 0.5, ts[j], ts[j+1], &parms[2*j+1]);
      stat = EGlite_evaluate(edges[i], &parms[2*j+1], result);
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: EGlite_evaluate Edge %d/%d = %d (EGlite_quadTess)!\n",
                 i+1, j+1, stat);
        EGlite_free(coords);
        EGlite_free(edges);
        EGlite_deleteObject(newTess);
    ...
    stat = EGlite_setTessEdge(newTess, i+1, 2*npts-1, coords, parms);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_setTessEdge %d = %d (EGlite_quadTess)!\n",
               i+1, stat);
      EGlite_free(coords);
      EGlite_free(edges);
      EGlite_deleteObject(newTess);
  ...
  if (obj->oclass == EBODY) {
    stat = EGlite_getBodyTopos(obj, NULL, EFACE, &nfaces, &faces);
  } else {
    stat = EGlite_getBodyTopos(obj, NULL,  FACE, &nfaces, &faces);
  }
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_getBodyTopos F = %d (EGlite_quadTess)!\n", stat);
    EGlite_free(edges);
    EGlite_deleteObject(newTess);
    return stat;
  }
  for (nside = npts = ntris = i = 0; i < nfaces; i++) {
    stat = EGlite_getTessFace(tess, i+1, &np, &xyzs, &uvs, &ptype, &pindex,
                          &nt, &trs, &trc);
  ...
  k      = npts + ntris + nside;
  coords = (double *) EGlite_alloc(5*k*sizeof(double));
  if (coords == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc for %d Face points (EGlite_quadTess)!\n", k);
    EGlite_free(faces);
    EGlite_free(edges);
    EGlite_deleteObject(newTess);
    return EGADS_MALLOC;
  ...
  parms = &coords[3*k];
  tris  = (int *) EGlite_alloc(6*3*ntris*sizeof(int));
  if (tris == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc for %d Face tris (EGlite_quadTess)!\n", 3*ntris);
    EGlite_free(coords);
    EGlite_free(faces);
    EGlite_free(edges);
    EGlite_deleteObject(newTess);
  ...
  mid = (midside *) EGlite_alloc(nside*sizeof(midside));
  if (mid == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc for %d Face mids (EGlite_quadTess)!\n", nside);
    EGlite_free(tris);
    EGlite_free(coords);
    EGlite_free(faces);
    EGlite_free(edges);
  ...
  table = (int *) EGlite_alloc(npts*sizeof(int));
  if (table == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc for %d Face table (EGlite_quadTess)!\n", npts);
    EGlite_free(mid);
    EGlite_free(tris);
    EGlite_free(coords);
    EGlite_free(faces);
  ...
  for (i = 0; i < nfaces; i++) {
    ...
    iuv[0]    = iuv[1]    = 0;
    if (obj->oclass == EBODY) {
      stat    = EGlite_getBodyTopos(obj, faces[i], EEDGE, &k, &objs);
    } else {
      stat    = EGlite_getBodyTopos(obj, faces[i],  EDGE, &k, &objs);
    }
    if (stat != EGADS_SUCCESS) {
      printf(" EGADS Internal: EGlite_getBodyTopos on Face %d = %d\n", i+1, stat);
    } else {
      for (j = 0; j < k; j++) {
        stat = EGlite_getTopology(objs[j], &geom, &oclass, &mtype,
                              trange, &n, &nodes, &senses);
        if (stat != EGADS_SUCCESS) {
          printf(" EGADS Internal: EGlite_getTopology on Edge = %d\n", stat);
          continue;
        }
        if (mtype != DEGENERATE) continue;
        stat = EGlite_getEdgeUVeval(faces[i], objs[j], 0, trange[0], result);
        if (stat != EGADS_SUCCESS) {
          printf(" EGADS Internal: EGlite_getEdgeUVeval = %d\n", stat);
          continue;
        }
        n = EGlite_indexBodyTopo(obj, nodes[0]);
        if (n > 0) {
          if (degens[0] == 0) {
            degens[0] = n;
    ...
    for (is = j = 0; j < nt; j++)
      for (k = 0; k < 3; k++) {
        ...
        m = EGlite_findMidSide(i1, i2, table, mid);
        if (m > 0) continue;
        uvm[0] = uvs[2*i1  ];
        uvm[1] = uvs[2*i1+1];
        uvp[0] = uvs[2*i2  ];
        ...
        if (trc[3*j+k] > 0) {
          ...
          EGlite_getSidepoint(faces[i], 0.5, uvm, uvp, &uvs[2*i0], &uvs[2*i3], uv);
          stat = EGlite_evaluate(faces[i], uv, result);
        } else {
          /* Edge side */
          ie   = -trc[3*j+k] - 1;
          stat = EGlite_getTopology(edges[ie], &geom, &oclass, &mtype,
                                trange, &n, &objs, &senses);
          if (stat != EGADS_SUCCESS) {
            if (outLevel > 0)
              printf(" EGADS Error: EGlite_getTopology %d/%d = %d (EGlite_quadTess)!\n",
                     i+1, ie+1, stat);
            EGlite_free(table);
            EGlite_free(mid);
            EGlite_free(tris);
          ...
          n = EGlite_indexBodyTopo(obj, objs[0]);
          if ((ptype[i1] == 0) && (ptype[i2] == 0)) {
            ...
            if (ien < 0) {
              printf(" EGADS Internal: EGlite_quadTess ien = %d!\n", ien);
              continue;
            }
          ...
          stat = EGlite_getEdgeUV(faces[i], edges[ie], 0, t, uv);
          if (stat == EGADS_TOPOERR) {
            /* sense in Face twice! */
            stat = EGlite_getEdgeUV(faces[i], edges[ie], -1, t, uvm);
            if (stat != EGADS_SUCCESS) {
              printf(" EGADS Info:  EGlite_getEdgeUV -1\n");
            } else {
              stat = EGlite_getEdgeUV(faces[i], edges[ie],  1, t, uvp);
              if (stat != EGADS_SUCCESS) {
                printf(" EGADS Info:  EGlite_getEdgeUV +1\n");
              } else {
          ...
          if (stat != EGADS_SUCCESS) {
            if (outLevel > 0)
              printf(" EGADS Error: EGlite_getEdgeUV %d/%d = %d (EGlite_quadTess)!\n",
                     i+1, ie+1, stat);
            EGlite_free(table);
            EGlite_free(mid);
            EGlite_free(tris);
        ...
        if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Error: EGlite_evaluate %d %d/%d = %d (EGlite_quadTess)!\n",
                   i+1, j+1, k+1, stat);
          EGlite_free(table);
          EGlite_free(mid);
          EGlite_free(tris);
    ...
    for (j = 0; j < nt; j++) {
      for (k = 0; k < 3; k++) {
        ...
        side[k] = EGlite_findMidSide(i1, i2, table, mid);
        if (side[k] <= 0) {
          if (outLevel > 0)
            printf(" EGADS Error: findMidSide %d = %d (EGlite_quadTess)!\n",
                   i+1, side[k]);
          EGlite_free(table);
          EGlite_free(mid);
          EGlite_free(tris);
      ...
      t    = 1.0/3.0;
      stat = EGlite_baryInsert(faces[i], t, t, t, &parms[2*(side[0]-1)],
                           &parms[2*(side[1]-1)], &parms[2*(side[2]-1)], uv);
      if (stat != EGADS_SUCCESS) {
        printf(" EGADS Info: EGlite_baryInsert = %d (EGlite_quadTess)!\n", stat);
        ...
        EGlite_getInterior(faces[i], xyz, uv);
      }
      stat = EGlite_evaluate(faces[i], uv, result);
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: EGlite_evaluate %d %d = %d (EGlite_quadTess)!\n",
                 i+1, j+1, stat);
        EGlite_free(table);
        EGlite_free(mid);
        EGlite_free(tris);
    ...
    stat = EGlite_setTessFace(newTess, i+1, iv, coords, parms, 6*nt, tris);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_setTessFace %d = %d (EGlite_quadTess)!\n",
               i+1, stat);
      EGlite_free(table);
      EGlite_free(mid);
      EGlite_free(tris);
  ...
  for (i = 0; i < nfaces; i++) {
    ...
    for (is = j = 0; j < nt/2; j++)
      for (k = 0; k < 4; k++) {
        ...
        if (ie > 0) {
          ...
          EGlite_getSidepoint(faces[i], 0.5, uvm, uvp, NULL, NULL, uv);
          stat = EGlite_evaluate(faces[i], uv, result);
        } else {
          /* Edge side */
          ie   = -ie - 1;
    ...
    for (j = 0; j < nt/2; j++) {
      ...
      /* get middle vertex based on mid-sides */
      EGlite_minArc4(faces[i], 0.5, 0.5,
    ...
    stat = EGlite_setTessFace(newTess, i+1, iv, coords, parms, 4*nt, tris);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_setTessFace %d = %d (EGlite_quadTess)!\n",
               i+1, stat);
  ...
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_statusTessBody = %d (EGlite_quadTess)!\n", stat);
    EGlite_deleteObject(newTess);
    return stat;
  }
  if (i != 1) {
    if (outLevel > 0)
      printf(" EGADS Error: New Tessellation Object is Open (EGlite_quadTess)!\n");
    EGlite_deleteObject(newTess);
    return EGADS_TESSTATE;
  }
#ifndef LITE
  table = (int *) EGlite_alloc(nfaces*sizeof(int));
  if (table != NULL) {
    for (i = 0; i < nfaces; i++) table[i] = ntess->tess2d[i].ntris/2;
    stat = EGlite_attributeAdd(newTess, ".mixed", ATTRINT, nfaces,
                           table, NULL, NULL);
    if (stat != EGADS_SUCCESS)
      printf(" EGADS Warning: EGlite_attributeAdd m = %d !\n", stat);
    EGlite_free(table);
  }
  ...
  if (stat != EGADS_SUCCESS)
    if (outLevel > 0)
      printf(" EGADS Warning: EGlite_attributeAdd Q = %d (EGlite_quadTess)!\n", stat);
#endif

  /* do we regularize? */
  stat = EGlite_attributeRet(tess, ".qRegular", &atype, &alen, &ints, &reals, &str);
  if (stat == EGADS_SUCCESS)
    if ((atype == ATTRSTRING) && (str != NULL))
#ifdef __CUDACC__
      if ((EGlite_strncmp(str,"OFF",3) == 0) || (EGlite_strncmp(str,"Off",3) == 0) ||
          (EGlite_strncmp(str,"off",3) == 0)) {
#else
  ...
  if (btess->src->oclass == EBODY) {
    stat = EGlite_getBodyTopos(btess->src, NULL, EEDGE, &bodydata.nedges, NULL);
  } else {
    stat = EGlite_getBodyTopos(btess->src, NULL,  EDGE, &bodydata.nedges, NULL);
  }
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Warning: EGlite_getBodyTopos E = %d (EGlite_quadTess)!\n", stat);
    EGlite_deleteObject(newTess);
    return stat;
  }
  if (btess->src->oclass == EBODY) {
    stat = EGlite_getBodyTopos(btess->src, NULL, EFACE, &bodydata.nfaces,
                           &bodydata.faces);
  } else {
    stat = EGlite_getBodyTopos(btess->src, NULL,  FACE, &bodydata.nfaces,
                           &bodydata.faces);
  }
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Warning: EGlite_getBodyTopos F = %d (EGlite_quadTess)!\n", stat);
    EGlite_deleteObject(newTess);
    return stat;
  }
  stat = EGlite_createMeshMap(&bodydata);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Warning: EGlite_createMeshMap = %d (EGlite_quadTess)!\n", stat);
    EGlite_free(bodydata.faces);
    *quadTess = newTess;
    return EGADS_SUCCESS;
  }
  ...
  qthread.mutex    = NULL;
  qthread.master   = EMP_LITE_ThreadID();
  ...
  if (np > 1) {
    ...
    qthread.mutex = EMP_LITE_LockCreate();
    if (qthread.mutex == NULL) {
      ...
      if (threads == NULL) {
        EMP_LITE_LockDestroy(qthread.mutex);
        np = 1;
      }
    }
  }
  ...
  if (qthread.mutex != NULL) EMP_LITE_LockDestroy(qthread.mutex);
  ...
  stat = EGlite_makeQuadTess(bodydata, &obj);
  EGlite_deleteObject(newTess);
  EGlite_destroyMeshMap(&bodydata);
  EGlite_free(bodydata.faces);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Warning: EGlite_makeQuadTess = %d (EGlite_quadTess)!\n", stat);
    return stat;
  }
