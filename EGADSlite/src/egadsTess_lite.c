...
__PROTO_H_AND_D__ int  EGlite_tolerance( const egObject *topo, double *tol );
__PROTO_H_AND_D__ int  EGlite_getTopology( const egObject *topo, egObject **geom,
                                       int *oclas, int *type,
                                       /*@null@*/ double *limits, int *nChild,
                                       egObject ***children, int **senses );
__PROTO_H_AND_D__ int  EGlite_getBodyTopos( const egObject *body,
                                        /*@null@*/ egObject *src, int oclass,
                                        int *ntopo,
                                        /*@null@*/ egObject ***topos );
__PROTO_H_AND_D__ int  EGlite_indexBodyTopo( const egObject *body,
                                         const egObject *src );
__PROTO_H_AND_D__ int  EGlite_sameBodyTopo( const egObject *bod1,
                                        const egObject *bod2 );
__PROTO_H_AND_D__ int  EGlite_getEdgeUV( const egObject *face,
                                     const egObject *edge, int sense, double t,
                                     double *result );
__PROTO_H_AND_D__ int  EGlite_getRange( const egObject *geom, double *range,
                                    int *pflag );
__PROTO_H_AND_D__ int  EGlite_getGeometry( const egObject *geom, int *oclass,
                                       int *type, egObject **rGeom,
                                       /*@null@*/ int    **ivec,
                                       /*@null@*/ double **rvec );
__PROTO_H_AND_D__ int  EGlite_evaluate( const egObject *geom,
                                    /*@null@*/ const double *param,
                                    double *result );
__PROTO_H_AND_D__ int  EGlite_invEvaluate( const egObject *geom, double *xyz,
                                       double *param, double *result );
__PROTO_H_AND_D__ int  EGlite_arcLength( const egObject *geom, double t1, double t2,
                                     double *result );
__PROTO_H_AND_D__ int  EGlite_isSame( const egObject *geo1, const egObject *geo2 );
__PROTO_H_AND_D__ int  EGlite_attributeRet( const egObject *obj, const char *name,
                                        int *atype, int *len,
                                        /*@null@*/ const int    **ints,
                                        /*@null@*/ const double **reals,
                                        /*@null@*/ const char   **str );
__PROTO_H_AND_D__ int  EGlite_attributeNumSeq( const egObject *obj,
                                           const char *name, int *num );
__PROTO_H_AND_D__ int  EGlite_attributeRetSeq( const egObject *obj, const char *nam,
                                           int index, int *atype, int *len,
                                           /*@null@*/ const int    **ints,
                                           /*@null@*/ const double **reals,
                                           /*@null@*/ const char   **str );
#ifndef LITE
           extern int  EGlite_fullAttrs( const egObject *obj );
           extern int  EGlite_effectiveMap( egObject *EObject, double *eparam,
                                        egObject **Object, double *param );
           extern int  EGlite_attributeDel( egObject *obj,
                                        /*@null@*/ const char *name );
           extern int  EGlite_attributeAdd( egObject *obj, const char *name,
                                        int type, int len,
                                        /*@null@*/ const int    *ints,
                                        /*@null@*/ const double *reals,
                                        /*@null@*/ const char   *str );
...
__PROTO_H_AND_D__ int  EGlite_tessellate( int outLevel, triStruct *ts, long tID );
__PROTO_H_AND_D__ int  EGlite_quadFill( const egObject *face, double *parms,
                                    int *elens, double *uv, int *npts,
                                    double **uvs, int *npat, int *pats,
                                    int **vpats );
__PROTO_H_AND_D__ int  EGlite_baryFrame( egTess2D *tess2d );
__PROTO_H_AND_D__ int  EGlite_baryTess( egTess2D tess2d, const double *uv,
                                    double *w );
__PROTO_H_AND_D__ void EGlite_mapTessTs( egTess1D src, egTess1D dst );
__PROTO_H_AND_D__ int  EGlite_relPosTs( egObject *geom, int n,
                                    /*@null@*/ const double *rel,
                                    double *ts, double *xyzs );
#ifdef INSERTKNOTS
__PROTO_H_AND_D__ int  EGlite_mapSequen( egObject *src, egObject *dst,
                                     egObject **result );
#endif
...
EGlite_attrRet3R(const egObject *obj, const char *name, double *vals)
{
  ...
  stat = EGlite_attributeRet(obj, name, &atype, &alen, &ints, &reals, &str);
  ...
  /* do we need to merge? */
  stat = EGlite_attributeNumSeq(obj, name, &nSeq);
  if (stat != EGADS_SUCCESS) return stat;
  if (nSeq == 0) return EGADS_NOTFOUND;
  
  for (i = 1; i <= nSeq; i++) {
    stat = EGlite_attributeRetSeq(obj, name, i, &atype, &alen, &ints, &reals, &str);
...
__HOST_AND_DEVICE__ static int
EGlite_faceConnIndex(egFconn conn, int face)
{
  int i;
...
EGlite_checkTriangulation(egTessel *btess)
{
          ...
          if ((ie < 1) || (ie > btess->nEdge)) {
            ...
            if (iv == 0) {
              for (i = 0; i <  btess->tess1d[ie-1].npts-1; 
                ...
                if (nf > 0) {
                  k = EGlite_faceConnIndex(btess->tess1d[ie-1].faces[0], iface);
                  if (k != 0)
                    if (btess->tess1d[ie-1].faces[0].tric[i*nf+k-1] == itri) break;
                }
                nf = btess->tess1d[ie-1].faces[1].nface;
                if (nf > 0) {
                  k = EGlite_faceConnIndex(btess->tess1d[ie-1].faces[1], iface);
                  if (k != 0) 
...
__HOST_AND_DEVICE__ static int
EGlite_crossSeg(int index, const double *mid, int i2, const double *vertices, 
...
EGlite_fillArea(int nc, const int *cntr, const double *vertices, int *triangles,
            int *n_fig8, int pass, fillArea *fa)
{
  ...
  *n_fig8 = 0;
  ...
  if (fa->front == NULL) {
    ...
    fa->front = (Front *) EGlite_alloc(fa->mfront*sizeof(Front));
    if (fa->front == NULL) return 0;
    fa->segs  = (int *) EGlite_alloc(2*fa->mfront*sizeof(int));
    if (fa->segs  == NULL) {
      EGlite_free(fa->front);
      fa->front = NULL;
      return 0;
    }
  } else {
    if (fa->mfront < fa->nfront) {
      ...
      tmp = (Front *) EGlite_reall(fa->front, i*sizeof(Front));
      if (tmp == NULL) return 0;
      itmp = (int *) EGlite_reall(fa->segs, 2*i*sizeof(int));
      if (itmp == NULL) {
        EGlite_free(tmp);
        return 0;
      }
  ...
  if (fa->pts == NULL) {
    ...
    fa->pts = (int *) EGlite_alloc(fa->mpts*sizeof(int));
    if (fa->pts == NULL) return 0;
  } else {
    if (fa->mpts < npts) {
      i = fa->mpts;
      while (i < npts) i += CHUNK;
      itmp = (int *) EGlite_reall(fa->pts, i*sizeof(int));
...
small:
    ...
    for (i = 0; i < fa->nfront; i++) {
      ...
      if (area > 0.0) {
        ...
        if (d < dist) {
          if (EGlite_crossSeg(index, mid, i2, vertices, pass, fa)) continue;
          dist  = d;
          indx2 = i;
        }
      }
...
EGlite_makeConnect(int k1, int k2, int *tri, int *kedge, int *ntable,
    ...
    if (etable[iface].node2 == kn2) {
      if (etable[iface].tri != NULL) {
        ...
        *tri = look;
        ...
        printf(", Side %d %d complete [but %d] (EGlite_makeConnect)!\n",
                k1+1, k2+1, *tri);
      }
      return;
    } else {
...
__HOST_AND_DEVICE__ int
EGlite_makeNeighbors(triStruct *ts, int f)
{
  int     *ntab, nside, j;
  connect *etab;
  
  ntab = (int *) EGlite_alloc(ts->nverts*sizeof(int));
  if (ntab == NULL) {
    printf(" EGADS Error: Vert Table Malloc (EGlite_makeTessBody)!\n");
    return EGADS_MALLOC;    
  }
  etab = (connect *) EGlite_alloc(ts->ntris*3*sizeof(connect));
  if (etab == NULL) {
    printf(" EGADS Error: Edge Table Malloc (EGlite_makeTessBody)!\n");
    EGlite_free(ntab);
    return EGADS_MALLOC;    
  }
  ...
  for (j = 0; j < ts->ntris;  j++) {
    EGlite_makeConnect( ts->tris[j].indices[1], ts->tris[j].indices[2],
                   &ts->tris[j].neighbors[0], &nside, ntab, etab, f);
    EGlite_makeConnect( ts->tris[j].indices[0], ts->tris[j].indices[2], 
                   &ts->tris[j].neighbors[1], &nside, ntab, etab, f);
    EGlite_makeConnect( ts->tris[j].indices[0], ts->tris[j].indices[1],
                   &ts->tris[j].neighbors[2], &nside, ntab, etab, f);
  }

  for (j = 0; j < ts->nsegs; j++)
    EGlite_makeConnect( ts->segs[j].indices[0], ts->segs[j].indices[1],
...
__HOST_AND_DEVICE__ static void
EGlite_updateTris(triStruct *ts, egTessel *btess, int fIndex)
{
  int    i, j, k, m, n, nf, edge, *ptype, *pindex, *frame, *frlps, *tris, *tric;
  double *xyz, *uv;
  
  xyz    = (double *) EGlite_alloc(3*ts->nverts*sizeof(double));
  uv     = (double *) EGlite_alloc(2*ts->nverts*sizeof(double));
  ptype  = (int *)    EGlite_alloc(  ts->nverts*sizeof(int));
  pindex = (int *)    EGlite_alloc(  ts->nverts*sizeof(int));
  frame  = (int *)    EGlite_alloc(3*ts->nframe*sizeof(int));
  frlps  = (int *)    EGlite_alloc(  ts->nloop* sizeof(int));
  tris   = (int *)    EGlite_alloc(3*ts->ntris* sizeof(int));
  tric   = (int *)    EGlite_alloc(3*ts->ntris* sizeof(int));
  if ((xyz    == NULL) || (uv    == NULL) || (ptype == NULL) ||
    ...
    printf(" EGADS Error: Cannot Allocate Tessellation Memory for %d!\n",
           fIndex);
    if (tric   != NULL) EGlite_free(tric);
    if (tris   != NULL) EGlite_free(tris);
    if (frlps  != NULL) EGlite_free(frlps);
    if (frame  != NULL) EGlite_free(frame);
    if (pindex != NULL) EGlite_free(pindex);
    if (ptype  != NULL) EGlite_free(ptype);
    if (uv     != NULL) EGlite_free(uv);
    if (xyz    != NULL) EGlite_free(xyz);
    return;
  }
    ...
    for (j = 0; j < 3; j++) 
      if (tric[3*i+j] < 0) {
        ...
        if (ts->segs[n-1].edge > 0) {
          ...
          m  = EGlite_faceConnIndex(btess->tess1d[edge-1].faces[1], fIndex);
          if (m == 0) {
          ...
          m  = EGlite_faceConnIndex(btess->tess1d[edge-1].faces[0], fIndex);
          if (m == 0) {
            printf(" EGADS Internal: Face %d not found in Edge (-) %d!\n",
                   fIndex, edge);
          } else {
...
__HOST_AND_DEVICE__ int
EGlite_getTessEdge(const egObject *tess, int indx, int *len,
               const double **xyz, const double **t)
{
  ...
  *xyz = *t = NULL;
  ...
  outLevel = EGlite_outLevel(tess);

  index = abs(indx);
  btess = (egTessel *) tess->blind;
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_getTessEdge)!\n");  
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_getTessEdge)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_getTessEdge)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_getTessEdge)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Edge Tessellations (EGlite_getTessEdge)!\n");
    return EGADS_NODATA;  
  }
  if ((index < 1) || (index > btess->nEdge)) {
    if (outLevel > 0)
      printf(" EGADS Error: Index = %d [1-%d] (EGlite_getTessEdge)!\n",
             index, btess->nEdge);
  ...
  /* mapping? */
  if (indx < 0) {
    stat = EGlite_attributeRet(obj, ".eMap", &aType, &alen, &ints, &reals, &str);
    if (stat == EGADS_SUCCESS) {
      if (aType != ATTRINT) {
        if (outLevel > 0)
          printf(" EGADS Error: Map Attribute Not an Int (EGlite_getTessEdge)!\n");
        return EGADS_ATTRERR;
      }
      if (alen != btess->nEdge) {
        if (outLevel > 0)
          printf(" EGADS Error: Len Mismatch %d %d (EGlite_getTessEdge)!\n",
                 alen, btess->nFace);
      ...
      if ((index < 1) || (index > btess->nEdge)) {
        if (outLevel > 0)
          printf(" EGADS Error: Mapped Index = %d [1-%d] (EGlite_getTessEdge)!\n",
                 index, btess->nEdge);
        return EGADS_INDEXERR;
      }
    }
...
__HOST_AND_DEVICE__ int
EGlite_getTessFace(const egObject *tess, int indx, int *len, const double **xyz,
               ...
               int *ntri, const int **tris, const int **tric)
{
  ...
  *ptype = *pindex = NULL;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_getTessFace)!\n");  
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_getTessFace)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_getTessFace)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_getTessFace)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess2d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_getTessFace)!\n");
    return EGADS_NODATA;  
  }
  if ((index < 1) || (index > btess->nFace)) {
    if (outLevel > 0)
      printf(" EGADS Error: Index = %d [1-%d] (EGlite_getTessFace)!\n",
             index, btess->nFace);
    return EGADS_INDEXERR;
  }
  ...
  /* mapping? */
  if (indx < 0) {
    stat = EGlite_attributeRet(obj, ".fMap", &aType, &alen, &ints, &reals, &str);
    if (stat == EGADS_SUCCESS) {
      if (aType != ATTRINT) {
        if (outLevel > 0)
          printf(" EGADS Error: Map Attribute Not an Int (EGlite_getTessFace)!\n");
        return EGADS_ATTRERR;
      }
      if (alen != btess->nFace) {
        if (outLevel > 0)
          printf(" EGADS Error: Len Mismatch %d %d (EGlite_getTessFace)!\n",
                 alen, btess->nFace);
        return EGADS_TOPOERR;
      }
      index = ints[index-1];
      if ((index < 1) || (index > btess->nFace)) {
        if (outLevel > 0)
          printf(" EGADS Error: Mapped Index = %d [1-%d] (EGlite_getTessFace)!\n",
                 index, btess->nFace);
        return EGADS_INDEXERR;
      }
    }
...
__HOST_AND_DEVICE__ int
EGlite_getTessLoops(const egObject *tess, int fIndex, int *nloop,
                const int **lIndices)
{
  egTessel *btess;
  egObject *obj;
...
__HOST_AND_DEVICE__ static int
EGlite_loopQuad(double edgeTol, int *lens, double *uvs, triStruct *ts)
{
  ...
  types = (int *) EGlite_alloc(2*len*sizeof(int));
...
__HOST_AND_DEVICE__ int
EGlite_fillTris(egObject *body, int iFace, egObject *face, egObject *tess,
            triStruct *ts, fillArea *fa, long tID)
{
  ...
  stat = EGlite_getTopology(face, &geom, &oclass, &ori, range, &nloop, &loops,
                        &lsenses);
  if (stat != EGADS_SUCCESS) return stat;
  if (face->oclass == EFACE) {
    ...
    stat = EGlite_getRange(geom, srange, &iper);
    if (stat != EGADS_SUCCESS) return stat;
    geomtype = geom->mtype;
  }
  if (nloop > 1) {
    stat = EGlite_attributeRet(face, ".innerLoops", &atype, &alen,
                           &aInts, &aReals, &aStr);
    if (stat == EGADS_SUCCESS) {
      if (atype != ATTRSTRING) {
        if (outLevel > 0)
...
#ifdef __CUDACC__
        if ((EGlite_strncmp(aStr,"OFF",3) == 0) || (EGlite_strncmp(aStr,"Off",3) == 0) ||
            (EGlite_strncmp(aStr,"off",3) == 0)) {
#else
  ...
  for (m = ntot = i = 0; i < nloop; i++) {
    stat = EGlite_getTopology(loops[i], &geom, &oclass, &mtype, NULL, &nedge,
                          &edges, &senses);
    if (stat != EGADS_SUCCESS) return stat;
    for (j = 0; j < nedge; j++) {
      k = EGlite_indexBodyTopo(body, edges[j]);
      if (k <= EGADS_SUCCESS) {
        printf("%lX EGADS Error: Face %d -> Can not find Edge = %d!\n",
               tID, iFace, k);
        return EGADS_NOTFOUND;
      }
      stat = EGlite_getTopology(edges[j], &geom, &oclass, &mtype, trange, &nd,
                            &nds, &sns);
      if (stat != EGADS_SUCCESS) return stat;
#ifndef FULLDEGEN
      if (mtype == DEGENERATE) continue;
#endif
      stat = EGlite_getTessEdge(tess, k, &npts, &xyzs, &tps);
      if (stat != EGADS_SUCCESS) return stat;
      ntot += npts-1;
    }
    if (nedge != 0) m++;
  ...
  n *= CHUNK;
  if (ts->verts == NULL) {
    ts->verts = (triVert *) EGlite_alloc(n*sizeof(triVert));
    if (ts->verts == NULL) return EGADS_MALLOC;
    ts->mverts = n;
  } else {
    if (n > ts->mverts) {
      tv = (triVert *) EGlite_reall(ts->verts, n*sizeof(triVert));
  ...
  n *= CHUNK;
  if (ts->segs == NULL) {
    ts->segs = (triSeg *) EGlite_alloc(n*sizeof(triSeg));
    if (ts->segs == NULL) return EGADS_MALLOC;
    ts->msegs = n;
  } else {
    if (n > ts->msegs) {
      tsg = (triSeg *) EGlite_reall(ts->segs, n*sizeof(triSeg));
  ...
  if (nloop > ts->mloop) {
    if (ts->loop == NULL) {
      ts->loop = (int *) EGlite_alloc(nloop*sizeof(int));
      if (ts->loop == NULL) return EGADS_MALLOC;
#ifdef DEBUG
      printf(" Alloc Loop: with %d\n", nloop);
#endif
    } else {
      tmp = (int *) EGlite_reall(ts->loop, nloop*sizeof(int));
  ...
  ts->nloop = nloop;
  uvs = (double *) EGlite_alloc((ntot*2+2)*sizeof(double) + ntot*sizeof(int));
  ...
  for (nfig8 = i = 0; i < nloop; i++) {
    ...
    for (degen = ntot = j = 0; j < nedge; j++, n += ori*lor) {
      k = EGlite_indexBodyTopo(body, edges[n]);
      if (k <= EGADS_SUCCESS) {
        printf("%lX EGADS Error: Face %d -> Can not find Edge = %d!\n",
               tID, iFace, k);
        EGlite_free(uvs);
      ...
      stat = EGlite_getTopology(edges[n], &geom, &oclass, &mtype, trange, &nd,
                            &nds, &sns);
      if (stat != EGADS_SUCCESS) {
        EGlite_free(uvs);
        return stat;
      ...
      for (m = 0; m < nedge; m++) {
        if (m == n) continue;
        if (k == EGlite_indexBodyTopo(body, edges[m])) {
          uvm[0] = uvm[1] = -1.0;
          uvp[0] = uvp[1] =  1.0;
          EGlite_getEdgeUV(face, edges[n], -1, 0.5*(trange[0]+trange[1]), uvm);
          EGlite_getEdgeUV(face, edges[n],  1, 0.5*(trange[0]+trange[1]), uvp);
          if ((uvm[0] == uvp[0]) && (uvm[1] == uvp[1]) && (intEdg == NULL)) {
            ...
            intEdg = (double *) EGlite_alloc(4*npts*sizeof(double));
            if (intEdg == NULL) continue;
            for (m = 0; m < npts; m++) {
              stat = EGlite_getEdgeUV(face, edges[n], senses[n]*lor, tps[m],
                                  &intEdg[4*m]);
              if (stat != EGADS_SUCCESS) {
      ...
      if (sen == 1) {
        for (m = 0; m < npts-1; m++, np++) {
          ...
          if (intEdg == NULL) {
            stat = EGlite_getEdgeUV(face, edges[n], senses[n]*lor, tps[m], &uvs[2*np]);
            if (stat != EGADS_SUCCESS) {
              printf("%lX EGADS Error: getEdgeUV+ = %d  for Face %d/%d, Edge = %d\n",
                     tID, stat, iFace, i+1, n+1);
              EGlite_free(uvs);
          ...
          if (m == 0) {
            ...
            ts->verts[np-1].index  = EGlite_indexBodyTopo(body, nds[0]);
            ts->verts[np-1].loop   = i+1;
        ...
        for (m = npts-1; m > 0; m--, np++) {
          ...
          if (m == npts-1) {
            ...
            ts->verts[np-1].edge    = 0;
            if (mtype == TWONODE) {
              ts->verts[np-1].index = EGlite_indexBodyTopo(body, nds[1]);
            } else {
              ts->verts[np-1].index = EGlite_indexBodyTopo(body, nds[0]);
            }
            ts->verts[np-1].loop    = i+1;
...
#endif
      if (intEdg != NULL) EGlite_free(intEdg);
      ntot += npts-1;
    }
...
/*    printf(" %d %d   %d %d   %d %d   %d\n", jm, ts->verts[jm-1].loop,
  ...
  if ((nloop == 1) && (ts->qparm[0] >= 0.0)) {
    /* avoid all together? */
    stat = EGlite_loopQuad(ts->qparm[0], ts->lens, &uvs[2], ts);
  ...
  tris = (int *) EGlite_alloc(3*ntri*sizeof(int));
  if (tris == NULL) {
/*@-kepttrans@*/
    EGlite_free(uvs);
/*@+kepttrans@*/
  ...
  n = EGlite_fillArea(nloop, ts->loop, uvs, tris, &nfig8, 0, fa);
  
  /* adjust for figure 8 configurations */
  if (nfig8 != 0) {
    printf("%lX EGlite_fillArea Warning: Face %d -> Found %d figure 8's!\n",
           tID, iFace, nfig8);
    for (i = 0; i < nfig8; i++) if (n+2*i == ntri) ntri = n;
  }
#ifdef DEBUG
  printf("%lX:   EGlite_fillArea = %d (%d),  #loops = %d, or = %d,  #fig8 = %d\n",
  ...
  if (n != ntri) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      n = EGlite_fillArea(nloop, ts->loop, uvs, tris, &nfig8, 1, fa);
...
/*@-kepttrans@*/
  if (ts->uvs == NULL) EGlite_free(uvs);
/*@+kepttrans@*/
  if (n != ntri) {
    EGlite_free(tris);
    if (ts->uvs != NULL) EGlite_free(ts->uvs);
    return EGADS_DEGEN;
  }
  ...
  n *= CHUNK;
  if (ts->tris == NULL) {
    ts->tris = (triTri *) EGlite_alloc(n*sizeof(triTri));
    if (ts->tris == NULL) {
      EGlite_free(tris);
      if (ts->uvs != NULL) EGlite_free(ts->uvs);
      return EGADS_MALLOC;
    ...
    if (n > ts->mtris) {
      tt = (triTri *) EGlite_reall(ts->tris, n*sizeof(triTri));
      if (tt == NULL) {
        EGlite_free(tris);
        if (ts->uvs != NULL) EGlite_free(ts->uvs);
        return EGADS_MALLOC;
  ...
  stat = EGlite_makeNeighbors(ts, iFace);
  if (stat != EGADS_SUCCESS) {
    if (ts->uvs != NULL) EGlite_free(ts->uvs);
    return stat;
  }
  ...
  stat = EGlite_tessellate(outLevel, ts, tID);
  if (stat == EGADS_SUCCESS) {
    /* set it in the tessellation structure */
    EGlite_updateTris(ts, btess, iFace);
  }
...
__HOST_AND_DEVICE__ void
EGlite_cleanupTess(egTessel *btess)
{
  int i;
  
  if (btess->xyzs != NULL) EGlite_free(btess->xyzs);
  
  if (btess->tess1d != NULL) {
    for (i = 0; i < btess->nEdge; i++) {
      if (btess->tess1d[i].faces[0].faces != NULL)
        EGlite_free(btess->tess1d[i].faces[0].faces);
      if (btess->tess1d[i].faces[1].faces != NULL)
        EGlite_free(btess->tess1d[i].faces[1].faces);
      if (btess->tess1d[i].faces[0].tric  != NULL)
        EGlite_free(btess->tess1d[i].faces[0].tric);
      if (btess->tess1d[i].faces[1].tric  != NULL)
        EGlite_free(btess->tess1d[i].faces[1].tric);
      if (btess->tess1d[i].xyz    != NULL)
        EGlite_free(btess->tess1d[i].xyz);
      if (btess->tess1d[i].t      != NULL)
        EGlite_free(btess->tess1d[i].t);
      if (btess->tess1d[i].global != NULL)
        EGlite_free(btess->tess1d[i].global);
    }
    EGlite_free(btess->tess1d);
  }
  
  if (btess->tess2d != NULL) {
    for (i = 0; i < 2*btess->nFace; i++) {
      if (btess->tess2d[i].mKnots != NULL)
        EGlite_deleteObject(btess->tess2d[i].mKnots);
      if (btess->tess2d[i].xyz    != NULL) 
        EGlite_free(btess->tess2d[i].xyz);
      if (btess->tess2d[i].uv     != NULL) 
        EGlite_free(btess->tess2d[i].uv);
      if (btess->tess2d[i].global != NULL)
        EGlite_free(btess->tess2d[i].global);
      if (btess->tess2d[i].ptype  != NULL) 
        EGlite_free(btess->tess2d[i].ptype);
      if (btess->tess2d[i].pindex != NULL) 
        EGlite_free(btess->tess2d[i].pindex);
      if (btess->tess2d[i].bary   != NULL)
        EGlite_free(btess->tess2d[i].bary);
      if (btess->tess2d[i].frame  != NULL)
        EGlite_free(btess->tess2d[i].frame);
      if (btess->tess2d[i].frlps  != NULL)
        EGlite_free(btess->tess2d[i].frlps);
      if (btess->tess2d[i].tris   != NULL) 
        EGlite_free(btess->tess2d[i].tris);
      if (btess->tess2d[i].tric   != NULL) 
        EGlite_free(btess->tess2d[i].tric);
    }
    EGlite_free(btess->tess2d);
  }
  if (btess->globals != NULL) EGlite_free(btess->globals);

}


__HOST_AND_DEVICE__ static double
EGlite_surfInsert(egObject *face, double *uv, int sense, double d, double *dx)
{
  ...
  /* get face limits */
  stat = EGlite_getTopology(face, &ref, &oclass, &mtype, lims, &nc, &chld, &sen);
  if (stat != EGADS_SUCCESS) return -2.0;
  
  /* get normal at mid-point in UV */
  stat  = EGlite_evaluate(face, uv, result);
  ...
  if ((stat != EGADS_SUCCESS) || (count == 20)) {
    stat = EGlite_invEvaluate(face, x1, uv, x2);
    if (stat != EGADS_SUCCESS) return -2.0;
    stat = EGlite_evaluate(face, uv, result);
...
__HOST_AND_DEVICE__ static double
EGlite_surfDist2(egObject *face, const double *xyz0, const double *uv0,
             ...
             const double *uv2)
{
  ...
  stat = EGlite_evaluate(face, uv, data);
...
__HOST_AND_DEVICE__ static int
EGlite_otherFaces(egTess1D tess1d, egObject **faces, int facex, egObject *edge,
              double t0, double th, double t1)
{
  ...
  for (n = 0; n < 2; n++) {
    ...
    for (nf = 0; nf < tess1d.faces[n].nface; nf++) {
      ...
      stat = EGlite_getEdgeUV(faces[face-1], edge, sense, t0, uv0);
      if (stat != EGADS_SUCCESS) return stat;
      stat = EGlite_getEdgeUV(faces[face-1], edge, sense, th, uvh);
      if (stat != EGADS_SUCCESS) return stat;
      stat = EGlite_getEdgeUV(faces[face-1], edge, sense, t1, uv1);
      if (stat != EGADS_SUCCESS) return stat;
      dirw[0] = uv1[0] - uv0[0];
      dirw[1] = uv1[1] - uv0[1];
...
__HOST_AND_DEVICE__ static int
EGlite_effectIndex(egObject *ebody, egObject *eedge, double t)
{
#ifdef LITE
  return EGADS_SUCCESS;
#else
  ...
  stat = EGlite_effectiveMap(eedge, &t, &edge, &torig);
  if (stat != EGADS_SUCCESS) return stat;
  stat = EGlite_getTopology(ebody, &body, &oclass, &mtype, NULL, &nobj, &objs,
                        &senses);
  if (stat != EGADS_SUCCESS) return stat;
  return EGlite_indexBodyTopo(body, edge);
#endif
}
  

__HOST_AND_DEVICE__ static int
EGlite_evalEffect(egObject *eobj, double t, double *result)
{
  int      stat;
#ifndef LITE
  double   torig, xyz[3];
  ...
  stat = EGlite_evaluate(eobj, &t, result);
#ifndef LITE
  if (eobj->oclass == EEDGE) {
    xyz[0] = result[0];
    xyz[1] = result[1];
    xyz[2] = result[2];
    stat   = EGlite_effectiveMap(eobj, &t, &edge, &torig);
    if (stat != EGADS_SUCCESS) return stat;
    stat      = EGlite_evaluate(edge, &torig, result);
    result[0] = xyz[0];
    result[1] = xyz[1];
    result[2] = xyz[2];
  }
...
__HOST_AND_DEVICE__ static int
EGlite_tessEdge(egTessel *btess, egObject **faces, int j, egObject *edge,
            int ignore, long tID)
{
  ...
  if (ignore != 1) {
    if (body->oclass == EBODY) {
      stat = EGlite_getBodyTopos(body, edge, EFACE, &n, &dum);
    } else {
      stat = EGlite_getBodyTopos(body, edge,  FACE, &n, &dum);
    }
    if (stat == EGADS_SUCCESS) {
      for (i = 0; i < n; i++) {
        aStat = EGlite_attrRet3R(dum[i], ".tParams", dx);
        if (aStat == EGADS_SUCCESS)
          for (k = 0; k < 3; k++)
            if ((dx[k] < params[k]) && (dx[k] > 0.0)) params[k] = dx[k];
        aStat = EGlite_attrRet3R(dum[i], ".tParam",  dx);
        if (aStat == EGADS_SUCCESS)
          for (k = 0; k < 3; k++)
    ...
    aStat = EGlite_attrRet3R(edge, ".tParams", dx);
    if (aStat == EGADS_SUCCESS)
      for (i = 0; i < 3; i++)
        if ((dx[i] < params[i]) && (dx[i] > 0.0)) params[i] = dx[i];
    aStat = EGlite_attrRet3R(edge, ".tParam",  dx);
    if (aStat == EGADS_SUCCESS)
      for (i = 0; i < 3; i++)
  ...
  stat = EGlite_getTopology(edge, &geom, &oclass, &mtype, limits,
                        &nnode, &nodes, &senses);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf("%lX EGADS Warning: Edge %d EGlite_getTopology = %d (EGlite_tessEdge)!\n",
             tID, j+1, stat);
    return stat;
  }
  geomtype = 0;
  ...
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf("%lX EGADS Warning: Edge %d EGlite_getTopology0 = %d (EGlite_tessEdge)!\n",
             tID, j+1, stat);
    return stat;
  }
  npts      = 2;
  ...
  if (mtype == TWONODE) {
    ...
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf("%lX EGADS Warning: Edge %d EGlite_getTopology1 = %d (EGlite_tessEdge)!\n",
               tID, j+1, stat);
      return stat;
    }
  ...
  if (mtype == ONENODE) {
    stat = EGlite_tolerance(edge, &dist);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf("%lX EGADS Warning: Edge %d EGlite_tolerance = %d (EGlite_tessEdge)!\n",
                 tID, j+1, stat);
      return stat;
    }
    if (dist < 1.e-7) dist = 1.e-7;
    stat = EGlite_arcLength(edge, limits[0], limits[1], &d);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf("%lX EGADS Warning: Edge %d EGlite_arcLength = %d (EGlite_tessEdge)!\n",
               tID, j+1, stat);
      return stat;
    }
    ...
    if (d < dist) {
      if (outLevel > 0)
        printf("%lX EGADS Warning: Edge %d Tol = %le aLen = %le (EGlite_tessEdge)!\n",
               tID, j+1, dist, d);
      npts = 1;
      btess->tess1d[j].xyz = (double *) EGlite_alloc(3*npts*sizeof(double));
      if (btess->tess1d[j].xyz == NULL) {
        if (outLevel > 0)
          printf("%lX EGADS Error: Alloc %d Pts Edge %d (EGlite_tessEdge)!\n",
                 tID, npts, j+1);
        return EGADS_MALLOC;
      }
      ...
      if (btess->tess1d[j].t == NULL) {
        ...
        if (outLevel > 0)
          printf("%lX EGADS Error: Alloc %d Ts Edge %d (EGlite_tessEdge)!\n",
                 tID, npts, j+1);
        return EGADS_MALLOC;
      }
      for (i = 0; i < npts; i++) {
  ...
  if (ignore != 1) {
    aStat = EGlite_attributeRet(edge, ".tPos", &aType, &aLen, &aInts, &aReals,
                            &aStr);
    if (aStat == EGADS_SUCCESS)
      if ((aType == ATTRSTRING) || (aType == ATTRCSYS) || (aType == ATTRPTR)) {
        printf("%lX EGADS Warning: tPos NonReal Attribute (EGlite_tessEdge)!\n",
               tID);
        ...
        for (k = i = 0; i < aLen; i++)
          if ((aReals[i] <= t[0]) || (aReals[i] >= t[1])) {
            printf("%lX EGADS Error: %d tPos[%d] = %lf [%lf-%lf] (EGlite_tessEdge)!\n",
        ...
        if (k == 0)
          for (i = 0; i < aLen-1; i++)
            if (aReals[i] >= aReals[i+1]) {
              printf("%lX EGADS Error: %d tPos[%d] = %lf %lf NotMono (EGlite_tessEdge)!\n",
                     tID, j+1, i, aReals[i], aReals[i+1]);
              k++;
            }
        if (aLen+2 > MAXELEN) {
          printf("%lX EGADS Error: Edge %d Attr len %d > %d (EGlite_tessEdge)!\n",
        ...
        if (k == 0) {
          ...
          for (i = 0; i < aLen; i++) {
            t[i+1] = aReals[i];
            stat   = EGlite_evaluate(edge, &t[i+1], result);
            if (stat != EGADS_SUCCESS) {
              if (outLevel > 0)
                printf("%lX EGADS Warning: Edge %d EGlite_eval = %d (EGlite_tessEdge)!\n",
                       tID, j+1, stat);
              return stat;
            }
            xyz[i+1][0] = result[0];
    ...
    aStat = EGlite_attributeRet(edge, ".rPos", &aType, &aLen, &aInts, &aReals,
                            &aStr);
    if (aStat == EGADS_SUCCESS)
      if ((aType == ATTRSTRING) || (aType == ATTRCSYS) || (aType == ATTRPTR)) {
        printf("%lX EGADS Warning: rPos NonReal Attribute (EGlite_tessEdge)!\n",
               tID);
      } else if (aType == ATTRINT) {
        if ((aLen == 1) && (aInts[0] == 0)) goto fill1D;
        printf("%lX EGADS Warning: rPos NonReal Attribute (EGlite_tessEdge)!\n",
        ...
        for (k = i = 0; i < aLen; i++)
          if ((aReals[i] <= 0.0) || (aReals[i] >= 1.0)) {
            printf("%lX EGADS Error: %d rPos[%d] = %lf [0 - 1] (EGlite_tessEdge)!\n",
                   tID, j+1, i, aReals[i]);
            k++;
          }
        if (k == 0)
          for (i = 0; i < aLen-1; i++)
            if (aReals[i] >= aReals[i+1]) {
              printf("%lX EGADS Error: %d rPos[%d] = %lf %lf NotMono (EGlite_tessEdge)!\n",
                     tID, j+1, i, aReals[i], aReals[i+1]);
              k++;
            }
        if (aLen+2 > MAXELEN) {
        ...
        if (k == 0) {
          ...
          xyz[aLen+1][2] = xyz[1][2];
          t[aLen+1]      = t[1];
          stat = EGlite_relPosTs(edge, aLen+2, aReals, t, (double *) xyz);
          if (stat == EGADS_SUCCESS) {
            npts = aLen+2;
            goto fill1D;
          }
          printf("%lX EGADS Error: EGlite_relPosTs = %d (EGlite_tessEdge)!\n",
                 tID, stat);
        }
      }
    aStat = EGlite_attributeRet(edge, ".nPos", &aType, &aLen, &aInts, &aReals,
                            &aStr);
    if (aStat == EGADS_SUCCESS)
      if ((aType == ATTRSTRING) || (aType == ATTRCSYS) || (aType == ATTRPTR)) {
        printf("%lX EGADS Warning: nPos NonNumeric Attribute (EGlite_tessEdge)!\n",
               tID);
      } else {
        if (aLen != 1) {
          printf("%lX EGADS Warning: nPos length not 1 (EGlite_tessEdge)!\n", tID);
        }
        if (aType == ATTRREAL) {
          k = aReals[0] + 0.0001;
        } else {
        ...
        xyz[k+1][2] = xyz[1][2];
        t[k+1]      = t[1];
        stat = EGlite_relPosTs(edge, k+2, NULL, t, (double *) xyz);
        if (stat == EGADS_SUCCESS) {
          npts = k+2;
          goto fill1D;
        }
  ...
  stat = EGlite_evaluate(edge, &t[0], result);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf("%lX EGADS Warning: Edge %d EGlite_evaluateM0 = %d (EGlite_tessEdge)!\n",
             tID, j+1, stat);
    EGlite_getRange(edge, range, &i);
  ...
  stat = EGlite_evaluate(edge, &t[1], result);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf("%lX EGADS Warning: Edge %d EGlite_evaluateM1 = %d (EGlite_tessEdge)!\n",
             tID, j+1, stat);
    EGlite_getRange(edge, range, &i);
  ...
  stat = EGlite_tolerance(edge, &tol);
  if (stat == EGADS_SUCCESS)
  ...
  if (body->oclass == EBODY) {
    ...
    for (i = 1; i < eedge->nsegs-1; i++) {
      ...
      stat         = EGlite_getTopology(eedge->segs[i].nstart, &ref, &oclass,
                                    &ntype, xyz[npts-1], &ndum, &dum, &senses);
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf("%lX EGADS Warning: Edge %d EGlite_getTopologyE = %d (EGlite_tessEdge)!\n",
                 tID, j+1, stat);
        return stat;
      }
      npts++;
  ...
  if (mtype != ONENODE) {
    if (body->oclass == EBODY) {
      stat = EGlite_getBodyTopos(body, edge, EFACE, &k, &facs);
    } else {
      stat = EGlite_getBodyTopos(body, edge,  FACE, &k, &facs);
    }
    if ((stat == EGADS_SUCCESS) && (k != 0)) {
      for (i = 0; i < k; i++) {
        if (body->oclass == EBODY) {
          stat = EGlite_getBodyTopos(body, facs[i], EEDGE, &aLen, &edges);
        } else {
          stat = EGlite_getBodyTopos(body, facs[i],  EDGE, &aLen, &edges);
        }
        if (stat != EGADS_SUCCESS) continue;
        if (aLen == 2) {
#ifdef DEBUG
  ...
  C0[0] = C0[1] = 0;
#endif
  iobj[0] = EGlite_effectIndex(body, edge, t[0]);
  iobj[1] = EGlite_effectIndex(body, edge, t[1]);
  ...
  if (geomtype != LINE) {
    ...
    if (mtype == ONENODE) {
      ...
      for (i = 1; i <= 3; i++) {
        ...
        stat      = EGlite_evaluate(edge, &t[i], result);
        if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf("%lX EGADS Warning: Edge %d ONENODE = %d (EGlite_tessEdge)!\n",
                   tID, j+1, stat);
          return stat;
        }
        xyz[i][0] = result[0];
        xyz[i][1] = result[1];
        xyz[i][2] = result[2];
        iobj[i]   = EGlite_effectIndex(body, edge, t[i]);
      }
      npts = 5;
    } else if (f2e != 0) {
      xyz[2][0] = xyz[1][0];
      ...
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf("%lX EGADS Warning: Edge %d TWONODE = %d (EGlite_tessEdge)!\n",
                 tID, j+1, stat);
        return stat;
      }
      xyz[1][0] = result[0];
    ...
    /* put vertex at C0 */
    if (geomtype == BSPLINE) {
      stat = EGlite_getGeometry(geom, &oclass, &btype, &rref, &info, &prv);
      if (stat == EGADS_SUCCESS) {
          ...
          if (k == info[1]-1) {
            ...
            for (l = 1; l < npts; l++) {
              if (fabs(prv[i]-t[l]) < UVTOL) {
                t[l]      = prv[i];
                stat      = EGlite_evaluate(edge, &t[l], result);
                if (stat != EGADS_SUCCESS) return stat;
                xyz[l][0] = result[0];
                xyz[l][1] = result[1];
                xyz[l][2] = result[2];
            ...
            if (k != -1) {
              ...
              t[k+1] = prv[i];
              stat        = EGlite_evaluate(edge, &t[k+1], result);
              if (stat != EGADS_SUCCESS) return stat;
              xyz[k+1][0] = result[0];
              xyz[k+1][1] = result[1];
              xyz[k+1][2] = result[2];
    ...
    if (params[1] > 0.0) {
      ...
      for (i = 0; i < npts-1; i++) {
        d    = 0.5*(t[i]+t[i+1]);
        stat = EGlite_evaluate(edge, &d, result);
        if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf("%lX EGADS Warning: Edge %d SAG = %d (EGlite_tessEdge)!\n",
                   tID, j+1, stat);
          return stat;
        }
        aux[i][0] = result[0];
      ...
      while (npts < MAXELEN) {
        ...
        xyz[k+1][1] = aux[k][1];
        xyz[k+1][2] = aux[k][2];
        iobj[k+1]   = EGlite_effectIndex(body, edge, t[k+1]);
#ifdef SPLITC0
        C0[k+1]     = 0;
#endif
        d    = 0.5*(t[k+1]+t[k+2]);
  ...
  if (params[0] > 0.0) {
    ...
    /* check face uvs
    for (i = 0; i < npts-2; i++) {
      stat = EGlite_otherFaces(btess->tess1d[j], faces, 0, edge,
                           t[i], t[i+1], t[i+2]);
      if (stat == EGADS_SUCCESS) continue;
...
/*    stat = EGlite_otherFaces(btess->tess1d[j], faces, 0, edge,
  ...
  if (geomtype != LINE) {
    ...
    if (params[2] != 0.0) {
      ...
      for (i = 0; i < npts; i++) {
        stat = EGlite_evalEffect(edge, t[i], result);
        if (stat != EGADS_SUCCESS) return stat;
      ...
      while (npts < MAXELEN) {
        ...
        t[k+1] = 0.5*(t[k]+t[k+2]);
        stat   = EGlite_evalEffect(edge, t[k+1], result);
        if (stat != EGADS_SUCCESS) return stat;
        dist   = sqrt(result[3]*result[3] + result[4]*result[4] +
                      result[5]*result[5]);
        if (dist == 0.0) dist = 1.0;
  ...
  if (params[2] > 0.0) {
    ...
    for (n = 0; n < 2; n++) {
      ...
      for (nf = 0; nf < btess->tess1d[j].faces[n].nface; nf++) {
        ...
        if (face <= 0) continue;
        stat = EGlite_getTopology(faces[face-1], &ref, &oclass, &ntype, range,
                              &ndum, &dum, &senses);
        if (stat != EGADS_SUCCESS) continue;
        if (ref != NULL)
          if (ref->mtype == PLANE) continue;
        stat = EGlite_tolerance(faces[face-1], &tol);
        if (stat != EGADS_SUCCESS) continue;
        ...
        for (i = 0; i < npts; i++) {
          aux[i][2] = 1.0;
          stat = EGlite_getEdgeUV(faces[face-1], edge, sense, t[i], aux[i]);
          if (stat != EGADS_SUCCESS) {
            aux[i][2] = 0.0;
          } else {
        ...
        if ((ref != NULL) && ((fabs(result[1]-result[0]) < UVTOL) ||
          ...
          stat = EGlite_getGeometry(ref, &oclass, &btype, &rref, &info, &prv);
          if (stat == EGADS_SUCCESS) {
            if (btype == BSPLINE) {
        ...
        for (i = 0; i < npts-1; i++) {
          ...
          d     = DOT(dx, dx);
          /* get normal at mid-point in UV */
          stat  = EGlite_getEdgeUV(faces[face-1], edge, sense, 0.5*(t[i]+t[i+1]),
                               uv);
          if (stat != EGADS_SUCCESS) {
            aux[i][2] = 0.0;
          } else {
            dot = EGlite_surfInsert(faces[face-1], uv, sense*ntype, d, dx);
            if (dot < -1.1) {
              aux[i][2] = -1.0;
            } else {
              stat = EGlite_otherFaces(btess->tess1d[j], faces, face, edge,
                                   t[i], 0.5*(t[i]+t[i+1]), t[i+1]);
              if (stat != EGADS_SUCCESS) {
                aux[i][2] = -1.0;
              } else {
                if (params[1] != 0.0)
                  sag = EGlite_surfDist2(faces[face-1], xyz[i], aux[i], xyz[i+1],
                                     aux[i+1], dx, uv);
              }
            }
          }
        ...
        while (npts < MAXELEN) {
          ...
          xyz[k+1][2] = result[2];
          aux[k+1][2] = 1.0;
          stat = EGlite_getEdgeUV(faces[face-1], edge, sense, t[k+1],
                              aux[k+1]);
          if (stat != EGADS_SUCCESS) aux[k+1][2] = 0.0;
          stat = EGlite_getEdgeUV(faces[face-1], edge, sense, 0.5*(t[k]+t[k+1]),
                              uv);
          if (stat != EGADS_SUCCESS) {
            aux[k][2] = 0.0;
          } else {
            ...
            if (dot < -1.1) {
              ...
              if (stat != EGADS_SUCCESS) {
                ...
                if (params[1] != 0.0)
                  sag = EGlite_surfDist2(faces[face-1], xyz[k], aux[k], xyz[k+1],
                                     aux[k+1], dx, uv);
              }
            }
            if (aux[k][2] > 0.0)
          ...
          stat = EGlite_getEdgeUV(faces[face-1], edge, sense, 0.5*(t[k+1]+t[k+2]),
                              uv);
          if (stat != EGADS_SUCCESS) {
            aux[k+1][2] = 0.0;
          } else {
            ...
            if (dot < -1.1) {
              ...
              if (stat != EGADS_SUCCESS) {
                ...
                if (params[1] != 0.0)
                  sag = EGlite_surfDist2(faces[face-1], xyz[k+1], aux[k+1], xyz[k+2],
                                     aux[k+2], dx, uv);
              }
            }
            if (aux[k+1][2] > 0.0)
  ...
  for (n = 0; n < 2; n++) {
    ...
    for (nf = 0; nf < btess->tess1d[j].faces[n].nface; nf++) {
      ...
      if (be[0] < 2) {
        stat = EGlite_getEdgeUV(faces[face-1], edge, sense, t[0], aux[0]);
        if (stat == EGADS_SUCCESS) {
          stat = EGlite_evaluate(faces[face-1], aux[0], result);
...
/*            printf("%lX Face #%d: Edge %d, sense %d beg is degenerate!\n",
      ...
      /* end of Edge */
      if (be[1] >= 2) continue;
      stat = EGlite_getEdgeUV(faces[face-1], edge, sense, t[npts-1], aux[1]);
      if (stat != EGADS_SUCCESS) continue;
      stat = EGlite_evaluate(faces[face-1], aux[1], result);
...
/*      printf("%lX Face #%d: Edge %d, sense %d end is degenerate!\n",
  ...
  for (aLen = n = 0; n < 2; n++) {
    for (nf = 0; nf < btess->tess1d[j].faces[n].nface; nf++) {
      ...
      if (face <= 0) continue;
      if (body->oclass == EBODY) {
        stat = EGlite_getBodyTopos(body, faces[face-1], EEDGE, &ndum, NULL);
      } else {
        stat = EGlite_getBodyTopos(body, faces[face-1],  EDGE, &ndum, NULL);
      }
      if (stat != EGADS_SUCCESS) continue;
      aLen += ndum;
    }
  ...
  edges = (egObject **) EGlite_alloc(aLen*sizeof(egObject *));
  if (edges == NULL) goto fill1D;
  stat  = EGlite_getBodyTopos(body, NULL, NODE, &nnode, &nodes);
  ...
  for (aLen = n = 0; n < 2; n++) {
    for (nf = 0; nf < btess->tess1d[j].faces[n].nface; nf++) {
      ...
      if (body->oclass == EBODY) {
        stat = EGlite_getBodyTopos(body, faces[face-1], EEDGE, &ndum, &dum);
      } else {
        stat = EGlite_getBodyTopos(body, faces[face-1],  EDGE, &ndum, &dum);
      }
      if (stat != EGADS_SUCCESS) continue;
      for (i = 0; i < ndum; i++) {
        ...
        aLen++;
        if (nodes != NULL) {
          stat = EGlite_getTopology(dum[i], &ref, &oclass, &ntype, range,
                                &nobj, &objs, &senses);
          if (stat != EGADS_SUCCESS) continue;
          k = EGlite_indexBodyTopo(body, objs[0]);
          if (k > 0) nodes[k-1] = objs[0];
          if (nobj > 1) {
            k = EGlite_indexBodyTopo(body, objs[1]);
  ...
  if (aLen == 0) {
    if (nodes != NULL) EGlite_free(nodes);
    EGlite_free(edges);
    goto fill1D;
  }
  if (nodes != NULL) {
    stat = EGlite_getTopology(edge, &ref, &oclass, &ntype, range,
                          &nobj, &objs, &senses);
    if (stat == EGADS_SUCCESS) {
      k = EGlite_indexBodyTopo(body, objs[0]);
      nodes[k-1] = NULL;
  ...
  for (i = 0; i < npts-1; i++) {
    uv[0]     = 0.5*(t[i]+t[i+1]);
    uv[1]     = 0.0;
    stat      = EGlite_evaluate(edge, uv, result);
    if (stat != EGADS_SUCCESS) continue;
    ...
    aux[i][2] = 0.0;
    for (n = 0; n < aLen; n++) {
      stat     = EGlite_invEvaluate(edges[n], xyzm, uv, result);
      if (stat != EGADS_SUCCESS) continue;
  ...
  while (npts < MAXELEN) {
    ...
    t[n]      = 0.5*(t[n-1]+t[n+1]);
    stat      = EGlite_evaluate(edge, &t[n], result);
    if (stat != EGADS_SUCCESS) {
      if (nodes != NULL) EGlite_free(nodes);
      EGlite_free(edges);
      return stat;
    ...
    for (i = n-1; i <= n; i++) {
      ...
      aux[i][2] = 0.0;
      for (k = 0; k < aLen; k++) {
        stat  = EGlite_invEvaluate(edges[k], xyzm, uv, result);
        if (stat != EGADS_SUCCESS) continue;
        dx[0] = result[0] - xyzm[0];
        dx[1] = result[1] - xyzm[1];
        dx[2] = result[2] - xyzm[2];
  ...
  if (nodes != NULL) {
    ...
    for (n = 0; n < nnode; n++) {
      ...
      if (npts >= MAXELEN)  continue;
      stat = EGlite_getTopology(nodes[n], &ref, &oclass, &ntype, xyzm,
                            &nobj, &objs, &senses);
      if (stat != EGADS_SUCCESS) continue;
      stat = EGlite_invEvaluate(edge, xyzm, uv, result);
      if (stat != EGADS_SUCCESS) continue;
...
fill1D:
  ...
  if (nf > 0) {
    ...
    if (btess->tess1d[j].faces[0].tric == NULL) {
      ...
      btess->tess1d[j].xyz = NULL;
      if (outLevel > 0)
        printf("%lX EGADS Error: Alloc %d Tric- Edge %d (EGlite_tessEdge)!\n",
               tID, npts, j+1);
      return EGADS_MALLOC;
    }
  }
...
EGlite_edgeThread(void *struc)
{
...
#ifdef PROGRESS
  outLevel = EGlite_outLevel(tthread->body);
#endif
  ...
  for (;;) {
    ...
    /* do the work */
    stat = EGlite_tessEdge(tthread->btess, tthread->faces, index,
                       tthread->edges[index], tthread->ignore, ID);
    if (stat != EGADS_SUCCESS)
      printf(" EGADS Warning: Edge %d -> EGlite_tessEdge = %d (EGlite_edgeThread)!\n",
             index+1, stat);
  }
...
__HOST_AND_DEVICE__ static int
EGlite_tessEdges(egTessel *btess, int ignore, /*@null@*/ int *retess)
{
  ...
  if (body->oclass == EBODY) {
    stat = EGlite_getBodyTopos(body, NULL, EEDGE, &nedge, &edges);
    if (stat != EGADS_SUCCESS) return stat;
    stat = EGlite_getBodyTopos(body, NULL, EFACE, &nface, &faces);
    if (stat != EGADS_SUCCESS) return stat;
  } else {
  ...
  if (retess == NULL) {
    if (btess->tess1d == NULL) {
      ...
      if (btess->tess1d == NULL) {
        if (outLevel > 0)
          printf(" EGADS Error: Alloc %d Edges (EGlite_tessEdges)!\n", nedge);
        EGlite_free(faces);
        EGlite_free(edges);
        return EGADS_MALLOC;
      }
            ...
            if (btess->tess1d[n-1].faces[0].nface != 0) {
              if (btess->tess1d[n-1].faces[0].nface == 1) {
                ...
                if (btess->tess1d[n-1].faces[0].faces == NULL) {
                  if (outLevel > 0)
                    printf(" EGADS Error: Alloc (-) Edge %d (EGlite_tessEdges)!\n", n);
                  EGlite_free(faces);
                  EGlite_free(edges);
                  return EGADS_MALLOC;
                }
                ...
                if (finds == NULL) {
                  if (outLevel > 0)
                    printf(" EGADS Error: ReAlloc (-) Edge %d (EGlite_tessEdges)!\n", n);
                  EGlite_free(faces);
                  EGlite_free(edges);
                  return EGADS_MALLOC;
                } 
...
__HOST_AND_DEVICE__ int
EGlite_makeTessGeom(egObject *obj, double *params, int *sizes, egObject **tess)
{
  ...
  *tess = NULL;
  ...
  if  (EGlite_sameThread(obj))        return EGADS_CNTXTHRD;
  outLevel = EGlite_outLevel(obj);
  ...
  if (obj->oclass == SURFACE) {
    ...
    if (obj->mtype == BSPLINE) {
      stat = EGlite_getGeometry(obj, &oclass, &mtype, &ref, &ivec, &rvec);
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: EGlite_getGeometry stat = %d (EGlite_makeTessGeom)!\n",
                 stat);
        return stat;
      }
      ...
      if ((nu < 2) || (nv < 2)) {
        if (outLevel > 0)
          printf(" EGADS Error: Surface size = %d %d (EGlite_makeTessGeom)!\n",
                 nu, nv);
        return EGADS_INDEXERR;
      }
      np *= nv;
    ...
    if (obj->mtype == BSPLINE) {
      ...
      if (nu < 2) {
        if (outLevel > 0)
          printf(" EGADS Error: Curve len = %d (EGlite_makeTessGeom)!\n", nu);
        return EGADS_INDEXERR;
      }
#ifdef SPLINE_KNOT_TESSGEOM
    }
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Blind Malloc (EGlite_makeTessGeom)!\n");
#ifdef SPLINE_KNOT_TESSGEOM
    EGlite_free(ivec);
    EGlite_free(rvec);
#endif
  ...
  dtess = (double *) EGlite_alloc(3*np*sizeof(double));
  if (dtess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Data Malloc (EGlite_makeTessGeom)!\n");
    EGlite_free(btess);
#ifdef SPLINE_KNOT_TESSGEOM
    EGlite_free(ivec);
    EGlite_free(rvec);
  ...
  stat = EGlite_makeObject(context, &gtess);
  if (stat != EGADS_SUCCESS) {
    EGlite_free(dtess);
    EGlite_free(btess);
#ifdef SPLINE_KNOT_TESSGEOM
  ...
  gtess->blind  = btess;
  EGlite_referenceObject(gtess, context);
  EGlite_referenceTopObj(obj,   gtess);
        ...
        for (jj = (j == 0 ? 0 : 1); jj <= jdeg+1; jj++) {
          ...
          for (i = 0; i < iknot-1; i++) {
            for (ii = (i == 0 ? 0 : 1); ii <= ideg+1; ii++, k++) {
              ...
              stat  = EGlite_evaluate(obj, uv, result);
              dtess[3*k  ] = result[0];
              dtess[3*k+1] = result[1];
              dtess[3*k+2] = result[2];
              if (stat == EGADS_SUCCESS) continue;
              if (outLevel > 0)
                printf(" EGADS Warning: %d/%d, %d/%d eval ret = %d (EGlite_makeTessGeom)!\n",
    ...
    if (obj->mtype == BSPLINE) {
      ...
      for (k = i = 0; i < iknot-1; i++) {
        for (ii = (i == 0 ? 0 : 1); ii <= ideg+1; ii++, k++) {
          ...
          if (stat == EGADS_SUCCESS) continue;
          if (outLevel > 0)
            printf(" EGADS Warning: %d/%d evaluate ret = %d  (EGlite_makeTessGeom)!\n",
                   i+ii+1, nu, stat);
        }
      }
...
__HOST_AND_DEVICE__ int
EGlite_getTessGeom(const egObject *tess, int *sizes, double **xyz)
{
  int      outLevel;
  egTessel *btess;
  egObject *obj;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_getTessGeom)!\n");  
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_getTessGeom)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_getTessGeom)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != SURFACE) && (obj->oclass != CURVE)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not a Curve/Surface (EGlite_getTessGeom)!\n");
...
__HOST_AND_DEVICE__ static void
EGlite_deleteQuads(egTessel *btess, int iface)
{
  int i, j;
  
  i = btess->nFace + iface - 1;
  if (btess->tess2d[i].xyz    != NULL) EGlite_free(btess->tess2d[i].xyz);
  if (btess->tess2d[i].uv     != NULL) EGlite_free(btess->tess2d[i].uv);
  if (btess->tess2d[i].ptype  != NULL) EGlite_free(btess->tess2d[i].ptype);
  if (btess->tess2d[i].pindex != NULL) EGlite_free(btess->tess2d[i].pindex);
  for (j = 0; j < btess->tess2d[i].npatch; j++) {
    if (btess->tess2d[i].patch[j].ipts != NULL) 
      EGlite_free(btess->tess2d[i].patch[j].ipts);
    if (btess->tess2d[i].patch[j].bounds != NULL) 
      EGlite_free(btess->tess2d[i].patch[j].bounds);
  }
  EGlite_free(btess->tess2d[i].patch);
...
__HOST_AND_DEVICE__ int
EGlite_moveEdgeVert(egObject *tess, int eIndex, int vIndex, double t)
{
  ...
  if (tess->magicnumber != MAGIC)   return EGADS_NOTOBJ;
  if (tess->oclass != TESSELLATION) return EGADS_NOTTESS;
  if (EGlite_sameThread(tess))          return EGADS_CNTXTHRD;
  outLevel = EGlite_outLevel(tess);
  
  btess = (egTessel *) tess->blind;
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_moveEdgeVert)!\n");  
    return EGADS_NOTFOUND;
  }
  if (btess->done != 1) {
    if (outLevel > 0)
      printf(" EGADS Error: Bad State (EGlite_moveEdgeVerts)!\n");
    return EGADS_TESSTATE;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_moveEdgeVert)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_moveEdgeVert)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_moveEdgeVert)!\n");
    return EGADS_NOTBODY;
  }
  if (obj->mtype == WIREBODY) {
    if (outLevel > 0)
      printf(" EGADS Error: Source is WireBody (EGlite_moveEdgeVert)!\n");
    return EGADS_TOPOERR;
  }
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Edge Tessellations (EGlite_moveEdgeVert)!\n");
    return EGADS_NODATA;  
  }
  if ((eIndex < 1) || (eIndex > btess->nEdge)) {
    if (outLevel > 0)
      printf(" EGADS Error: eIndex = %d [1-%d] (EGlite_moveEdgeVert)!\n",
             eIndex, btess->nEdge);
    return EGADS_INDEXERR;
  }
  if ((vIndex < 2) || (eIndex >= btess->tess1d[eIndex-1].npts)) {
    if (outLevel > 0)
      printf(" EGADS Error: vIndex = %d [2-%d] (EGlite_moveEdgeVert)!\n",
  ...
  if ((t <= btess->tess1d[eIndex-1].t[vIndex-2]) ||
    ...
    if (outLevel > 0)
      printf(" EGADS Error: t = %lf [%lf-%lf] (EGlite_moveEdgeVert)!\n",
             t, btess->tess1d[eIndex-1].t[vIndex-2], 
                btess->tess1d[eIndex-1].t[vIndex]);
    return EGADS_RANGERR;
  }
  if (obj->oclass == EBODY) {
    stat = EGlite_getBodyTopos(obj, NULL, EEDGE, &nedge, &edges);
    if (stat != EGADS_SUCCESS) return stat;
    stat = EGlite_getBodyTopos(obj, NULL, EFACE, &nface, &faces);
  } else {
    stat = EGlite_getBodyTopos(obj, NULL,  EDGE, &nedge, &edges);
    if (stat != EGADS_SUCCESS) return stat;
    stat = EGlite_getBodyTopos(obj, NULL,  FACE, &nface, &faces);
  }
  if (stat != EGADS_SUCCESS) {
    EGlite_free(edges);
    return stat;
  ...
  stat = EGlite_evaluate(edges[eIndex-1], &t, result);
  if (stat != EGADS_SUCCESS) {
    EGlite_free(faces);
    EGlite_free(edges);
    return stat;
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (j = 0; j < nf; j++) {
      ...
      if (iface != 0) {
        ...
        if (EGlite_faceConnIndex(btess->tess1d[eIndex-1].faces[1-m], iface) == 0)
          sense = 0;
        if (m == 0) sense = -sense;
        stat = EGlite_getEdgeUV(faces[iface-1], edges[eIndex-1], sense, t, uv);
        if (stat != EGADS_SUCCESS) {
          EGlite_free(faces);
          EGlite_free(edges);
          return stat;
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (j = 0; j < nf; j++) {
      ...
      if (EGlite_faceConnIndex(btess->tess1d[eIndex-1].faces[1-m], iface) == 0)
        sense = 0;
      if (m == 0) sense = -sense;
      EGlite_getEdgeUV(faces[iface-1], edges[eIndex-1], sense, t, uv);
...
__HOST_AND_DEVICE__ void
EGlite_cleanupTessMaps(egTessel *btess)
{
  int i;
  
  if (btess->xyzs != NULL) {
    EGlite_free(btess->xyzs);
    btess->xyzs = NULL;
  }
  if (btess->tess1d != NULL)
    for (i = 0; i < btess->nEdge; i++) {
...
__HOST_AND_DEVICE__ int
EGlite_deleteEdgeVert(egObject *tess, int eIndex, int vIndex, int dir)
{
  ...
  if ((dir != -1) && (dir != 1)) {
    if (outLevel > 0)
      printf(" EGADS Error: Collapse Dir = %d (EGlite_deleteEdgeVert)!\n",
             dir);  
    return EGADS_RANGERR;
  }
  btess = (egTessel *) tess->blind;
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_deleteEdgeVert)!\n");  
    return EGADS_NOTFOUND;
  }
  if (btess->done != 1) {
    if (outLevel > 0)
      printf(" EGADS Error: Bad State (EGlite_deleteEdgeVerts)!\n");
    return EGADS_TESSTATE;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_deleteEdgeVert)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_deleteEdgeVert)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_deleteEdgeVert)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Edge Tessellations (EGlite_deleteEdgeVert)!\n");
    return EGADS_NODATA;  
  }
  if ((eIndex < 1) || (eIndex > btess->nEdge)) {
    if (outLevel > 0)
      printf(" EGADS Error: eIndex = %d [1-%d] (EGlite_deleteEdgeVert)!\n",
             eIndex, btess->nEdge);
    return EGADS_INDEXERR;
  }
  if ((vIndex < 2) || (eIndex >= btess->tess1d[eIndex-1].npts)) {
    if (outLevel > 0)
      printf(" EGADS Error: vIndex = %d [2-%d] (EGlite_deleteEdgeVert)!\n",
             vIndex, btess->tess1d[eIndex-1].npts-1);
    return EGADS_INDEXERR;
  }
  
  /* cleanup mappings */
  EGlite_cleanupTessMaps(btess);
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (n = 0; n < nf; n++) {
      ...
      if ((iv[0] == -1) || (iv[1] == -1)) {
        printf(" EGADS Internal: EGlite_deleteEdgeVert Verts = %d %d!\n",
               iv[0], iv[1]);
      ...
      if (n1 > 0) {
        ...
        ie  = -n1;
        ref = EGlite_faceConnIndex(btess->tess1d[ie-1].faces[0], iface);
        ...
        ref = EGlite_faceConnIndex(btess->tess1d[ie-1].faces[1], iface);
...
#ifdef CHECK
  EGlite_checkTriangulation(btess);
#endif
...
__HOST_AND_DEVICE__ int
EGlite_insertEdgeVerts(egObject *tess, int eIndex, int vIndex, int npts,
                   double *t)
{
  ...
  if (npts <= 0) {
    if (outLevel > 0)
      printf(" EGADS Error: Zero Inserts (EGlite_insertEdgeVerts)!\n");
    return EGADS_RANGERR;
  }
  for (i = 0; i < npts-1; i++)
    if (t[i+1] <= t[i]) {
      if (outLevel > 0)
        printf(" EGADS Error: Ts are NOT monitonic (EGlite_insertEdgeVerts)!\n");  
      return EGADS_RANGERR;
    }
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_insertEdgeVerts)!\n");  
    return EGADS_NOTFOUND;
  }
  if (btess->done != 1) {
    if (outLevel > 0)
      printf(" EGADS Error: Bad State (EGlite_insertEdgeVerts)!\n");
    return EGADS_TESSTATE;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_insertEdgeVerts)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_insertEdgeVerts)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_insertEdgeVerts)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Edge Tessellations (EGlite_insertEdgeVerts)!\n");
    return EGADS_NODATA;  
  }
  if ((eIndex < 1) || (eIndex > btess->nEdge)) {
    if (outLevel > 0)
      printf(" EGADS Error: eIndex = %d [1-%d] (EGlite_insertEdgeVerts)!\n",
             eIndex, btess->nEdge);
    return EGADS_INDEXERR;
  }
  if ((vIndex < 1) || (vIndex >= btess->tess1d[eIndex-1].npts)) {
    if (outLevel > 0)
      printf(" EGADS Error: vIndex = %d [1-%d] (EGlite_insertEdgeVerts)!\n",
             vIndex, btess->tess1d[eIndex-1].npts-1);
    return EGADS_INDEXERR;
  }
  if ((t[0]      <= btess->tess1d[eIndex-1].t[vIndex-1]) ||
    ...
    if (outLevel > 0)
      printf(" EGADS Error: t = %lf %lf [%lf-%lf] (EGlite_insertEdgeVerts)!\n",
    ...
    for (nx = 0; nx < nf; nx++) {
      ...
      if (btess->tess2d[iface-1].pindex[i1] == btess->tess2d[iface-1].pindex[i2])
        if ((btess->tess2d[iface-1].ptype[i1] == 0) &&
          ...
          if (outLevel > 0) {
            printf(" EGADS Error: Degen EDGE (EGlite_insertEdgeVerts)!\n");
  ...
  stripe = 3 + 2*cnt;
  vals   = (double *) EGlite_alloc(stripe*npts*sizeof(double));
  if (vals == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on Tmp %d %d (EGlite_insertEdgeVerts)!\n",
             npts, stripe);
    return EGADS_MALLOC;
  }
  if (obj->oclass == EBODY) {
  ...
  for (i = 0; i < npts; i++) {
    stat = EGlite_evaluate(edges[eIndex-1], &t[i], result);
    if (stat != EGADS_SUCCESS) {
      EGlite_free(faces);
      EGlite_free(edges);
      EGlite_free(vals);
    ...
    for (cnt = m = 0; m < 2; m++) {
      ...
      for (nx = 0; nx < nf; nx++) {
        ...
        if (EGlite_faceConnIndex(btess->tess1d[eIndex-1].faces[1-m], iface) == 0)
          sense = 0;
        if (m == 0) sense = -sense;
        stat = EGlite_getEdgeUV(faces[iface-1], edges[eIndex-1], sense, 
  ...
  xyzs = (double *) EGlite_alloc(3*(npts+btess->tess1d[eIndex-1].npts)*
                             sizeof(double));
  ts   = (double *) EGlite_alloc(  (npts+btess->tess1d[eIndex-1].npts)*
                             sizeof(double));
  if ((xyzs == NULL) || (ts == NULL)) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on Edge %d %d (EGlite_insertEdgeVerts)!\n",
             npts, btess->tess1d[eIndex-1].npts);
    if (ts   != NULL) EGlite_free(ts);
    if (xyzs != NULL) EGlite_free(xyzs);
    EGlite_free(vals);
    return EGADS_MALLOC;
  }
  ...
  if (nf > 0) {
    etric[0] = (int *) EGlite_alloc(nf*(npts+btess->tess1d[eIndex-1].npts-1)*
                                sizeof(int));
    if (etric[0] == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Malloc on Edge- %d %d (EGlite_insertEdgeVerts)!\n",
  ...
  if (nf > 0) {
    etric[1] = (int *) EGlite_alloc(nf*(npts+btess->tess1d[eIndex-1].npts-1)*
                                sizeof(int));
    if (etric[1] == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Malloc on Edge+ %d %d (EGlite_insertEdgeVerts)!\n",
               npts, btess->tess1d[eIndex-1].npts-1);
      if (etric[0] != NULL) EGlite_free(etric[0]);
      EGlite_free(ts);
      EGlite_free(xyzs);
      EGlite_free(vals);
      return EGADS_MALLOC;
  ...
  for (cnt = m = 0; m < 2; m++) {
    ...
    for (nx = 0; nx < nf; nx++) {
      ...
      if (nf > 1) iface = btess->tess1d[eIndex-1].faces[m].faces[nx];
      if (iface == 0) continue;
      xyz    = (double *) EGlite_alloc(3*(npts+btess->tess2d[iface-1].npts)*
                                   sizeof(double));
      uv     = (double *) EGlite_alloc(2*(npts+btess->tess2d[iface-1].npts)*
                                   sizeof(double));
      ptype  = (int *)    EGlite_alloc(  (npts+btess->tess2d[iface-1].npts)*
                                   sizeof(int));
      pindex = (int *)    EGlite_alloc(  (npts+btess->tess2d[iface-1].npts)*
                                   sizeof(int));
      tris   = (int *)    EGlite_alloc(3*(npts+btess->tess2d[iface-1].ntris)*
                                   sizeof(int));
      tric   = (int *)    EGlite_alloc(3*(npts+btess->tess2d[iface-1].ntris)*
                                   sizeof(int));
      if ((xyz == NULL)    || (uv == NULL)   || (ptype == NULL) ||
          (pindex == NULL) || (tris == NULL) || (tric == NULL)) {
        if (outLevel > 0)
        ...
        if (xyz      != NULL) EGlite_free(xyz);
        if (etric[0] != NULL) EGlite_free(etric[0]);
        if (etric[1] != NULL) EGlite_free(etric[1]);
        EGlite_free(ts);
        EGlite_free(xyzs);
        EGlite_free(vals);
        if (cnt != 0) EGlite_deleteObject(tess);
        return EGADS_MALLOC;
      }
      for (i = 0; i < btess->tess2d[iface-1].npts; i++) {
...
/*    printf("       %d: %d", iface, itri); 
      ...
      if (btess->tess2d[iface-1].xyz    != NULL) 
        EGlite_free(btess->tess2d[iface-1].xyz);
      if (btess->tess2d[iface-1].uv     != NULL) 
        EGlite_free(btess->tess2d[iface-1].uv);
      if (btess->tess2d[iface-1].ptype  != NULL) 
        EGlite_free(btess->tess2d[iface-1].ptype);
      if (btess->tess2d[iface-1].pindex != NULL) 
        EGlite_free(btess->tess2d[iface-1].pindex);
      if (btess->tess2d[iface-1].bary   != NULL)
        EGlite_free(btess->tess2d[iface-1].bary);
      if (btess->tess2d[iface-1].frame  != NULL)
        EGlite_free(btess->tess2d[iface-1].frame);
      ...
      if (btess->tess2d[iface-1].tris   != NULL)
        EGlite_free(btess->tess2d[iface-1].tris);
      if (btess->tess2d[iface-1].tric   != NULL) 
        EGlite_free(btess->tess2d[iface-1].tric);
...
EGlite_tessThread(void *struc)
{
  ...
  for (;;) {
    ...
    /* adjust the parameters? */
    if (tthread->ignore != 1) {
      aStat = EGlite_attrRet3R(tthread->faces[index], ".tParams", aReals);
      if (aStat == EGADS_SUCCESS) {
      ...
      aStat = EGlite_attrRet3R(tthread->faces[index], ".tParam", aReals);
      if (aStat == EGADS_SUCCESS) {
        params[0] = tthread->params[0];
        params[1] = tthread->params[1];
        params[2] = tthread->params[2];
      ...
      tst.qparm[1] = tthread->qparam[1];
      tst.qparm[2] = tthread->qparam[2];
      aStat = EGlite_attrRet3R(tthread->faces[index], ".qParams", aReals);
      if (aStat == EGADS_SUCCESS)
    ...
    if ((stat != EGADS_SUCCESS) && (tthread->silent == 0))
      printf(" EGADS Warning: Face %d -> EGlite_fillTris = %d (EGlite_tessThread)!\n",
             index+1, stat);
...
__HOST_AND_DEVICE__ int
EGlite_makeTessBody(egObject *object, double *paramx, egObject **tess)
{
  ...
  *tess = NULL;
  ...
  if  (EGlite_sameThread(context))       return EGADS_CNTXTHRD;
  ...
  params[2] =      paramx[2];
  if (ignore != 1) {
    aStat   = EGlite_attrRet3R(object, ".tParams", rparm);
    if (aStat == EGADS_SUCCESS)
      for (i = 0; i < 3; i++)
  ...
  aStat = EGlite_attrRet3R(object, ".qParams", rparm);
  
  btess = (egTessel *) EGlite_alloc(sizeof(egTessel));
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Blind Malloc (EGlite_makeTessBody)!\n");
    return EGADS_MALLOC;
  }
  btess->src       = object;
  btess->xyzs      = NULL;
  ...
  stat = EGlite_tessEdges(btess, ignore, NULL);
  if (stat != EGADS_SUCCESS) {
  ...
  *tess = ttess;
  ...
  if (object->oclass == EBODY) {
    stat = EGlite_getBodyTopos(object, NULL, EFACE, &nface, &faces);
  } else {
    stat = EGlite_getBodyTopos(object, NULL,  FACE, &nface, &faces);
  }
  if (stat != EGADS_SUCCESS) {
    printf(" EGADS Error: EGlite_getBodyTopos = %d (EGlite_makeTessBody)!\n",
           stat);
  ...
  if (btess->tess2d == NULL) {
    printf(" EGADS Error: Alloc %d Faces (EGlite_makeTessBody)!\n", nface);  
    EGlite_deleteObject(ttess);
    *tess = NULL;
    return EGADS_MALLOC;
  }
  ...
  aStat = EGlite_attributeRet(object, ".silent", &aType, &aLen, &aInts,
                          &aReals, &aStr);
  ...
  if (threads != NULL)
    for (i = 0; i < np-1; i++) {
      threads[i] = EMP_LITE_ThreadCreate(EGlite_tessThread, &tthread);
      if (threads[i] == NULL)
        printf(" EMP Error Creating Thread #%d!\n", i+1);
    }
  /* now run the thread block from the original thread */
  EGlite_tessThread(&tthread);
  ...
  if (i != 0) {
    ...
    qints = (int *) EGlite_alloc(nface*sizeof(int));
    if (qints != NULL) {
      for (j = 0; j < nface; j++) {
        qints[j] = 0;
        if (btess->tess2d[j].tfi == 1) {
      ...
      stat = EGlite_attributeAdd(ttess, ".mixed", ATTRINT, nface, qints, NULL, NULL);
      if (stat != EGADS_SUCCESS)
        if (outLevel > 0)
          printf(" EGADS Warning: EGlite_attributeAdd m = %d (EGlite_makeTessBody)!\n",
                 stat);
      EGlite_free(qints);
      stat = EGlite_attributeAdd(ttess, ".tessType", ATTRSTRING, 5, NULL, NULL,
                             "Mixed");
      if (stat != EGADS_SUCCESS)
        if (outLevel > 0)
          printf(" EGADS Warning: EGlite_attributeAdd T = %d (EGlite_makeTessBody)!\n",
                 stat);
    }
  }
#endif
...
__HOST_AND_DEVICE__ int
EGlite_remakeTess(egObject *tess, int nobj, egObject **objs, double *paramx)
{
  ...
  for (hit = j = 0; j < nobj; j++) {
    if (objs[j] == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: NULL Object[%d] (EGlite_remakeTess)!\n",
               j+1);
      return EGADS_NULLOBJ;
    }
    if (objs[j]->magicnumber != MAGIC) {
      if (outLevel > 0)
        printf(" EGADS Error: Not an Object[%d] (EGlite_remakeTess)!\n",
               j+1);
      return EGADS_NOTOBJ;
    }
    if ((objs[j]->oclass != EDGE) && (objs[j]->oclass != FACE)) {
      if (outLevel > 0)
        printf(" EGADS Error: Not Edge/Face[%d] (EGlite_remakeTess)!\n",
               j+1);
      return EGADS_NOTOBJ;
    }
    stat = EGlite_indexBodyTopo(object, objs[j]);
    if (stat == EGADS_NOTFOUND) {
      if (outLevel > 0)
        printf(" EGADS Error: Object[%d] Not in Body (EGlite_remakeTess)!\n",
               j+1);
      return stat;
    }
    ...
    if (objs[j]->mtype == DEGENERATE) {
      if (outLevel > 0)
        printf(" EGADS Error: Edge[%d] is DEGENERATE (EGlite_remakeTess)!\n",
               j+1);
      return EGADS_DEGEN;
    }
    hit++;
  ...
  if (btess->nFace != 0) {
    marker = (int *) EGlite_alloc(btess->nFace*sizeof(int));
    if (marker == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: MALLOC on %d Faces (EGlite_remakeTess)!\n",
               btess->nFace);
      return EGADS_MALLOC;
    }
    for (j = 0; j < btess->nFace; j++) marker[j] = 0;
    for (j = 0; j < nobj; j++) {
      i = EGlite_indexBodyTopo(object, objs[j]);
  ...
  if (hit != 0) {
    ed = (int *) EGlite_alloc(btess->nEdge*sizeof(int));
    if (ed == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: MALLOC on %d Faces (EGlite_remakeTess)!\n",
               btess->nFace);
    ...
    for (j = 0; j < nobj; j++) {
      ...
      i = EGlite_indexBodyTopo(object, objs[j]);
      if (btess->tess1d[i-1].xyz != NULL) EGlite_free(btess->tess1d[i-1].xyz);
      if (btess->tess1d[i-1].t   != NULL) EGlite_free(btess->tess1d[i-1].t);
      if (btess->tess1d[i-1].faces[0].tric  != NULL)
        EGlite_free(btess->tess1d[i-1].faces[0].tric);
      if (btess->tess1d[i-1].faces[1].tric  != NULL)
        EGlite_free(btess->tess1d[i-1].faces[1].tric);
    ...
    btess->params[1] = params[1];
    btess->params[2] = params[2];
    stat = EGlite_tessEdges(btess, 0, ed);
    ...
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_tessEdges =  %d (EGlite_remakeTess)!\n",
               stat);
      EGlite_free(marker);
      return stat;
    }
  ...
  if (stat != EGADS_SUCCESS) {
    printf(" EGADS Error: EGlite_getBodyTopos = %d (EGlite_remakeTess)!\n",
           stat);
    EGlite_free(marker);
    return stat;
  }
  ...
  for (j = 0; j < btess->nFace; j++) {
    ...
    if (btess->tess2d[j].xyz    != NULL) EGlite_free(btess->tess2d[j].xyz);
    if (btess->tess2d[j].uv     != NULL) EGlite_free(btess->tess2d[j].uv);
    if (btess->tess2d[j].ptype  != NULL) EGlite_free(btess->tess2d[j].ptype);
    if (btess->tess2d[j].pindex != NULL) EGlite_free(btess->tess2d[j].pindex);
    if (btess->tess2d[j].bary   != NULL) EGlite_free(btess->tess2d[j].bary);
    if (btess->tess2d[j].frlps  != NULL) EGlite_free(btess->tess2d[j].frlps);
    if (btess->tess2d[j].frame  != NULL) EGlite_free(btess->tess2d[j].frame);
    if (btess->tess2d[j].tris   != NULL) EGlite_free(btess->tess2d[j].tris);
    if (btess->tess2d[j].tric   != NULL) EGlite_free(btess->tess2d[j].tric);
    btess->tess2d[j].xyz    = NULL;
    btess->tess2d[j].uv     = NULL;
    btess->tess2d[j].ptype  = NULL;
    btess->tess2d[j].pindex = NULL;
...
__HOST_AND_DEVICE__ int
EGlite_finishTess(egObject *tess, double *paramx)
{
  ...
  if (j != 0) {
    ed = (int *) EGlite_alloc(btess->nEdge*sizeof(int));
    if (ed == NULL) {
      printf(" EGADS Error: Allocating %d Edge ints (EGlite_finishTess)!\n",
             btess->nEdge);
      return EGADS_MALLOC;
    }
    for (i = 0; i < btess->nEdge; i++) {
    ...
    stat = EGlite_tessEdges(btess, ignore, ed);
    EGlite_free(ed);
    if (stat != EGADS_SUCCESS) {
      printf(" EGADS Error: EGlite_tessEdges = %d (EGlite_finishTess)!\n",
             stat);
      return stat;
    }
  }
  ...
  if (stat != EGADS_SUCCESS) {
    printf(" EGADS Error: EGlite_getBodyTopos = %d (EGlite_finishTess)!\n",
           stat);
    return stat;
  }
  ...
  qints = (int *) EGlite_alloc(nface*sizeof(int));
  if (qints == NULL) {
    printf(" EGADS Warning: Allocating %d Face ints (EGlite_finishTess)!\n", nface);
  } else {
    for (i = 0; i < nface; i++) qints[i] = 0;
    stat = EGlite_attributeRet(tess, ".mixed", &type, &aLen, &aInts, &reals, &aStr);
    if (stat == EGADS_SUCCESS) {
      if ((aLen == nface) && (type == ATTRINT)) {
        for (i = 0; i < nface; i++)
          if (btess->tess2d[i].xyz != NULL) qints[i] = aInts[i];
      } else {
        printf(" EGADS Warning: .mixed attr wrong length/type (EGlite_finishTess)!\n");
      }
#ifndef LITE
      stat = EGlite_attributeDel(tess, ".mixed");
      if (stat != EGADS_SUCCESS)
        printf(" EGADS Warning: delete .mixed attr = %d (EGlite_finishTess)!\n",
               stat);
#endif
    }
  }
...
#ifndef LITE
    if (EGlite_fullAttrs(object) == 1) {
      (void) EGlite_attributeDel(tess, ".mixed");
      (void) EGlite_attributeDel(tess, ".tessType");
    }
    if (j == nface) {
      stat = EGlite_attributeAdd(tess, ".mixed", ATTRINT, nface, qints, NULL, NULL);
      if (stat != EGADS_SUCCESS)
        if (outLevel > 0)
          printf(" EGADS Warning: EGlite_attributeAdd m = %d (EGlite_finishTess)!\n",
                 stat);
      stat = EGlite_attributeAdd(tess, ".tessType", ATTRSTRING, 4, NULL, NULL,
                             "Quad");
      if (stat != EGADS_SUCCESS)
        if (outLevel > 0)
          printf(" EGADS Warning: EGlite_attributeAdd Q = %d (EGlite_finishTess)!\n",
                 stat);
    } else if (j != 0) {
      stat = EGlite_attributeAdd(tess, ".mixed", ATTRINT, nface, qints, NULL, NULL);
      if (stat != EGADS_SUCCESS) {
        ...
        stat = EGlite_attributeAdd(tess, ".tessType", ATTRSTRING, 5, NULL, NULL,
                               "Mixed");
        if (stat != EGADS_SUCCESS)
          if (outLevel > 0)
            printf(" EGADS Warning: EGlite_attributeAdd T = %d (EGlite_finishTess)!\n",
                   stat);
      }
    }
#endif
  ...
  if (threads != NULL) free(threads);
  EGlite_free(faces);
  if (qints != NULL) EGlite_free(qints);
  if (outLevel > 1)
    printf(" EMP Number of Seconds on Face Thread Block = %ld\n",
           EMP_LITE_Done(&start));
...
#ifndef LITE
int
EGlite_mapTessBody(egObject *tess, egObject *body, egObject **mapTess)
{
  ...
  *mapTess = NULL;
  ...
  if (EGlite_sameThread(body))          return EGADS_CNTXTHRD;
  context  = EGlite_context(body);
  outLevel = EGlite_outLevel(body);

  /* get mappings if the attributes exist */
  stat = EGlite_attributeRet(body, ".fMap", &aType, &alen, &fMap, NULL, NULL);
  if (stat == EGADS_SUCCESS) {
    if (aType != ATTRINT) {
      printf(" EGADS Error: fMap Attribute Not an Int (EGlite_mapTessBody)!\n");
      return EGADS_ATTRERR;
    }
    if (alen != btess->nFace) {
      printf(" EGADS Error: fLen Mismatch %d %d (EGlite_mapTessBody)!\n",
             alen, btess->nFace);
      return EGADS_TOPOERR;
    }
    for (i = 0; i < btess->nFace; i++)
      if ((fMap[i] < 1) || (fMap[i] > btess->nFace)) {
        printf(" EGADS Error: Face Map %d = %d [1-%d] (EGlite_mapTessBody)!\n",
               i+1, fMap[i], btess->nFace);
        return EGADS_TOPOERR;
      }
  }
  stat = EGlite_attributeRet(body, ".eMap", &aType, &alen, &eMap, NULL, NULL);
  if (stat == EGADS_SUCCESS) {
    if (aType != ATTRINT) {
      printf(" EGADS Error: eMap Attribute Not an Int (EGlite_mapTessBody)!\n");
      return EGADS_ATTRERR;
    }
    if (alen != btess->nEdge) {
      printf(" EGADS Error: eLen Mismatch %d %d (EGlite_mapTessBody)!\n",
             alen, btess->nEdge);
      return EGADS_TOPOERR;
    }
    for (i = 0; i < btess->nEdge; i++)
      if ((eMap[i] < 1) || (eMap[i] > btess->nEdge)) {
        printf(" EGADS Error: Edge Map %d = %d [1-%d] (EGlite_mapTessBody)!\n",
               i+1, eMap[i], btess->nEdge);
        return EGADS_TOPOERR;
      }
  }
  stat = EGlite_attributeRet(body, ".nMap", &aType, &alen, &nMap, NULL, NULL);
  if (stat == EGADS_SUCCESS) {
    if (aType != ATTRINT) {
      printf(" EGADS Error: nMap Attribute Not an Int (EGlite_mapTessBody)!\n");
      return EGADS_ATTRERR;
    }
    stat = EGlite_getBodyTopos(body, NULL, NODE, &nnode, &nodes);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: getBodyTopo NODES = %d (EGlite_mapTessBody)!\n",
               stat);
      return stat;
    }
    if (alen != nnode) {
      printf(" EGADS Error: nLen Mismatch %d %d (EGlite_mapTessBody)!\n",
             alen, nnode);
      return EGADS_TOPOERR;
    }
    for (i = 0; i < nnode; i++)
      if ((nMap[i] < 1) || (nMap[i] > nnode)) {
        printf(" EGADS Error: Node Map %d = %d [1-%d] (EGlite_mapTessBody)!\n",
               i+1, nMap[i], nnode);
        return EGADS_TOPOERR;
      }
    EGlite_free(nodes);
  ...
  if (fMap == NULL) {
    if ((nMap != NULL) || (eMap != NULL)) {
      if (outLevel > 0)
        printf(" EGADS Error: Node and/or Egde Map exists (EGlite_mapTessBody)!\n");
      return EGADS_NODATA;
    }
    stat = EGlite_sameBodyTopo(tessb, body);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: sameBodyTopo = %d (EGlite_mapTessBody)!\n", stat);
      return stat;
    }
  } else {
    if ((nMap == NULL) || (eMap == NULL)) {
      if (outLevel > 0)
        printf(" EGADS Error: Node and/or Egde Map missing (EGlite_mapTessBody)!\n");
      return EGADS_NODATA;
    }
  }
  ...
  for (j = 0; j < btess->nFace; j++)
    if (btess->tess2d[j].frame == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Frame %d is NULL (EGlite_mapTessBody)!\n", j+1);
      return EGADS_NOTFOUND;
    }
  
  /* make the object data */
  mtess = (egTessel *) EGlite_alloc(sizeof(egTessel));
  if (mtess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Blind Malloc (EGlite_mapTessBody)!\n");
    return EGADS_MALLOC;
  }
  mtess->src       = body;
  mtess->xyzs      = NULL;
  ...
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: getBodyTopo Edges = %d (EGlite_mapTessBody)!\n",
             stat);
    EGlite_cleanupTess(mtess);
    EGlite_free(mtess);
    return stat;
  }

  mtess->tess1d = (egTess1D *) EGlite_alloc(mtess->nEdge*sizeof(egTess1D));
  if (mtess->tess1d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Alloc %d Edges (EGlite_mapTessBody)!\n", mtess->nEdge);
    EGlite_cleanupTess(mtess);
    EGlite_free(mtess);
    return EGADS_MALLOC;
  }
  ...
  if (mtess->nFace > 0) {
    mtess->tess2d = (egTess2D *) EGlite_alloc(2*mtess->nFace*sizeof(egTess2D));
    if (mtess->tess2d == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Alloc %d Faces (EGlite_mapTessBody)!\n", mtess->nFace);
      EGlite_cleanupTess(mtess);
      EGlite_free(mtess);
      return EGADS_MALLOC;
    }
  ...
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: getBodyTopo Faces = %d (EGlite_mapTessBody)!\n",
             stat);
    EGlite_free(nodes);
    EGlite_cleanupTess(mtess);
    EGlite_free(mtess);
  ...
  for (j = 0; j < mtess->nEdge; j++) {
    ...
    if (n > 1) {
      mtess->tess1d[k].faces[0].faces = (int *) EGlite_alloc(n*sizeof(int));
    ...
    if (n > 1) {
      mtess->tess1d[k].faces[0].tric = (int *) EGlite_alloc(n*sizeof(int));
    ...
    if (n > 1) {
      mtess->tess1d[k].faces[1].faces = (int *) EGlite_alloc(n*sizeof(int));
    ...
    n *= mtess->tess1d[k].npts-1;
    if (n > 1) {
      mtess->tess1d[k].faces[1].tric = (int *) EGlite_alloc(n*sizeof(int));
    ...
    if (n <= 0) {
      if (mtess->tess1d[k].obj->mtype != DEGENERATE) {
        if (outLevel > 0)
          printf(" EGADS Error: NonDegenerate 0 pnt Edge %d (EGlite_mapTessBody)!\n",
                 k+1);
        EGlite_free(faces);
        EGlite_free(nodes);
        EGlite_cleanupTess(mtess);
      ...
      if (btess->tess1d[j].t != NULL) {
        if (outLevel > 0)
          printf(" EGADS Error: Degenerate nonNULL Edge %d w/ 0 (EGlite_mapTessBody)!\n",
                 k+1);
        EGlite_free(faces);
        EGlite_free(nodes);
        EGlite_cleanupTess(mtess);
    ...
    mtess->tess1d[k].xyz = (double *) EGlite_alloc(3*n*sizeof(double));
    mtess->tess1d[k].t   = (double *) EGlite_alloc(  n*sizeof(double));
    ...
    stat = EGlite_getTopology(mtess->tess1d[k].obj, &geom, &oclass, &mtype, range,
                          &nc, &child, &senses);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: getTopology Edge %d = %d (EGlite_mapTessBody)!\n",
               k+1, stat);
      EGlite_free(faces);
      EGlite_free(nodes);
      EGlite_cleanupTess(mtess);
    ...
    stat = EGlite_getTopology(child[0], &geom, &oclass, &mtype,
                          &mtess->tess1d[k].xyz[0], &i, &edges, &senses);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: getTopology N0 Edge %d = %d (EGlite_mapTessBody)!\n",
               j+1, stat);
      EGlite_free(faces);
      EGlite_free(nodes);
      EGlite_cleanupTess(mtess);
    ...
    for (i = 1; i < n-1; i++) {
      stat = EGlite_evaluate(mtess->tess1d[k].obj, &mtess->tess1d[k].t[i], result);
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: evaluate %d Edge %d/%d = %d (EGlite_mapTessBody)!\n",
                 i+1, j+1, k+1, stat);
        EGlite_free(faces);
        EGlite_free(nodes);
        EGlite_cleanupTess(mtess);
    ...
    stat = EGlite_getTopology(child[nc-1], &geom, &oclass, &mtype,
                          &mtess->tess1d[k].xyz[3*n-3], &i, &edges, &senses);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: getTopology N1 Edge %d = %d (EGlite_mapTessBody)!\n",
               j+1, stat);
      EGlite_free(faces);
      EGlite_free(nodes);
      EGlite_cleanupTess(mtess);
...
/*          printf(" Edge %d: nseg = %d/%d\n", j+1, i0, n-1);  */
            EGlite_mapTessTs(btess->tess1d[j], mtess->tess1d[k]);
          }
        }
        free(s);
      }
  ...
  if (mtess->tess2d != NULL)
    for (j = 0; j < mtess->nFace; j++) {
      ...
      stat = EGlite_getTopology(faces[k], &geom, &oclass, &mtype, range, &i, &child,
                            &senses);
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: EGlite_getTopology Face %d = %d (EGlite_mapTessBody)!\n",
                 k+1, stat);
        EGlite_free(faces);
        EGlite_free(nodes);
        EGlite_cleanupTess(mtess);
      ...
      if (geom->mtype == BSPLINE) {
        ...
        OK = -1;
        EGlite_getGeometry(geom, &oclass, &mtype, &rGeom, &ivec, &rvec);
        stat = EGlite_getBodyTopos(btess->src, NULL, FACE, &n, &faceb);
        if (stat == EGADS_SUCCESS) {
          tr   = 0.0;
          stat = EGlite_getTopology(faceb[j], &geomm, &oclass, &mtype, range, &i,
                                &child, &senses);
          EGlite_free(faceb);
          if (stat == EGADS_SUCCESS) {
            if (geomm->mtype == BSPLINE) {
              if (outLevel > 1)
                printf(" EGADS Info: Face %d both BSpline (EGlite_mapTessBody)!\n",
                       j+1);
              EGlite_getGeometry(geomm, &oclass, &mtype, &rGeom, &ivecb, &rvecb);
              if ((ivec != NULL) && (ivecb != NULL) &&
                ...
                if ((ivec[3] == ivecb[3]) && (ivec[6] == ivecb[6])) {
                  ...
                  printf(" EGADS Warning: Face %d BSpline #knots (EGlite_mapTessBody)!\n",
                         j+1);
                }
              EGlite_free(ivecb);
              EGlite_free(rvecb);
            } else {
              printf(" EGADS Warning: Face %d BSpline Mismatch (EGlite_mapTessBody)!\n",
                     j+1);
            }
          }
          if (OK == -1) {
            ...
            if (OK != 0)
              printf(" EGADS Info: Face %d BSp knots OK=%d  %le (EGlite_mapTessBody)!\n",
                     j+1, OK, tr);
          }
#ifdef INSERTKNOTS
          if ((OK > 0) && (OK < 4)) {
            stat = EGlite_mapSequen(geomm, geom, &btess->tess2d[k].mKnots);
            if (stat != EGADS_SUCCESS)
              printf(" EGADS Error: mapSequen Face %d = %d (EGlite_mapTessBody)!\n",
                     j+1, stat);
          }
#endif
        }
      ...
      if (btess->tess2d[j].bary == NULL) {
        stat = EGlite_baryFrame(&btess->tess2d[j]);
        if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Error: baryFrame Face %d = %d (EGlite_mapTessBody)!\n",
                   j+1, stat);
          EGlite_free(faces);
          EGlite_free(nodes);
          EGlite_cleanupTess(mtess);
      ...
      /* allocate our space */
      xyz    = (double *) EGlite_alloc(3*mtess->tess2d[k].npts*sizeof(double));
      uv     = (double *) EGlite_alloc(2*mtess->tess2d[k].npts*sizeof(double));
      ptype  = (int *)    EGlite_alloc(  mtess->tess2d[k].npts*sizeof(int));
      pindex = (int *)    EGlite_alloc(  mtess->tess2d[k].npts*sizeof(int));
      bary   = (egBary *) EGlite_alloc(  mtess->tess2d[k].npts*sizeof(egBary));
      frame  = (int *)    EGlite_alloc(3*mtess->tess2d[k].nframe*sizeof(int));
      frlps  = (int *)    EGlite_alloc(  mtess->tess2d[k].nfrlps*sizeof(int));
      tris   = (int *)    EGlite_alloc(3*mtess->tess2d[k].ntris* sizeof(int));
      tric   = (int *)    EGlite_alloc(3*mtess->tess2d[k].ntris* sizeof(int));
      if ((xyz    == NULL) || (uv   == NULL) || (ptype == NULL) ||
          (pindex == NULL) || (tris == NULL) || (tric  == NULL) ||
          (frame  == NULL) || (bary == NULL) || (frlps == NULL)) {
        printf(" EGADS Error: Can't Alloc Tess Memory for %d (EGlite_mapTessBody)!\n",
               k+1);
        if (bary   != NULL) EGlite_free(bary);
        if (tric   != NULL) EGlite_free(tric);
        if (tris   != NULL) EGlite_free(tris);
        if (frlps  != NULL) EGlite_free(frlps);
        if (frame  != NULL) EGlite_free(frame);
      ...
      for (last = iloop = i = 0;
        ...
        if (ptype[i] == 0) {
          /* a node */
          stat = EGlite_getTopology(nodes[pindex[i]-1], &geom, &oclass, &mtype,
                                &xyz[3*i], &nc, &edges, &senses);
          if (stat != EGADS_SUCCESS) {
            if (outLevel > 0)
              printf(" EGADS Error: getTopo N %d Face %d = %d (EGlite_mapTessBody)!\n",
                     pindex[i], k+1, stat);
            EGlite_free(uv);
            EGlite_free(xyz);
            EGlite_free(faces);
          ...
          stat = EGlite_getBodyTopos(body, nodes[pindex[i]-1], EDGE, &nc, &edges);
          if (stat != EGADS_SUCCESS) {
            if (outLevel > 0)
              printf(" EGADS Error: getBodyTopo E Face %d = %d (EGlite_mapTessBody)!\n",
                     k+1, stat);
            EGlite_free(uv);
            EGlite_free(xyz);
            EGlite_free(faces);
          ...
          for (sen = n = 0; n < nc; n++) {
            i2 = EGlite_indexBodyTopo(body, edges[n]);
            i0 = EGlite_faceConnIndex(mtess->tess1d[i2-1].faces[0], k+1);
            i1 = EGlite_faceConnIndex(mtess->tess1d[i2-1].faces[1], k+1);
            ...
            if (mtess->tess1d[i2-1].nodes[0] == mtess->tess1d[i2-1].nodes[1]) {
              ...
              if ((ptype[lst] > 0) && (pindex[lst] == i2)) {
                ...
                if (edges[n]->mtype != DEGENERATE)
                  printf(" EGADS Info: Can't find E=%d dir F=%d (EGlite_mapTessBody)!\n",
                         i2, k+1);
/*               printf("   pointer = %ld   npts = %d\n",
            ...
            stat = EGlite_getEdgeUV(faces[k], edges[n], 0,
                                mtess->tess1d[i2-1].t[ifrm], &uv[2*i]);
/*          if (edges[n]->mtype == DEGENERATE)
              printf(" EGADS Info: Degen UV = %lf %lf\n", uv[2*i], uv[2*i+1]); */
            if (stat != EGADS_SUCCESS) {
              if (outLevel > 0)
                printf(" EGADS Error: getEdgeUV Node %d = %d (EGlite_mapTessBody)!\n",
                       pindex[i], stat);
              EGlite_free(edges);
              EGlite_free(uv);
              EGlite_free(xyz);
            ...
            EGlite_evaluate(faces[k], &uv[2*i], result);
            if (sqrt(result[3]*result[3] + result[4]*result[4] +
 ...
 */
          ...
          EGlite_free(edges);
          if (sen == 0)
            printf(" EGADS Info: Can't Find Node %d dir F=%d (EGlite_mapTessBody)!\n",
                   pindex[i], k+1);
          
        } else if (ptype[i] > 0) {
          
          /* an edge vertex */
          stat = EGlite_evaluate( mtess->tess1d[pindex[i]-1].obj,
                             &mtess->tess1d[pindex[i]-1].t[ptype[i]-1], result);
          if (stat != EGADS_SUCCESS) {
            if (outLevel > 0)
              printf(" EGADS Error: evaluate %d EDGE %d = %d (EGlite_mapTessBody)!\n",
          ...
          i0  = EGlite_faceConnIndex(mtess->tess1d[pindex[i]-1].faces[0], k+1);
          i1  = EGlite_faceConnIndex(mtess->tess1d[pindex[i]-1].faces[1], k+1);
          sen = 0;
          if ((i0 != 0) && (i1 != 0)) {
            ...
            if ((ptype[i] != 0) && (ptype[nxt] != 0) &&
              ...
              if (mtess->tess1d[pindex[i]-1].nodes[0] ==
                ...
                printf(" EGADS Info: Can't Edge %d dir F=%d (EGlite_mapTessBody)!\n",
                       pindex[i], k+1);
              }
            }
/*          printf("  face = %d, pnt = %d %d, sense = %d, sen = %d\n",
          ...
          stat = EGlite_getEdgeUV(faces[k], mtess->tess1d[pindex[i]-1].obj, sen,
                              mtess->tess1d[pindex[i]-1].t[ptype[i]-1], &uv[2*i]);
          if (stat != EGADS_SUCCESS) {
            if (outLevel > 0)
              printf(" EGADS Error: getEdgeUV %d EDGE %d = %d (EGlite_mapTessBody)!\n",
                     ptype[i], pindex[i], stat);
            EGlite_free(uv);
            EGlite_free(xyz);
            EGlite_free(faces);
          ...
          printf(" EGADS Info: Bad Frame Vert %d (%d %d) F=%d (EGlite_mapTessBody)!\n",
      ...
      for (i = mtess->tess2d[k].frlps[mtess->tess2d[k].nfrlps-1];
        ...
        stat = EGlite_evaluate(obj2D, &uv[2*i], result);
        if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Error: evaluate %d Face %d = %d (EGlite_mapTessBody)!\n",
                   i+1, k+1, stat);
          EGlite_free(uv);
          EGlite_free(xyz);
          EGlite_free(faces);
  ...
  stat = EGlite_makeObject(context, &mapObj);
  if (stat != EGADS_SUCCESS) {
    EGlite_cleanupTess(mtess);
    EGlite_free(mtess);
    return stat;
  ...
  mapObj->blind  = mtess;
  EGlite_referenceObject(mapObj, context);
  EGlite_referenceTopObj(body,   mapObj);
  *mapTess = mapObj;
...
__HOST_AND_DEVICE__ int
EGlite_locateTessBody(const egObject *tess, int npts, const int *ifaces,
                  const double *uvs, /*@null@*/ int *itris, double *results)
{
  ...
  stat = EGlite_attributeRet(tessb, ".fMap", &aType, &alen, &ints, &reals, &str);
  if (stat == EGADS_SUCCESS) {
    if (aType != ATTRINT) {
      printf(" EGADS Error: Map Attribute Not an Int (EGlite_locateTessBody)!\n");
      return EGADS_ATTRERR;
    }
    if (alen != btess->nFace) {
      printf(" EGADS Error: Len Mismatch %d %d (EGlite_locateTessBody)!\n",
             alen, btess->nFace);
      return EGADS_TOPOERR;
    }
  }
  ...
  if (itris == NULL) {
    stat = EGlite_getBodyTopos(tessb, NULL, FACE, &nface, &faces);
    if (stat != EGADS_SUCCESS) return stat;

    for (i = 0; i < npts; i++) {
      iface = abs(ifaces[i]);
      if ((iface < 1) || (iface > btess->nFace)) {
        printf(" EGADS Error: %d = %d [1-%d] (EGlite_locateTessBody)!\n",
      ...
      if ((stat == EGADS_SUCCESS) && (ifaces[i] < 0)) {
        if (ints == NULL) {
          printf(" EGADS Error: %d Mapped w/ No Mapping (EGlite_locateTessBody)!\n",
                 i+1);
          EGlite_free(faces);
          return EGADS_INDEXERR;
        }
        iface = ints[iface-1];
        if ((iface < 1) || (iface > btess->nFace)) {
          printf(" EGADS Error: Mapped %d = %d [1-%d] (EGlite_locateTessBody)!\n",
                 i+1, iface, btess->nFace);
          EGlite_free(faces);
          return EGADS_INDEXERR;
        }
      ...
      stat = EGlite_evaluate(obj2D, &uvs[2*i], data);
      if (stat != EGADS_SUCCESS) {
        EGlite_free(faces);
        return stat;
      }
  ...
  for (i = 0; i < npts; i++) {
    ...
    itris[i] = EGlite_baryTess(btess->tess2d[iface-1], &uvs[2*i], &results[3*i]);
  }
...
__HOST_AND_DEVICE__ int
EGlite_getTessQuads(const egObject *tess, int *nquad, int **fIndices)
{
  ...
  *fIndices = NULL;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_getTessQuads)!\n");  
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_getTessQuads)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_getTessQuads)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_getTessQuads)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess2d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_getTessQuads)!\n");
    return EGADS_NODATA;  
  }
  ...
  ivec = (int *) EGlite_alloc(n*sizeof(int));
  if (ivec == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on %d Faces (EGlite_getTessQuads)!\n", n);
    return EGADS_MALLOC;
  }
...
__HOST_AND_DEVICE__ static int
EGlite_quadLoop(egTessel *btess, int outLevel, int nedge, int *eindex, 
...
__HOST_AND_DEVICE__ int
EGlite_makeQuads(egObject *tess, double *parms, int index)
{
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_makeQuads)!\n");  
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_makeQuads)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_makeQuads)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_makeQuads)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess2d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_makeQuads)!\n");
    return EGADS_NODATA;  
  }
  if ((index < 1) || (index > btess->nFace)) {
    if (outLevel > 0)
      printf(" EGADS Error: Index = %d [1-%d] (EGlite_makeQuads)!\n",
             index, btess->nFace);
    return EGADS_INDEXERR;
  }
  ...
  if (stat != EGADS_SUCCESS) return stat;
  stat = EGlite_getTopology(faces[index-1], &geom, &oclass, &ftype, limits,
                        &nloop, &loops, &senses);
  if (stat != EGADS_SUCCESS) {
    EGlite_free(faces);
    return stat;
  ...
  if (nloop != 1) {
    loop = NULL;
    stat = EGlite_attributeRet(faces[index-1], ".innerLoops", &atype, &alen,
                           &aInts, &aReals, &aStr);
    if (stat == EGADS_SUCCESS) {
      if (atype != ATTRSTRING) {
        if (outLevel > 0)
    ...
    if (loop == NULL) {
     if (outLevel > 0)
        printf(" EGADS Error: Face %d has %d loops (EGlite_makeQuads)!\n",
               index, nloop);
      EGlite_free(faces);
      return EGADS_TOPOERR;
    }
  }
  stat = EGlite_getTopology(loop, &geom, &oclass, &mtype, limits, &nedge, &edges,
                        &senses);
  if (stat != EGADS_SUCCESS) {
    EGlite_free(faces);
    return stat;
  ...
  if (nedge < 4) {
    if (outLevel > 0)
      printf(" EGADS Error: %d Edges in Face %d (EGlite_makeQuads)!\n", 
             nedge, index);
    EGlite_free(faces);
    return EGADS_INDEXERR;
  }
  ...
  /* get Edge Indices */
  eindex = (int *) EGlite_alloc(nedge*sizeof(int));
  if (eindex == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on %d Edges (EGlite_makeQuads)!\n",
             nedge);
    EGlite_free(faces);
    return EGADS_MALLOC;
  }
  for (i = 0; i < nedge; i++) {
    if (edges[i]->mtype == DEGENERATE) {
      if (outLevel > 0)
        printf(" EGADS Error: Edge in Face %d is Degenerate (EGlite_makeQuads)!\n", 
               index);
      EGlite_free(eindex);
      EGlite_free(faces);
      return EGADS_INDEXERR;
    ...
    if (eindex[i] == 0) {
      if (outLevel > 0)
        printf(" EGADS Error: Edge Not Found in Tess (EGlite_makeQuads)!\n");
      EGlite_free(eindex);
      EGlite_free(faces);
      return EGADS_NOTFOUND;
    }
  ...
  lim[3] = 3;
  if (nedge > 4) {
    stat = EGlite_quadLoop(btess, outLevel, nedge, eindex, senses, 
                       parms, lim);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: %d Edges in Face %d (EGlite_makeQuads)!\n", 
  ...
  xyzs = (double *) EGlite_alloc(3*npts*sizeof(double));
  if (xyzs == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on %d XYZs (EGlite_makeQuads)!\n",
             npts);
    EGlite_free(eindex);
    EGlite_free(faces);
    return EGADS_MALLOC;
  }
  uvs = (double *) EGlite_alloc(2*npts*sizeof(double));
  if (uvs == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on %d Points (EGlite_makeQuads)!\n",
             npts);
    EGlite_free(xyzs);
    EGlite_free(eindex);
    EGlite_free(faces);
  ...
  pin = (int *) EGlite_alloc(3*npts*sizeof(int));
  if (pin == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc on %d Pindex (EGlite_makeQuads)!\n",
             npts);
    EGlite_free(uvs);
    EGlite_free(xyzs);
    EGlite_free(eindex);
  ...
  for (npts = i = 0; i < nedge; i++) {
    ...
    if (sens == 1) {
      for (k = 0; k < btess->tess1d[j].npts-1; k++, npts++) {
        stat = EGlite_getEdgeUV(faces[index-1], btess->tess1d[j].obj, 
  ...
  stat = EGlite_quadFill(faces[index-1], parms, lens, uvs, 
                     &npt, &quv, &npat, pats, &vpats);
  EGlite_free(uvs);
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: quadFill = %d (EGlite_makeQuads)!\n",
             stat);
    EGlite_free(pin);
    EGlite_free(xyzs);
    EGlite_free(faces);
  ...
  xyz    = (double *) EGlite_alloc(3*npt*sizeof(double));
  ptype  = (int *)    EGlite_alloc(  npt*sizeof(int));
  pindex = (int *)    EGlite_alloc(  npt*sizeof(int));
  if ((xyz == NULL) || (ptype == NULL) || (pindex == NULL)) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc npts = %d (EGlite_makeQuads)!\n",
             npt);
    if (pindex != NULL) EGlite_free(pindex);
    if (ptype  != NULL) EGlite_free(ptype);
    if (xyz    != NULL) EGlite_free(xyz);
  ...
  for (i = npts; i < npt; i++) {
    pindex[i] = ptype[i] = -1;
    EGlite_evaluate(faces[index-1], &quv[2*i], res);
    xyz[3*i  ] = res[0];
    xyz[3*i+1] = res[1];
    xyz[3*i+2] = res[2];
  }
  EGlite_free(faces);
  patch = (egPatch *) EGlite_alloc(npat*sizeof(egPatch));
  if (patch == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Malloc npatchs = %d (EGlite_makeQuads)!\n",
             npat);
    EGlite_free(pindex);
    EGlite_free(ptype);
    EGlite_free(xyz);
  ...
  ntable = (int *) EGlite_alloc(npt*sizeof(int));
  if (ntable == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Vert Table Malloc (EGlite_makeQuads)!\n");
    EGlite_free(patch);
    EGlite_free(pindex);
    EGlite_free(ptype);
    EGlite_free(xyz);
  ...
  etable = (connect *) EGlite_alloc((nx+1)*sizeof(connect));
  if (etable == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Edge Table Malloc (EGlite_makeQuads)!\n");
    EGlite_free(ntable);
    EGlite_free(patch);
    EGlite_free(pindex);
    EGlite_free(ptype);
  ...
  for (k = m = 0; m < npat; m++) {
    ...
    patch[m].ipts   = (int *) EGlite_alloc(pats[2*m]*pats[2*m+1]*sizeof(int));
    patch[m].bounds = (int *) EGlite_alloc((2*(pats[2*m  ]-1) + 
  ...
  nside = -1;
  for (j = 0; j < npts-1; j++)
    EGlite_makeConnect(j+1, j+2, &pin[3*j+2], 
                   &nside, ntable, etable, index);
  EGlite_makeConnect(npts, 1, &pin[3*npts-1], 
                 &nside, ntable, etable, index);
  for (l = n = m = 0; m < npat; m++) {
    ...
    for (k = i = 0; i < patch[m].nu-1; i++, k++) {
      ...
      EGlite_makeConnect(iv, iv1, &patch[m].bounds[k], 
                     &nside, ntable, etable, index);
    }
  ...
  EGlite_deleteQuads(btess, index);
...
__HOST_AND_DEVICE__ int
EGlite_getQuads(const egObject *tess, int index, int *len, const double **xyz, 
            const double **uv, const int **ptype, const int **pindex, 
            int *npatch)
{
  int      i, outLevel;
  ...
  *ptype = *pindex = NULL;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_getQuads)!\n");  
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_getQuads)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_getQuads)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_getQuads)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess2d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_getQuads)!\n");
    return EGADS_NODATA;  
  }
  if ((index < 1) || (index > btess->nFace)) {
    if (outLevel > 0)
      printf(" EGADS Error: Index = %d [1-%d] (EGlite_getQuads)!\n",
             index, btess->nFace);
    return EGADS_INDEXERR;
  }
...
__HOST_AND_DEVICE__ int
EGlite_getPatch(const egObject *tess, int index, int patch, int *nu, int *nv, 
            const int **ipts, const int **bounds)
{
  ...
  *ipts = *bounds = NULL;
  ...
  if (btess == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Blind Object (EGlite_getPatch)!\n");  
    return EGADS_NOTFOUND;
  }
  obj = btess->src;
  if (obj == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: NULL Source Object (EGlite_getPatch)!\n");
    return EGADS_NULLOBJ;
  }
  if (obj->magicnumber != MAGIC) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not an Object (EGlite_getPatch)!\n");
    return EGADS_NOTOBJ;
  }
  if ((obj->oclass != BODY) && (obj->oclass != EBODY)) {
    if (outLevel > 0)
      printf(" EGADS Error: Source Not Body (EGlite_getPatch)!\n");
    return EGADS_NOTBODY;
  }
  if (btess->tess2d == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Face Tessellations (EGlite_getPatch)!\n");
    return EGADS_NODATA;  
  }
  if ((index < 1) || (index > btess->nFace)) {
    if (outLevel > 0)
      printf(" EGADS Error: Index = %d [1-%d] (EGlite_getPatch)!\n",
             index, btess->nFace);
    return EGADS_INDEXERR;
  }
  i = btess->nFace + index - 1;
  if (btess->tess2d[i].patch == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: No Patch Data (EGlite_getPatch)!\n");
    return EGADS_NODATA;
  }
  if ((patch < 1) || (patch > btess->tess2d[i].npatch)) {
    if (outLevel > 0)
      printf(" EGADS Error: Patch index = %d [1-%d] (EGlite_getPatch)!\n",
