...
__PROTO_H_AND_D__ int  EGlite_sameThread(const egObject *object);
__PROTO_H_AND_D__ int  EGlite_outLevel(const egObject *object);
__PROTO_H_AND_D__ int  EGlite_makeNeighbors(triStruct *ts, int f);
__PROTO_H_AND_D__ void EGlite_makeConnect(int k1, int k2, int *tri, int *kedge,
                                      int *ntable, connect *etable, int face);
__PROTO_H_AND_D__ int  EGlite_quad2tris3(long tID, const egObject *face,
                                     double *parms, int *elens, double *uv,
                                     int *npts, double **uvs,
                                     int *ntris, int **tris, int *flag);
__PROTO_H_AND_D__ int  EGlite_quad2tris (long tID, const egObject *face,
                                     double *parms, int *elens, double *uv,
                                     int *npts, double **uvs,
                                     int *ntris, int **tris, int *tfi);

__PROTO_H_AND_D__ double EGlite_orienTri(double *t0, double *t1, double *t2);
...
EGlite_hashit(KEY key, triStruct *ts)
{
...
__HOST_AND_DEVICE__ static void
EGlite_idestroy(/*@null@*/ ELEMENT *elem)
{
  if (elem != NULL) {
    EGlite_idestroy(elem->next);
    EGlite_free(elem);
  }
}
...
__HOST_AND_DEVICE__ static void
EGlite_hdestroy(triStruct *ts)
{
  int i;

  if (ts->hashTab != NULL) {
    /* free all the chains */
    for (i = 0; i < ts->numElem; i++) EGlite_idestroy(ts->hashTab[i]);

    /* now the table itself */
    EGlite_free(ts->hashTab);
    ts->numElem = -1;
    ts->hashTab = NULL;
  }
}
...
__HOST_AND_DEVICE__ static int
EGlite_hcreate(int how_many, triStruct *ts)
{
  int i, j;
  ...
  if ((ts->hashTab = (ELEMENT **) EGlite_calloc(ts->numElem, 
                                            sizeof(ELEMENT *))) == NULL)
    return 0;
  else
    return 1;
...
EGlite_hmakeKEY(int i0, int i1, int i2)
{
  KEY key;
...
__HOST_AND_DEVICE__ static int
EGlite_hfind(int i0, int i1, int i2, int *close, double *xyz, triStruct *ts)
{
  ELEMENT *ep;
  ENTRY   entry;
  int     hindex;
  ...
  entry.key = EGlite_hmakeKEY(i0, i1, i2);
  hindex    = EGlite_hashit(entry.key, ts);
...
__HOST_AND_DEVICE__ static int
EGlite_hadd(int i0, int i1, int i2, int close, double *xyz, triStruct *ts)
{
  ...
  if (ts->hashTab[hindex] == NULL) {	/* nothing there */
    ...
    e.item = entry;
    e.next = NULL;
    if ((ts->hashTab[hindex] = (ELEMENT *) EGlite_calloc(1, sizeof(ELEMENT))) == NULL)
      return NOTFILLED;
    *ts->hashTab[hindex] = e;
    return 0;
  } else {
    ...
    if ((ep2->next = (ELEMENT *) EGlite_calloc(1, sizeof(ELEMENT))) == NULL)
        return NOTFILLED;
    ep2->next->item = entry;
    ep2->next->next = NULL;
    return 0;
...
EGlite_getIntersect(double *pt0, double *pt1, double *pt2)
{
...
__HOST_AND_DEVICE__ static int
EGlite_recClose2Edge(int t0, double *xyz, int depth, triStruct *ts)
{
  int    i0, i1, i2, tn;
  double dist;
  ...
  if (tn <= 0) {
    if ((dist = EGlite_getIntersect(ts->verts[i1].xyz, ts->verts[i2].xyz, xyz)) < 
        ts->edist2) {
#ifdef DEBUG
    ...
    if (EGlite_recClose2Edge(tn-1, xyz, depth-1, ts) == 1) return 1;
  }

  tn = ts->tris[t0].neighbors[1];
  if (tn <= 0) {
    if ((dist = EGlite_getIntersect(ts->verts[i0].xyz, ts->verts[i2].xyz, xyz)) <
        ts->edist2) {
#ifdef DEBUG
      printf(" dist = %le, edist = %le\n", dist, ts->edist2);
#endif
  ...
  if (tn <= 0) {
    if ((dist = EGlite_getIntersect(ts->verts[i0].xyz, ts->verts[i1].xyz, xyz)) <
        ts->edist2) {
#ifdef DEBUG
      printf(" dist = %le, edist = %le\n", dist, ts->edist2);
#endif
...
__HOST_AND_DEVICE__ static int
EGlite_close2Edge(int t0, double *xyz, triStruct *ts)
{
  /* look down as many as 6 neighbors for an edge */

  return EGlite_recClose2Edge(t0, xyz, 6, ts);
}


__HOST_AND_DEVICE__ static double
EGlite_rayIntersect(double *pt0, double *pt1, double *pt2)
{
  ...
  if (d == 0.0) {
    printf(" EGADS Internal: Edge Segment w/ Zero len (EGlite_rayIntersect)\n");
    return 100.0;
  }
  dx[0] /= d;
  dx[1] /= d;
...
__HOST_AND_DEVICE__ static int
EGlite_recCloseEdge(int t0, double *xyz, int depth, triStruct *ts)
{
  int    i0, i1, i2, tn;
  double rayfac = 0.125;
  ...
  if (tn <= 0) {
    ...
    if (EGlite_rayIntersect(ts->verts[i1].xyz, ts->verts[i2].xyz, xyz) < rayfac)
      return 1;
  } else {
    if (EGlite_recCloseEdge(tn-1, xyz, depth-1, ts) == 1) return 1;
  }
  ...
  if (tn <= 0) {
    ...
    if (EGlite_rayIntersect(ts->verts[i0].xyz, ts->verts[i2].xyz, xyz) < rayfac)
      return 1;
  } else {
    if (EGlite_recCloseEdge(tn-1, xyz, depth-1, ts) == 1) return 1;
  }
  ...
  if (tn <= 0) {
    ...
    if (EGlite_rayIntersect(ts->verts[i0].xyz, ts->verts[i1].xyz, xyz) < rayfac)
      return 1;
  } else {
    if (EGlite_recCloseEdge(tn-1, xyz, depth-1, ts) == 1) return 1;
  }
...
__HOST_AND_DEVICE__ static int
EGlite_closeEdge(int t0, double *xyz, triStruct *ts)
{
  /* look down as many as 4 neighbors for an edge */

  return EGlite_recCloseEdge(t0, xyz, 4, ts);
}


__HOST_AND_DEVICE__ static int
EGlite_addVert(int type, int edge, int index, double *xyz, double *uv,
           triStruct *ts)
{
  int     n;
  triVert *tmp;
  ...
  if (ts->nverts >= ts->mverts) {
    n   = ts->mverts + CHUNK;
    tmp = (triVert *) EGlite_reall(ts->verts, n*sizeof(triVert));
...
__HOST_AND_DEVICE__ static int
EGlite_inTri(int t1, double *xyz, double fuzz, triStruct *ts)
{
  int    i0, i1, i2;
...
__HOST_AND_DEVICE__ static int
EGlite_checkOr(int t1, int side, int t2, triStruct *ts)
{
  int    i0, i1, i2, i3;
  double a1, a2;
  ...
  for (t1 = 0; t1 < ts->ntris; t1++) {
    for (side = 0; side < 3; side++) {
      ...
      if ((i3 < 1) || (i3 > ts->nverts)) {
        printf(" EGlite_checkUVs: Face %d -> %d/%d -- BAD index!\n",
               ts->fIndex, t1, t2);
        continue;
      }
      ...
      if ((a1*a2 <= 0.0) || (a1*ts->orUV < 0.0)) {
        printf(" EGlite_checkUVs: Face %d -> %d/%d inverted UVs %lf %lf  %d\n",
...
__HOST_AND_DEVICE__ static double
EGlite_maxXYZangle(int i1, int i2, int i3, triStruct *ts)
{
...
__HOST_AND_DEVICE__ static double
EGlite_maxUVangle(int i1, int i2, int i3, triStruct *ts)
{
...
__HOST_AND_DEVICE__ static int
EGlite_angXYZTest(int t1, int iedg, int t2, triStruct *ts)
{
  int    i0, i1, i2, i3;
  ...
  angle_now  = EGlite_maxXYZangle(i0-1, i1-1, i2-1, ts);
  angle      = EGlite_maxXYZangle(i1-1, i3-1, i2-1, ts);
  ...
  angle_swap = EGlite_maxXYZangle(i0-1, i1-1, i3-1, ts);
  angle      = EGlite_maxXYZangle(i0-1, i3-1, i2-1, ts);
...
__HOST_AND_DEVICE__ static int
EGlite_angUVTest(int t1, int iedg, int t2, triStruct *ts)
{
  int    i0, i1, i2, i3;
  ...
  angle_now  = EGlite_maxUVangle(i0-1, i1-1, i2-1, ts);
  angle      = EGlite_maxUVangle(i1-1, i3-1, i2-1, ts);
  angle_now  = MAX(angle, angle_now);
  angle_swap = EGlite_maxUVangle(i0-1, i1-1, i3-1, ts);
  angle      = EGlite_maxUVangle(i0-1, i3-1, i2-1, ts);
  angle_swap = MAX(angle, angle_swap);
...
__HOST_AND_DEVICE__ static int
EGlite_areaTest(int t1, int iedg, int t2, triStruct *ts)
{
  int    i0, i1, i2, i3;
  double a1, a2;
...
__HOST_AND_DEVICE__ static int
EGlite_diagTest(int t1, int iedg, int t2, triStruct *ts)
{
  int    i0, i1, i2, i3;
  ...
  if (newd > old + ANGTOL) {
    angle = EGlite_maxUVangle(i0-1, i1-1, i3-1, ts);
    angle = MAX(angle, EGlite_maxUVangle(i0-1, i3-1, i2-1, ts));
    if (angle > MAXANG) {
...
__HOST_AND_DEVICE__ static void
EGlite_fillSides(int t1, double mindist, double emndist, triStruct *ts)
{
  int j, i0, i1, i2, t2;
...
__HOST_AND_DEVICE__ static void
EGlite_fillMid(int t1, int close, triStruct *ts)
{
  int    i0, i1, i2;
  ...
  if (EGlite_evaluate(ts->face, uv, result) != EGADS_SUCCESS) return;

  ts->tris[t1].mid[0] = result[0];
  ts->tris[t1].mid[1] = result[1];
  ts->tris[t1].mid[2] = result[2];
  ts->tris[t1].close  = close;
  if (close != 0) ts->tris[t1].close = EGlite_closeEdge(t1, ts->tris[t1].mid, ts);
}


__HOST_AND_DEVICE__ static void
EGlite_swapTris(int (*test)(int, int, int, triStruct *), /*@unused@*/ char *string, 
      ...
      for (side = 0; side < 3; side++) {
        ...
        if (ts->phase == TOBEFILLED) {
          if (ts->tris[t1].close != TOBEFILLED) 
            EGlite_hadd(ts->tris[t1].indices[0], ts->tris[t1].indices[1],
                    ts->tris[t1].indices[2], ts->tris[t1].close,
                    ts->tris[t1].mid, ts);
          if (ts->tris[t2].close != TOBEFILLED) 
            EGlite_hadd(ts->tris[t2].indices[0], ts->tris[t2].indices[1],
        ...
        if (n12 > 0)
          if (EGlite_checkOr(t1, 1, n12-1, ts) != 0) {
            ts->tris[t1].mark    |= 2;
        ...
        if (n22 > 0)
          if (EGlite_checkOr(t1, 2, n22-1, ts) != 0) {
            ts->tris[t1].mark    |= 4;
        ...
        if (n21 > 0)
          if (EGlite_checkOr(t2, 1, n21-1, ts) != 0) {
            ts->tris[t2].mark    |= 2;
        ...
        if (n11 > 0)
          if (EGlite_checkOr(t2, 2, n11-1, ts) != 0) {
            ts->tris[t2].mark    |= 4;
        ...
        EGlite_fillMid(t1, i, ts);
        EGlite_fillMid(t2, i, ts);
        swap++;
      }
    }
    for (t1 = 0; t1 < ts->ntris; t1++)
...
#ifdef DEBUG
    printf(" EGlite_tessellate -> swap %s: %d\n", string, swap);
#endif
    count++;
...
#ifdef DEBUG
  printf(" EGlite_tessellate -> Accumulated %s: %le\n", string, ts->accum);
#endif
}


__HOST_AND_DEVICE__ static void
EGlite_collapsEdge(int node, int tnode, int flag, triStruct *ts)
{
  ...
  if (flag == 0)
    if (ts->verts[node-1].type != FACE) {
      printf("EGADS Internal (EGlite_collapsEdge): Face %d -- vert is type = %d\n",
  ...
  tin = (int *) EGlite_alloc(nt*sizeof(int));
  if (tin == NULL) return;
  ...
  if (nn != 2) {
    printf(" EGADS Internal (EGlite_collapsEdge): Face %d -- ntris on side = %d\n",
           ts->fIndex, nn);
    EGlite_free(tin);
    return;
  }
  ...
  for (i = 0; i < nt; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (t2 <= 0) continue;
      if (EGlite_checkOr(t1, j, t2-1, ts) != 0) {
        ts->tris[t1].mark |= 1 << j;
    ...
    EGlite_fillMid(t1, NOTFILLED, ts);
  }
...
__HOST_AND_DEVICE__ static void
EGlite_zeroArea(triStruct *ts, int outLevel, long tID)
{
  int    i, stat, side, i0, i1, i2, s0, s1, per, other;
  int    pti0[2], pti1[2], pti2[2];
  double smallu, smallv, range[4], x1[3], x2[3], n[3];

  stat = EGlite_getRange(ts->face, range, &per);
  if (stat != EGADS_SUCCESS) {
    printf("%lX Face %d: EGlite_getRange = %d (zeroArea)!\n",
           tID, ts->fIndex, stat);
    return;
  }
  ...
  for (i = 0; i < ts->ntris; i++) {
    ...
    EGlite_collapsEdge(s0+1, s1+1, 1, ts);
  }
}


__HOST_AND_DEVICE__ static int
EGlite_checkQuadding(int outLevel, int flag, triStruct *ts, long tID)
{
  int    stat, i, i0, i1, i2;
  ...
  for (i = 0; i < ts->ntris; i++) {
    ...
    n[1] /= d;
    n[2] /= d;
    stat = EGlite_evaluate(ts->face, uv, result);
    if (stat != EGADS_SUCCESS) {
      printf("%lX Face %d: Quad tri = %d EGlite_evaluate = %d!\n",
...
__HOST_AND_DEVICE__ static int
EGlite_splitTri(int t0, double *uv, double *point, triStruct *ts)
{
  ...
  if (ts->ntris+1 >= ts->mtris) {
    n   = ts->mtris + CHUNK;
    tmp = (triTri *) EGlite_reall(ts->tris, n*sizeof(triTri));
    if (tmp == NULL) return EGADS_MALLOC;
    ts->tris  = tmp;
    ts->mtris = n;
#ifdef DEBUG
  ...
  node = EGlite_addVert(FACE, 0, 0, point, uv, ts);
  ...
  EGlite_fillMid(t0, NOTFILLED, ts);
  EGlite_fillMid(t1, NOTFILLED, ts);
  EGlite_fillMid(t2, NOTFILLED, ts);
  ...
  for (i = 0; i < 3; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (n <= 0) continue;
      if (EGlite_checkOr(t1, j, n-1, ts) != 0) {
        ts->tris[t1].mark |= 1 << j;
...
__HOST_AND_DEVICE__ static int
EGlite_splitSide(int t1, int side, int t2, int sideMid, triStruct *ts)
{
  ...
  if (((ts->verts[i1-1].type == NODE) && (ts->verts[i1-1].edge == -1)) ||
    ...
    status = EGlite_invEvaluate(ts->face, xyz, uv, point);
...
/*    printf(" Reject side Face %d\n", ts->fIndex);  */
      ...
      status = EGlite_evaluate(ts->face, uv, point);
    }
  } else {
    uv[0]  = 0.5*(ts->verts[i1-1].uv[0] + ts->verts[i2-1].uv[0]);
    uv[1]  = 0.5*(ts->verts[i1-1].uv[1] + ts->verts[i2-1].uv[1]);
  ...
  i = NOTFILLED;
  if ((ts->tris[t1].close == 0) && (ts->tris[t2].close == 0)) i = 0;
  EGlite_fillMid(t[0], i, ts);
  EGlite_fillMid(t[1], i, ts);
  EGlite_fillMid(t[2], i, ts);
  EGlite_fillMid(t[3], i, ts);

  for (i = 0; i < 4; i++) {
    for (j = 0; j < 3; j++) {
      n = ts->tris[t[i]].neighbors[j];
      if (n <= 0) continue;
      if (EGlite_checkOr(t[i], j, n-1, ts) != 0) {
...
__HOST_AND_DEVICE__ static double
EGlite_dotNorm(double *p0, double *p1, double *p2, double *p3)
{
...
__HOST_AND_DEVICE__ static void
EGlite_floodTriGraph(int t, int depth, triStruct *ts)
{
  int tn;
  ...
  if (tn > 0) EGlite_floodTriGraph(tn-1, depth-1, ts);
...
__HOST_AND_DEVICE__ static int
EGlite_breakTri(int mode, int stri, int *eg_split, triStruct *ts)
{
  ...
  for (split = i = 0; i < ts->ntris; i++) {
    ...
    i1 = ts->tris[i].indices[1]-1;
    i2 = ts->tris[i].indices[2]-1;
    if (EGlite_maxUVangle(i0, i1, i2, ts) > CUTANG) continue;
    x1[0] = ts->verts[i1].xyz[0] - ts->verts[i0].xyz[0];
    x2[0] = ts->verts[i2].xyz[0] - ts->verts[i0].xyz[0];
    x1[1] = ts->verts[i1].xyz[1] - ts->verts[i0].xyz[1];
    x2[1] = ts->verts[i2].xyz[1] - ts->verts[i0].xyz[1];
    ...
    for (j = side = 0; side < 3; side++)
      if (ts->tris[i].neighbors[side] > 0) {
        ...
        if (mode == -1) {
          ...
          dot = MIN(dot,EGlite_dotNorm(ts->verts[i0-1].xyz, ts->verts[i1-1].xyz,
  ...
  do {
    ...
    if (EGlite_evaluate(ts->face, uv, xyz) != EGADS_SUCCESS) continue;
    if (mode == 0) {
      if (EGlite_hfind(i0, i1, i2, &j, xyz, ts) == NOTFILLED) {
        if (((ts->verts[i0].type == NODE) && (ts->verts[i0].edge == -1)) ||
          ...
          if (EGlite_inTri(t1, xyz, 0.1, ts) == 0) {
            ...
            if (EGlite_invEvaluate(ts->face, x1, uv, xyz) != EGADS_SUCCESS) continue;
        ...
        EGlite_hadd(i0, i1, i2, 0, xyz, ts);
      }

      if (EGlite_inTri(t1, xyz, 0.0001, ts) == 0) continue;
      if (EGlite_dotNorm(ts->verts[i0].xyz, ts->verts[i1].xyz, xyz,
                                        ts->verts[i2].xyz) < -0.98) continue;
      if (EGlite_dotNorm(ts->verts[i1].xyz, ts->verts[i2].xyz, xyz,
                                        ts->verts[i0].xyz) < -0.98) continue;
      if (EGlite_dotNorm(ts->verts[i2].xyz, ts->verts[i0].xyz, xyz,
                                        ts->verts[i1].xyz) < -0.98) continue;
    } else {
      if (EGlite_inTri(t1, xyz, 0.0001, ts) == 1) continue;
      x1[0] = (ts->verts[i0].xyz[0] + ts->verts[i1].xyz[0] +
               ts->verts[i2].xyz[0]) / 3.0;
      x1[1] = (ts->verts[i0].xyz[1] + ts->verts[i1].xyz[1] +
               ts->verts[i2].xyz[1]) / 3.0;
    ...
    if (EGlite_closeEdge(t1, xyz, ts) == 1) continue;

    if (EGlite_splitTri(t1, uv, xyz, ts) != EGADS_SUCCESS) continue;
    ...
    *eg_split += 1;
    ...
    EGlite_floodTriGraph(t1, FLOODEPTH, ts);
...
__HOST_AND_DEVICE__ static int
EGlite_addFacetNorm(triStruct *ts)
{
...
/*  if (((ts->verts[i0].type == NODE) && (ts->verts[i0].edge == -1)) ||
    ...
    for (side = 0; side < 3; side++) {
      ...
      d = EGlite_dotNorm(ts->verts[i0-1].xyz, ts->verts[i1-1].xyz,
                     ts->verts[i2-1].xyz, ts->verts[i3-1].xyz);
      if (d < dot) 
        if (EGlite_dotNorm(mid,                 ts->verts[i1-1].xyz,
                       ts->verts[i2-1].xyz, ts->verts[i3-1].xyz) > d) dot = d;
    }
    /* is the minimum dot bigger than the threshold? */
    if (dot+ANGTOL > ts->dotnrm) continue;

    if (EGlite_splitTri(t1, uv, mid, ts) == EGADS_SUCCESS) split++;
    if (ts->maxPts > 0)
      if (ts->nverts > ts->maxPts) break;
  }

#ifdef DEBUG
  printf(" EGlite_tessellate -> split: %d\n", split);
#endif
  return split;
}
...
__HOST_AND_DEVICE__ static int
EGlite_addFacetDist(triStruct *ts)
{
  ...
  for (split = t1 = 0; t1 < total; t1++) {
    ...
    if (EGlite_maxUVangle(i0, i1, i2, ts) >  DEVANG) continue;
    xyz[0] = ts->tris[t1].mid[0];
    xyz[1] = ts->tris[t1].mid[1];
    xyz[2] = ts->tris[t1].mid[2];
    if (EGlite_inTri(t1, xyz, 0.10, ts) == 1) continue;
    if (EGlite_dotNorm(ts->verts[i0].xyz, ts->verts[i1].xyz, xyz,
    ...
    if (EGlite_splitTri(t1, uv, xyz, ts) == EGADS_SUCCESS) split++;
    if (ts->maxPts > 0)
      if (ts->nverts > ts->maxPts) break;
  }

#ifdef DEBUG
  printf(" EGlite_tessellate -> tri split: %d\n", split);
#endif
  return split;
}
...
__HOST_AND_DEVICE__ static int
EGlite_splitInter(int sideMid, /*@null@*/ double *aux, int cnt, triStruct *ts)
{
  ...
  for (split = t1 = 0; t1 < total; t1++) {
    ...
    if (aux == NULL) {
      if (EGlite_dotNorm(ts->verts[i0-1].xyz, point,
                     ts->verts[i2-1].xyz, ts->verts[i3-1].xyz) <= 0.1) continue;
      if (EGlite_dotNorm(ts->verts[i0-1].xyz, ts->verts[i1-1].xyz,
                     point,               ts->verts[i3-1].xyz) <= 0.1) continue;
    }

    if (EGlite_splitSide(t1, side, t2, sideMid, ts) == EGADS_SUCCESS) {
      EGlite_floodTriGraph(t1, FLOODEPTH, ts);
      EGlite_floodTriGraph(t2, FLOODEPTH, ts);
      if (aux != NULL) {
        i1 = ts->nverts - 1;
        aux[3*i1  ] = aux[3*i1+1] = aux[3*i1+2] = 0.0;
        status = EGlite_evaluate(ts->face, ts->verts[i1].uv, point);
...
#ifdef REMOVEB
__HOST_AND_DEVICE__ static int
EGlite_removePhaseB(triStruct *ts)
{
  ...
  for (t1 = 0; t1 < ts->ntris; t1++) {
    ...
    if (EGlite_closeEdge(t1, ts->verts[vert-1].xyz, ts) != 1) continue;
...
#endif
    EGlite_collapsEdge(tnode, vert, 0, ts);
    count++;
  }
...
__HOST_AND_DEVICE__ static int
EGlite_addSideDist(int iter, double maxlen2, int sideMid, triStruct *ts)
{
  ...
  for (split = t1 = 0; t1 < ts->ntris; t1++) {
    ts->tris[t1].hit = 0;
    EGlite_fillSides(t1, mindist, emndist, ts);
  }
 ...
 */
    ...
    if (ts->phase == 3) {
      ...
      if (EGlite_close2Edge(t1, xyz, ts) == 1) continue;
    }
    t2 = ts->tris[t1].neighbors[side]-1;
    if (EGlite_splitSide(t1, side, t2, sideMid, ts) == EGADS_SUCCESS) {
      split++;
      ...
      EGlite_floodTriGraph(t2, FLOODEPTH, ts);
      EGlite_fillSides(t1,          mindist, emndist, ts);
      EGlite_fillSides(t2,          mindist, emndist, ts);
      EGlite_fillSides(ts->ntris-2, mindist, emndist, ts);
      EGlite_fillSides(ts->ntris-1, mindist, emndist, ts);
    } else {
      ts->tris[t1].hit = 1;
    }
    if (ts->maxPts > 0)
...
EGlite_tessellate(int outLevel, triStruct *ts, long tID)
{
  ...
  for (i = 0; i < ts->nverts; i++) {
    if (EGlite_evaluate(ts->face, ts->verts[i].uv, result) != EGADS_SUCCESS)
      continue;
  ...
  if (ts->uvs != NULL) {
    ...
    if ((ts->ntris   == 2) && (ts->nverts  == 4) && (ts->lens[0] == 1) &&
      ...
      i   = EGADS_MALLOC;
      trs = (int *) EGlite_alloc(6*sizeof(int));
      if (trs != NULL) {
        trs[0]  = 0;
        trs[1]  = 1;
        trs[2]  = 2;
      ...
      /* cone-like quadding */
      i = EGlite_quad2tris3(tID, ts->face, ts->qparm, ts->lens, &ts->uvs[2], &nvrt,
                        &quv, &ntrs, &trs, &flag);
    } else {
      /* normal quadding */
      i = EGlite_quad2tris (tID, ts->face, ts->qparm, ts->lens, &ts->uvs[2], &nvrt,
                        &quv, &ntrs, &trs, &ts->tfi);
    }
    EGlite_free(ts->uvs);
    if ((i == EGADS_SUCCESS) && (trs != NULL)) {
      ...
      if (quv != NULL) {
        for (j = ts->nverts; j < nvrt; j++) {
          n0 = EGlite_evaluate(ts->face, &quv[2*j], result);
          if (n0 < EGADS_SUCCESS) {
            EGlite_free(quv);
            EGlite_free(trs);
            return n0;
          }
          n0 = EGlite_addVert(FACE, 0, 0, result, &quv[2*j], ts);
          if (n0 < EGADS_SUCCESS) {
            EGlite_free(quv);
            EGlite_free(trs);
            return n0;
      ...
      if (ntrs > ts->mtris) {
        tt = (triTri *) EGlite_reall(ts->tris, (ntrs+1)*sizeof(triTri));
        if (tt == NULL) {
          EGlite_free(trs);
          return EGADS_MALLOC;
        }
      ...
      if (EGlite_checkQuadding(outLevel, flag, ts, tID) == EGADS_SUCCESS) {
        ...
        return EGlite_makeNeighbors(ts, ts->fIndex);
      }
      ts->tfi    = 0;
  ...
  for (last = i = 0; i < ts->ntris; i++) {
    ...
    if (dist <= 0.0) {
      ...
      for (j = 0; j < 3; j++) {
        ...
        if (k <= i) continue;
        if (EGlite_checkOr(i, j, k, ts) == 0) continue;
        ts->tris[i].mark |= 1 << j;
  ...
  if (ts->planar == 0) {
    ...
    EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
#ifdef REPORT
    lang = ts->accum;
#endif
    EGlite_swapTris(EGlite_diagTest, "diagonals", 1.0, ts);
#ifdef REPORT
    ...
    deru  = &result[3];
    derv  = &result[6];
    aux   = (double *) EGlite_alloc(3*flag*sizeof(double));
    if (aux == NULL) {
      split = 0;
    } else {
      for (i = 0; i < ts->nverts; i++) {
        aux[3*i  ] = aux[3*i+1] = aux[3*i+2] = 0.0;
        status = EGlite_evaluate(ts->face, ts->verts[i].uv, result);
        if (status != EGADS_SUCCESS) {
          if (status != EGADS_EXTRAPOL)
            printf(" EGADS Internal: Face %d EGlite_evaluate %lf %lf = %d\n",
    ...
    while ((split != 0) && (ts->orCnt < MAXORCNT)) {
      split = EGlite_splitInter(sideMid, aux, flag, ts);
      if (split != 0) {
        EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
#ifdef REPORT
        lang = ts->accum;
    ...
    if (ts->maxlen > 0.0) {
      count = i = 0;
      do {
        split = EGlite_addSideDist(i, 4.0*maxlen2, sideMid, ts);
        if (split > 0) {
          EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
          lang = ts->accum;
          EGlite_swapTris(EGlite_diagTest, "diagonals", 1.0, ts);
    ...
    count = 0;
    do {
      split = EGlite_breakTri(-1, stri, &eg_split, ts);
      if (split > 0) {
        EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
#ifdef REPORT
        lang = ts->accum;
    ...
    split  = 1;
    while ((split != 0) && (ts->orCnt < MAXORCNT)) {
      split = EGlite_splitInter(sideMid, NULL, 0, ts);
      if (split != 0) {
        EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
#ifdef REPORT
        lang = ts->accum;
    ...
    if (count != 0) count = EGlite_removePhaseB(ts);
    if (count >  0) {
      EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
#ifdef REPORT
      lang = ts->accum;
    ...
    EGlite_hcreate(CHUNK, ts);
    count = 0;
    do {
      split = EGlite_breakTri(0, stri, &eg_split, ts);
      if (split > 0) {
        EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
#ifdef REPORT
        lang = ts->accum;
    ...
    if ((ts->maxlen > 0.0) && (badStart == 0)) {
      count = i = 0;
      do {
        split = EGlite_addSideDist(i, maxlen2, sideMid, ts);
        if (split > 0) {
          EGlite_swapTris(EGlite_angUVTest, "angleUV",  0.0, ts);
          lang = ts->accum;
          EGlite_swapTris(EGlite_diagTest, "diagonals", 1.0, ts);
    ...
    if (ts->accum < ts->dotnrm) {
      ts->phase = 1;
      count = lsplit = 0;
      for (i = 0; i < ts->ntris; i++) EGlite_fillMid(i, NOTFILLED, ts);
      do {
        split  = EGlite_addFacetNorm(ts);
        laccum = ts->accum;
        if (split != 0) {
          ts->phase = TOBEFILLED;
          EGlite_hcreate(CHUNK, ts);
          ...
          for (i = 0; i < ts->ntris; i++)
            if (ts->tris[i].close == TOBEFILLED)
              if (EGlite_hfind(ts->tris[i].indices[0], ts->tris[i].indices[1],
                           ts->tris[i].indices[2], &j, ts->tris[i].mid, ts)
                  != NOTFILLED){
                ts->tris[i].close = j;
              } else {
                EGlite_fillMid(i, NOTFILLED, ts);
              }
          EGlite_hdestroy(ts);
    ...
    if (outLevel > 1) {
      ...
      for (stat[0] = stat[1] = i = 0; i < ts->ntris; i++)
        for (j = 0; j < 3; j++) {
          ...
          dist = EGlite_dotNorm(ts->verts[n0-1].xyz, ts->verts[n1-1].xyz,
    ...
    ts->phase = -3;
    EGlite_swapTris(EGlite_angXYZTest, "angleXYZ", 0.0, ts);
    lang = ts->accum;
...
__HOST_AND_DEVICE__ int
EGlite_inTriExact(double *t1, double *t2, double *t3, double *p, double *w)
{
  int    d1, d2, d3;
  double sum;
  
  w[0] = EGlite_orienTri(t2, t3, p);
  w[1] = EGlite_orienTri(t1, p,  t3);
  w[2] = EGlite_orienTri(t1, t2, p);
  d1   = EGlite_sign(w[0]);
  d2   = EGlite_sign(w[1]);
  d3   = EGlite_sign(w[2]);
  sum  = w[0] + w[1] + w[2];
...
int
EGlite_baryFrame(egTess2D *tess2d)
{
  int    i, j, i0, i1, i2, cls;
  double neg, w[3];

  tess2d->bary = (egBary *) EGlite_alloc(tess2d->npts*sizeof(egBary));
  ...
  for (i = 0; i < tess2d->npts; i++) {
    ...
    for (cls = j = 0; j < tess2d->nframe; j++) {
      ...
      if (EGlite_inTriExact(&tess2d->uv[2*i0], &tess2d->uv[2*i1], &tess2d->uv[2*i2],
    ...
    if (tess2d->bary[i].tri == 0) {
      ...
      EGlite_inTriExact(&tess2d->uv[2*i0], &tess2d->uv[2*i1], &tess2d->uv[2*i2],
                    &tess2d->uv[2*i],  w);
      tess2d->bary[i].tri  = cls;
      tess2d->bary[i].w[0] = w[0];
      tess2d->bary[i].w[1] = w[1];
      printf(" EGADS Warning: Extrapolation for %lf %lf  %d (EGlite_baryFrame)!\n",
             tess2d->uv[2*i], tess2d->uv[2*i+1], i+1);
...
__HOST_AND_DEVICE__ int
EGlite_baryTess(egTess2D tess2d, const double *uv, double *w)
{
  int    j, i0, i1, i2, cls;
  ...
  for (j = 0; j < tess2d.ntris; j++) {
    ...
    if (EGlite_inTriExact(&tuv[2*i0], &tuv[2*i1], &tuv[2*i2], uvs, w) ==
  ...
  if (cls != 0) {
    ...
    EGlite_inTriExact(&tuv[2*i0], &tuv[2*i1], &tuv[2*i2], uvs, w);
  }
  return cls;
}
...
#ifndef LITE
int
EGlite_fitTriangles(egObject *context, int npts, double *xyzs, int ntris,
                ...
                egObject **bspline)
{
  ...
  *bspline = NULL;
  ...
  outLevel = EGlite_outLevel(context);
  pxyz     = (prmXYZ *) xyzs;
  
  ptris = (prmTri *) EGlite_alloc(ntris*sizeof(prmTri));
  ...
  for (i = 0; i < ntris; i++) {
    ...
    if ((tris[3*i  ] < 1) || (tris[3*i  ] > npts) ||
      ...
      if (outLevel > 0) {
        printf(" EGADS Warning: %d bad tris [1-%d] (EGlite_fitTriangles)!\n",
               i+1, npts);
        printf("                tris = %d %d %d\n",
    ...
    if ((tric[3*i  ] > ntris) || (tric[3*i+1] > ntris) ||
        (tric[3*i+2] > ntris)) {
      if (outLevel > 0) {
        printf(" EGADS Warning: %d bad tric [1-%d] (EGlite_fitTriangles)!\n",
               i+1, ntris);
        printf("                tric = %d %d %d\n",
  ...
  if (tric == NULL) {
    vtab = (int *) EGlite_alloc(npts*sizeof(int));
    if (vtab == NULL) {
      EGlite_free(ptris);
      return EGADS_MALLOC;
    }
    etab = (connect *) EGlite_alloc(ntris*3*sizeof(connect));
    if (etab == NULL) {
      EGlite_free(vtab);
      EGlite_free(ptris);
      return EGADS_MALLOC;
    ...
    for (i = 0; i < ntris;  i++) {
      EGlite_makeConnect( ptris[i].indices[1], ptris[i].indices[2],
                     &ptris[i].neigh[0], &n, vtab, etab, 0);
      EGlite_makeConnect( ptris[i].indices[0], ptris[i].indices[2],
                     &ptris[i].neigh[1], &n, vtab, etab, 0);
      EGlite_makeConnect( ptris[i].indices[0], ptris[i].indices[1],
...
/*    printf(" EGADS Info: Unconnected Side %d %d = %d\n",
  ...
  uv = (prmUV *) EGlite_alloc(npts*sizeof(prmUV));
  if (uv == NULL) {
    EGlite_free(ptris);
    return EGADS_MALLOC;
  }
  ...
  type = prm_CreateUV(0, ntris, ptris, NULL, npts, NULL, NULL, uv, pxyz,
                      &per, &ppnts);
  if (outLevel > 1) {
    printf(" EGlite_fitTriangles: prm_CreateUV = %d  per = %d\n", type, per);
...
/*    for (i = 0; i < npts; i++)
  ...
  if (type > 0) {
    ...
    if (outLevel > 1)
      printf(" EGlite_fitTriangles: prm_SmoothUV = %d\n", stat);
    if (stat == EGADS_MALLOC) stat = EGADS_SUCCESS;
    while ((stat != EGADS_SUCCESS) && (type < 7)) {
      if (ppnts != NULL) EGlite_free(ppnts);
      type++;
      if (type < 6) type = 6;
      n    = 1;
    ...
    if (stat == EGADS_SUCCESS) {
      ...
      if (outLevel > 1)
        printf(" EGlite_fitTriangles: prm_NormalizeUV = %d\n", stat);
      if (stat == EGADS_SUCCESS) {
        n    = 4;
        nu   = 2*npts;
        nv   = 0;
        ...
        if (stat == PRM_TOLERANCEUNMET) {
          printf(" EGlite_fitTriangles: Tolerance not met: %lf (%lf)!\n",
                 maxerr, tol);
          stat = EGADS_SUCCESS;
        }
        if (outLevel > 1)
          printf(" EGlite_fitTriangles: prm_BestGrid = %d  %d %d  %lf %lf (%lf)\n",
  ...
  sizes[1] = nv;
#endif
  stat = EGlite_approximate(context, 0, tol, sizes, grid, bspline);
  EGlite_free(grid);
