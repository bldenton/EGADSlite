...
int EMP_LITE_Init(long *start)
{
  ...
  if (start != NULL) *start = EMP_LITE_getseconds();
  return np;
#else
  return 1;
#endif
...
__HOST_AND_DEVICE__
long EMP_LITE_Done(long *start)
{
#ifndef __CUDA_ARCH__
  if (start == NULL) return 0L;
  return EMP_LITE_getseconds() - *start;
#else
  return 0L;
#endif
}
...
__HOST_AND_DEVICE__
HANDLE *EMP_LITE_ThreadCreate(void (*entry)(void *), void *arg)
{
#ifndef __CUDA_ARCH__
  HANDLE   *thread;
  unsigned  threadID;
...
__HOST_AND_DEVICE__
void EMP_LITE_ThreadWait(HANDLE *thread)
{
#ifndef __CUDA_ARCH__
...
__HOST_AND_DEVICE__
void EMP_LITE_ThreadDestroy(HANDLE *thread)
{
#ifndef __CUDA_ARCH__
  CloseHandle(*thread);
  free(thread);
...
__HOST_AND_DEVICE__
void EMP_LITE_LockDestroy(HANDLE *mutex)
{
#ifndef __CUDA_ARCH__
  ReleaseMutex(*mutex);
  free(mutex);
...
__HOST_AND_DEVICE__
void EMP_LITE_LockSet(HANDLE *mutex)
{
#ifndef __CUDA_ARCH__
...
__HOST_AND_DEVICE__
int EMP_LITE_LockTest(HANDLE *mutex)
{
#ifndef __CUDA_ARCH__
  DWORD stat;
...
__HOST_AND_DEVICE__
void EMP_LITE_LockRelease(HANDLE *mutex)
{
#ifndef __CUDA_ARCH__
...
__HOST_AND_DEVICE__
int EMP_LITE_Init(/*@null@*/ long *start)
{
#ifndef __CUDA_ARCH__
  char *env;
  long nprocs = 1;
...
__HOST_AND_DEVICE__
/*@null@*/ void *EMP_LITE_ThreadCreate(void (*entry)(void *), /*@null@*/ void *arg)
{
#ifndef __CUDA_ARCH__
  int            stat;
  pthread_t      *thread;
...
__HOST_AND_DEVICE__
void EMP_LITE_ThreadWait(void *vthread)
{
#ifndef __CUDA_ARCH__
  int       stat;
  pthread_t *thread;
...
__HOST_AND_DEVICE__
void EMP_LITE_ThreadDestroy(/*@only@*/ void *vthread)
{
#ifndef __CUDA_ARCH__
...
__HOST_AND_DEVICE__
void EMP_LITE_LockDestroy(/*@only@*/ void *vlock)
{
#ifndef __CUDA_ARCH__
...
__HOST_AND_DEVICE__
void EMP_LITE_LockSet(void *vlock)
{
#ifndef __CUDA_ARCH__
  int             stat;
  pthread_mutex_t *lock;
...
__HOST_AND_DEVICE__
int EMP_LITE_LockTest(void *vlock)
{
#ifndef __CUDA_ARCH__
  int             stat;
  pthread_mutex_t *lock;
...
__HOST_AND_DEVICE__
void EMP_LITE_LockRelease(void *vlock)
{
#ifndef __CUDA_ARCH__
  pthread_mutex_t *lock;
...
/* structure to hold control info for EMP_LITE_for, EMP_LITE_sum, and EMP_LITE_min */
...
typedef struct {
  ...
  int    (*forFn)(int);             /* routine that does work in EMP_LITE_for */
  int    (*sumFn)(int, double *);   /* routine that does work in EMP_LITE_sum */
  double sum;                       /* global sum */
  int    (*minFn)(int, double *);   /* routine that does work for EMP_LITE_min */
  double min;                       /* global minimum */
  int    imin;                      /* index associated with minimum (or -1) */
} emp_T;


/* Inner routine used by EMP_LITE_for */

static void EMP_LITE_for_inner(void *Global)
{
  int   index, status;
  long  ID;
  emp_T *global;
  ...
  while (1) {
    ...
    if (global->mutex != NULL ) EMP_LITE_LockSet(global->mutex);

    status        = global->status;
    index         = global->index;
    global->index = index + 1;

    if (global->mutex != NULL) EMP_LITE_LockRelease(global->mutex);
  ...
  if (ID != global->masterID) EMP_LITE_ThreadExit();
}


/* Simple parallel "for" loop */

int EMP_LITE_for(int maxproc, int nindex, int (*forFn)(int index))
{
  int   i, np;
  long  start;
  void  **threads = NULL;
  ...
  global.masterID = EMP_LITE_ThreadID();
  global.mutex    = NULL;
   ...
   */
  if (np > 1) {
    global.mutex = EMP_LITE_LockCreate();
    if (global.mutex == NULL) {
      np = 1;
    } else {
      threads = (void **) malloc((np-1)*sizeof(void *));
      if (threads == NULL) {
        EMP_LITE_LockDestroy(global.mutex);
        np = 1;
      }
    }
  }
  ...
  if (global.mutex != NULL) EMP_LITE_LockDestroy(global.mutex);
...
/* Inner routine used by EMP_LITE_sum */

static void EMP_LITE_sum_inner(void *Global)
{
  int    index, status;
  long   ID;
  double sum;
...
int EMP_LITE_sum(int maxproc, int nindex, int (*sumFn)(int index, double *sum),
            double *sum)
{
  int   i, np;
  long  start;
  ...
  if (threads != NULL)
    for (i = 0; i < np-1; i++) {
      threads[i] = EMP_LITE_ThreadCreate(EMP_LITE_sum_inner, &global);
      if (threads[i] == NULL) printf(" Error creating thread %d\n", i+1);
    }

  /* now run the inner routine from the original thread */
  EMP_LITE_sum_inner(&global);
...
/* Inner routine used by EMP_LITE_min */

static void EMP_LITE_min_inner(void *Global)
{
  int    index, status;
  long   ID;
  double min;
...
int EMP_LITE_min(int maxproc, int nindex, int (*minFn)(int index, double *min),
            double *min, int *imin)
{
  int   i, np;
  long  start;
  ...
  if (threads != NULL)
    for (i = 0; i < np-1; i++) {
      threads[i] = EMP_LITE_ThreadCreate(EMP_LITE_min_inner, &global);
      if (threads[i] == NULL) printf(" Error creating thread %d\n", i+1);
    }

  /* now run the inner routine from the original thread */
  EMP_LITE_min_inner(&global);
