...
#define DOT(a,b)         (a[0]*b[0] + a[1]*b[1] + a[2]*b[2])
...
#ifdef LITE
  extern void EGlite_EvaluateQuotientRule(  int dim, int der_count, int v_stride,
                                        double *v );
  extern void EGlite_EvaluateQuotientRule2( int dim, int der_count, int v_stride,
                                        double *v );
  extern bool EGlite_Bezier1DRat( int dim, int order, int cv_stride,
  ...
  extern int  EGlite_attributeRet( const egObject *obj, const char *name, int *atype,
                               ...
                               /*@null@*/ const char **str );
  extern int  EGlite_getRange( const egObject *geom, double *range, int *periodic );
  extern int  EGlite_evaluate( const egObject *geom, const double *param,
                           double *result );
  extern int  EGlite_invEvaluate( const egObject *geom, double *xyz, double *param,
                              double *result );
  extern int  EGlite_getTopology( const egObject *topo, egObject **geom,
                              int *oclass, int *mtype,
                              /*@null@*/ double *limits, int *nChildren,
                              egObject ***children, int **senses );
  extern int  EGlite_getEdgeUV( const egObject *face, const egObject *edge,
                            int sense, double t, double *result );
  extern int  EGlite_getBodyTopos( const egObject *body, /*@null@*/ egObject *src,
                               int oclass, int *ntopo,
                               /*@null@*/ egObject ***topos );
  extern int  EGlite_getBody( const egObject *object, egObject **body );

  /* forward prototype for EGlite_eval3deriv */
  extern int  EGlite_evaluateGeom( const egObject *geom, const double *param,
                               double *result );

#else
  TEMPLATE   void EGlite_EvaluateQuotientRule(  int dim, int der_count,
                                            int v_stride, DOUBLE *v );
  TEMPLATE   void EGlite_EvaluateQuotientRule2( int dim, int der_count,
                                            int v_stride, DOUBLE *v );
  TEMPLATE   bool EGlite_Bezier1DRat( int dim, int order, int cv_stride,
                                  const DOUBLE *cv, double t0, double t1,
                                  int der_count, DOUBLE t, int v_stride,
                                  DOUBLE *v );

  extern "C" int  EGlite_attributeRet( const egObject *obj, const char *name,
                                   int *atype, int *len,
                                   /*@null@*/ const int **ints,
                                   /*@null@*/ const double **reals,
                                   /*@null@*/ const char **str );
  extern "C" int  EGlite_getRange( const egObject *geom, double *range,
                               int *periodic );
  extern "C" int  EGlite_evaluate( const egObject *geom, const double *param,
                               double *result );
  extern "C" int  EGlite_invEvaluate( const egObject *geom, double *xyz,
                                  double *param, double *result );
  extern "C" int  EGlite_getTopology( const egObject *topo, egObject **geom,
                                  int *oclass, int *mtype,
                                  /*@null@*/ double *limits, int *nChildren,
                                  egObject ***children, int **senses );
  extern "C" int  EGlite_getEdgeUV( const egObject *face, const egObject *edge,
                                int sense, double t, double *result );
  extern "C" int  EGlite_getBodyTopos( const egObject *body, /*@null@*/ egObject *src,
                                   int oclass, int *ntopo,
                                   /*@null@*/ egObject ***topos );
  extern "C" int  EGlite_getBody( const egObject *object, egObject **body );

  extern "C" int  EGlite_getWindingAngle( egObject *edge, double t, double *angle );
  extern "C" int  EGlite_inFaceX( const egObject *face, const double *uva,
                              /*@null@*/ double *pt, /*@null@*/ double *uvx );

  /* forward prototype for EGlite_eval3deriv */
  TEMPLATE   int EGlite_evaluateGeom( const egObject *geom, const DOUBLE *param,
                                  DOUBLE *result );
#endif
...
TEMPLATE static int
EGlite_splinePCDeriv(int *ivec, DOUBLE *data, DOUBLE t, DOUBLE *deriv)
{
  int    der = 2;
  ...
  if ((ivec[0] != 0) && (ivec[0] != 2)) {
    printf(" EGlite_splinePCDeriv: flag = %d!\n", ivec[0]);
    return EGADS_GEOMERR;
  }
  if (degree >= MAXDEG) {
    printf(" EGlite_splinePCDeriv: degree %d >= %d!\n", degree, MAXDEG);
  ...
  if (ivec[0] == 0) {
    ...
    EGlite_EvaluateQuotientRule(2, der, 3, v);
    for (k = 0; k <= der; k++) {
      deriv[2*k  ] = v[3*k  ];
      deriv[2*k+1] = v[3*k+1];
    }
...
TEMPLATE static int
EGlite_spline1dDeriv(int *ivec, DOUBLE *data, DOUBLE t, DOUBLE *deriv)
{
  int    der = 2;
  int    i, j, k, degree, nKnots, span, dt;
  DOUBLE Nders[MAXDEG+1][MAXDEG+1], *Nder[MAXDEG+1], *CP, *w;
  ...
  dt     = MIN(der, degree);
  if ((ivec[0] != 0) && (ivec[0] != 2)) {
    printf(" EGlite_spline1dDeriv: flag = %d!\n", ivec[0]);
    return EGADS_GEOMERR;
  }
  if (degree >= MAXDEG) {
    printf(" EGlite_spline1dDeriv: degree %d >= %d!\n", degree, MAXDEG);
    return EGADS_CONSTERR;
  }
  for (i = 0; i <= degree; i++) Nder[i] = &Nders[i][0];
  ...
  if (ivec[0] == 0) {
    ...
    EGlite_EvaluateQuotientRule(3, der, 4, v);
    for (k = 0; k <= der; k++) {
      deriv[3*k  ] = v[4*k  ];
      deriv[3*k+1] = v[4*k+1];
      deriv[3*k+2] = v[4*k+2];
...
TEMPLATE static int
EGlite_spline2dDeriv(int *ivec, DOUBLE *data, const DOUBLE *uv, DOUBLE *deriv)
{
  int    der = 2;
  ...
  if ((ivec[0] != 0) && (ivec[0] != 2)) {
    printf(" EGlite_spline2dDeriv: flag = %d!\n", ivec[0]);
    return EGADS_GEOMERR;
  }
  if (degu >= MAXDEG) {
    printf(" EGlite_spline2dDeriv: degreeU %d >= %d!\n", degu, MAXDEG);
    return EGADS_CONSTERR;
  }
  if (degv >= MAXDEG) {
    printf(" EGlite_spline2dDeriv: degreeV %d >= %d!\n", degv, MAXDEG);
    return EGADS_CONSTERR;
  }
  ...
  if (ivec[0] == 0) {
    ...
    v[14]   = temp[2];
    v[15]   = temp[3];
    EGlite_EvaluateQuotientRule2(3, der, 4, v);
    for (m = l = 0; l <= der; l++)
      for (k = 0; k <= der-l; k++, m++) {
        deriv[3*m  ] = v[4*m  ];
        deriv[3*m+1] = v[4*m+1];
...
TEMPLATE static int
EGlite_bezierPCDeriv(int *ivec, DOUBLE *data, DOUBLE t, DOUBLE *deriv)
{
  bool   stat;
  int    i, k, degree;
  ...
  w      = data + 2*ivec[2];
  if ((ivec[0] != 0) && (ivec[0] != 2)) {
    printf(" EGlite_bezierPCDeriv: flag = %d!\n", ivec[0]);
    return EGADS_GEOMERR;
  }
  if (degree <= 0) {
    printf(" EGlite_bezierPCDeriv: degree %d <= 0!\n", degree);
    return EGADS_CONSTERR;
  }

  if (degree >= MAXDEG) {
    printf(" EGlite_bezierPCDeriv: degree %d >= %d!\n", degree, MAXDEG);
    return EGADS_CONSTERR;
  }
  ...
  if (ivec[0] == 0) {
    ...
    stat = EGlite_Bezier1DRat(2, ivec[2], 3, CP, 0.0, 1.0, 2, t, 2, deriv);
...
TEMPLATE static int
EGlite_bezier1dDeriv(int *ivec, DOUBLE *data, DOUBLE t, DOUBLE *deriv)
{
  bool   stat;
  int    i, k, degree;
  ...
  w      = data + 3*ivec[2];
  if ((ivec[0] != 0) && (ivec[0] != 2)) {
    printf(" EGlite_bezier1dDeriv: flag = %d!\n", ivec[0]);
    return EGADS_GEOMERR;
  }
  if (degree <= 0) {
    printf(" EGlite_bezier1dDeriv: degree %d <= 0!\n", degree);
    return EGADS_CONSTERR;
  }
  if (degree >= MAXDEG) {
    printf(" EGlite_bezier1dDeriv: degree %d >= %d!\n", degree, MAXDEG);
    return EGADS_CONSTERR;
  }
  ...
  if (ivec[0] == 0) {
    ...
    stat = EGlite_Bezier1DRat(3, ivec[2], 4, CP, 0.0, 1.0, 2, t, 3, deriv);
    if (!stat) return EGADS_GEOMERR;
...
TEMPLATE static int
EGlite_bezier2dDeriv(int *ivec, DOUBLE *data, const DOUBLE *uv, DOUBLE *deriv)
{
  ...
  m = ivec[4]-1;
  if ((ivec[0] != 0) && (ivec[0] != 2)) {
    printf(" EGlite_bezier2dDeriv: flag = %d!\n", ivec[0]);
    return EGADS_GEOMERR;
  }
  if ((n >= MAXDEG) || (m >= MAXDEG)) {
    printf(" EGlite_bezier2dDeriv: degree %d or %d >= %d!\n", n, m, MAXDEG);
    return EGADS_CONSTERR;
  }
  ...
  if (ivec[0] == 0) {
    ...
    return EGlite_spline2dDeriv(header, D, uv, deriv);
...
TEMPLATE static void
EGlite_rotatePC(DOUBLE *axes, DOUBLE *data, DOUBLE *result)
{
...
TEMPLATE static void
EGlite_rotate2D(DOUBLE *axes, DOUBLE *data, DOUBLE *result)
{
...
TEMPLATE static void
EGlite_rotate3D(DOUBLE *axes, DOUBLE *data, DOUBLE *result)
{
...
TEMPLATE static int
EGlite_eval3deriv(const egObject *geom, const DOUBLE *param, DOUBLE *result)
{
  int    stat;
  double step = 1.e-7;
  ...
  if (geom->oclass == PCURVE) {
    ...
    uv[0] = *param - step;
    stat  = EGlite_evaluateGeom(geom, uv, uminus);
    if (stat != EGADS_SUCCESS) return stat;
    uv[0] = *param + step;
    stat  = EGlite_evaluateGeom(geom, uv, uplus);
    if (stat != EGADS_SUCCESS) return stat;
    ...
    uv[0] = param[0];
    uv[1] = param[1] - step;
    stat  = EGlite_evaluateGeom(geom, uv, vminus);
    if (stat != EGADS_SUCCESS) return stat;
    uv[1] = param[1] + step;
    stat  = EGlite_evaluateGeom(geom, uv, vplus);
    if (stat != EGADS_SUCCESS) return stat;
...
TEMPLATE int
EGlite_evaluateGeom(const egObject *geom, const DOUBLE *param, DOUBLE *result)
{
  int    i, stat;
  ...
  if (geom->oclass == PCURVE) {
    ...
    switch (geom->mtype) {
      ...
      case CIRCLE:
        ...
        EGlite_rotatePC(gdata, data, result);
        stat    = EGADS_SUCCESS;
        break;
      ...
      case TRIMMED:
        stat = EGlite_evaluateGeom(lgeom->ref, param, result);
        break;
        
      case BEZIER:
        stat = EGlite_bezierPCDeriv(lgeom->header, gdata, param[0], result);
        break;
        
      case BSPLINE:
        stat = EGlite_splinePCDeriv(lgeom->header, gdata, param[0], result);
        break;
        
      case OFFSET:
        stat = EGlite_evaluateGeom(lgeom->ref, param, result);
        if (stat == EGADS_SUCCESS) {
          stat = EGlite_eval3deriv(lgeom->ref, param, der3);
    ...
    switch (geom->mtype) {
      ...
      case CIRCLE:
        ...
        data[8] =  0.0;
        EGlite_rotate2D(gdata, data, result);
        stat    = EGADS_SUCCESS;
        break;
      ...
      case BEZIER:
        stat = EGlite_bezier1dDeriv(lgeom->header, gdata, param[0], result);
        break;
        
      case BSPLINE:
        stat = EGlite_spline1dDeriv(lgeom->header, gdata, param[0], result);
        break;
    ...
    switch (geom->mtype) {
      ...
      case SPHERICAL:
        ...
        EGlite_rotate3D(axes, data, result);
        stat     = EGADS_SUCCESS;
        break;
      ...
      case CONICAL:
        ...
        EGlite_rotate3D(gdata, data, result);
        stat    = EGADS_SUCCESS;
        break;
      ...
      case REVOLUTION:
        stat = EGlite_evaluateGeom(lgeom->ref, &param[1], data);
        if (stat == EGADS_SUCCESS) {
          tan        = &gdata[3];
          tanv       = &data[3];
          CROSS(norm, tan, tanv);
      ...
      case EXTRUSION:
        stat = EGlite_evaluateGeom(lgeom->ref, param, data);
        if (stat == EGADS_SUCCESS) {
      ...
      case BEZIER:
        stat = EGlite_bezier2dDeriv(lgeom->header, gdata, param, result);
        break;
        
      case BSPLINE:
        stat = EGlite_spline2dDeriv(lgeom->header, gdata, param, result);
        break;
...
template int EGlite_evaluateGeom(const egObject *geom, const double *param,
                             double *result);
template int EGlite_evaluateGeom(const egObject *geom, const SurrealS<1> *param,
                             SurrealS<1> *result);
#endif


static int
EGlite_nearestOnPCurve(const egObject *geom, const double *coor, double *range,
                   double *t, double *uv)
{
  int    i, j, stat;
  ...
  for (i = 0; i < 40; i++) {
    ...
    for (j = 0; j < 20; j++) {
      ...
      if (j == 19) dt = 0.0;
      tt = *t + dt;
      stat = EGlite_evaluateGeom(geom, &tt, result);
      if (stat != EGADS_SUCCESS) return stat;
...
static int
EGlite_nearestOnCurve(const egObject *geom, const double *coor, double *range,
                  double *t, double *xyz)
{
  int    i, j, stat;
...
static int
EGlite_nearestOnSurface(const egObject *geom, const double *point, double *uv,
                    double *coor)
{
  ...
  stat = EGlite_getRange(geom, range, &per);
  if (stat != EGADS_SUCCESS) return stat;
  ...
  for (count = 0; count < 40; count++) {
    ...
    for (j = 0; j < 20; j++) {
      ...
      stat = EGlite_evaluateGeom(geom, uvs, result);
      if (stat != EGADS_SUCCESS) return stat;
...
static int
EGlite_nearestOnPCurveLM(const egObject *geom, const double *point, double *range,
                     double *t, double *coor)
{
  ...
  ts     = *t;
  status = EGlite_evaluateGeom(geom, &ts, result);
  ...
  for (istep = 0; istep < 250; istep++) {
    ...
    status = EGlite_evaluateGeom(geom, &tt, data);
    if (status != EGADS_SUCCESS) return status;
...
static int
EGlite_nearestOnCurveLM(const egObject *geom, const double *point, double *range,
                    double *t, double *coor)
{
  int    status, istep, i;
...
static int
EGlite_nearestOnSurfaceLM(const egObject *geom, const double *point, double *uv,
                      double *coor)
{
  ...
  uvs[0] = uv[0];
  uvs[1] = uv[1];
  status = EGlite_evaluateGeom(geom, uvs, result);
  if (status != EGADS_SUCCESS) return status;
  ...
  status = EGlite_getRange(geom, range, &per);
  if (status != EGADS_SUCCESS) return status;
  ...
  for (istep = 0; istep < 250; istep++) {
    ...
    status   = EGlite_evaluateGeom(geom, uvt, data);
    if (status != EGADS_SUCCESS) return status;
...
static void
EGlite_orderCandidates(liteIndex *cand, double dist2, double *uv, int i, int j)
{
...
int
EGlite_invEvaGeomLimits(const egObject *geomx, /*@null@*/ const double *limits,
  ...
  if (geom->oclass == PCURVE) {
    ...
    if (geom->mtype == BEZIER) {
      for (i = 0; i < 20; i++) {
        ...
        stat = EGlite_evaluateGeom(geom, &tx, data);
        if (stat != EGADS_SUCCESS) return stat;
      ...
      k    = lgeom->header[1];
      stat = EGlite_attributeRet(geom, ".Bad", &atype, &ulen, &ints, &reals, &str);
      ...
      for (j = i = 1; i < lgeom->header[3]; i++) {
        ...
        for (ii = 1; ii <= k; ii++) {
          ...
          a = (data[0]-xyz[0])*(data[0]-xyz[0]) +
              (data[1]-xyz[1])*(data[1]-xyz[1]);
          EGlite_orderCandidates(cand, a, &tx, i, 0);
          if (j == 1) {
            *param = tx;
            b      = a;
          } else {
      ...
      if (j < 20) {
        for (i = 0; i < 20; i++) {
          ...
          a = (data[0]-xyz[0])*(data[0]-xyz[0]) +
              (data[1]-xyz[1])*(data[1]-xyz[1]);
          EGlite_orderCandidates(cand, a, &tx, i+1, 0);
          if (j == 1) {
            *param = tx;
            b      = a;
          } else {
...
/*      printf(" Parabola new range = %le %le\n", range[0], range[1]);  */
    ...
    if (geom->mtype == BSPLINE) {
      for (i = 0; i < 2; i++) {
        ...
        tt   = cand[i].uv[0];
        stat = EGlite_nearestOnPCurve(geom, xyz, range, &tt, coord);
        if (stat != EGADS_SUCCESS) return stat;
      ...
      EGlite_nearestOnPCurveLM(geom, xyz, range, param, result);
    } else {
      stat = EGlite_nearestOnPCurve(geom, xyz, range, param, result);
      if (stat != EGADS_SUCCESS) return stat;
    }
...
/*      printf(" Parabola new range = %le %le\n", range[0], range[1]);  */
    ...
    if (geom->mtype == BSPLINE) {
      for (i = 0; i < 2; i++) {
        if (cand[i].uk == 0) continue;
        tt   = cand[i].uv[0];
        stat = EGlite_nearestOnCurve(geom, xyz, range, &tt, coord);
        if (stat != EGADS_SUCCESS) return stat;
        b = (coord[0]-xyz[0])*(coord[0]-xyz[0]) +
            (coord[1]-xyz[1])*(coord[1]-xyz[1]) +
      ...
      EGlite_nearestOnCurveLM(geom, xyz, range, param, result);
    } else {
      stat = EGlite_nearestOnCurve(geom, xyz, range, param, result);
      if (stat != EGADS_SUCCESS) return stat;
    }
    b = (result[0]-xyz[0])*(result[0]-xyz[0]) +
        (result[1]-xyz[1])*(result[1]-xyz[1]) +
    ...
    if (geom->mtype == BSPLINE) {
      ...
      if ((iDiv > 2) && (jDiv > 2)) {
        for (j = lgeom->header[4]+1;
          ...
          for (i = lgeom->header[1]+1;
            ...
            stat   = EGlite_evaluateGeom(geom, uvs, data);
            if (stat != EGADS_SUCCESS) continue;
            a = (data[0]-xyz[0])*(data[0]-xyz[0]) +
                (data[1]-xyz[1])*(data[1]-xyz[1]) +
                (data[2]-xyz[2])*(data[2]-xyz[2]);
            EGlite_orderCandidates(cand, a, uvs, i, j);
            cnt++;
          }
        }
      ...
      if ( (cnt == 0) ||
        ...
        uvs[0] = param[0];
        uvs[1] = param[1];
        stat   = EGlite_nearestOnSurface(geom, xyz, param, pt);
        if (stat == EGADS_SUCCESS) {
        ...
        for (k = 0; k < 4; k++) {
          ...
          for (jjj = 1; jjj <= lgeom->header[4]; jjj++) {
            ...
            for (iii = 1; iii <= lgeom->header[1]; iii++) {
              ...
              uvx[0] = uvs[0];
              uvx[1] = uvs[1];
              stat   = EGlite_nearestOnSurface(geom, xyz, uvx, pt);
              if (stat == EGADS_SUCCESS)
      ...
      EGlite_nearestOnSurfaceLM(geom, xyz, param, result);
      ...
      if (lgeom->ref->mtype == BSPLINE) {
        ...
        uvs[0] = param[0];
        uvs[1] = param[1];
        stat   = EGlite_nearestOnSurface(geom, xyz, param, result);
...
/*        printf(" Info: NEAREST Fails -- Seed point closer    stat = %d\n",
                 stat);  */
          EGlite_evaluateGeom(geom, uvs, data);
          param[0]  = uvs[0];
          param[1]  = uvs[1];
          result[0] = data[0];
          result[1] = data[1];
...
int
EGlite_invEvaluateGeomGuess(const egObject *geom, /*@null@*/ const double *limits,
...
static int
EGlite_inFace3D(const egObject *face, const double *uv, /*@null@*/ double *p,
            /*@null@*/ double *uvx)
{
  int      i, j, stat;
  ...
  param[0] = param[1] = 0.0;
  stat     = EGlite_evaluate(surface, uv, data);
  if (stat != EGADS_SUCCESS) return stat;
  for (i = 0; i < pface->nloops; i++) {
    ...
    for (j = 0; j < ploop->nedges; j++) {
...
#endif
      stat = EGlite_invEvaluate(edge, data, &tx, xyz);
      if (stat != EGADS_SUCCESS) return stat;
      a = (data[0]-xyz[0])*(data[0]-xyz[0]) +
          (data[1]-xyz[1])*(data[1]-xyz[1]) +
          (data[2]-xyz[2])*(data[2]-xyz[2]);
      if (a < b) {
        ...
        pt[1] = xyz[1];
        pt[2] = xyz[2];
        stat  = EGlite_getEdgeUV(face, edge, ploop->senses[j], tx, param);
        if (stat != EGADS_SUCCESS) return stat;
        if (b < tol*tol) {
          if (p != NULL) {
            p[0]   = pt[0];
        ...
        stat = EGlite_evaluate(edge, &tx, edata);
        if (stat != EGADS_SUCCESS) return stat;
    ...
    for (i = 0; i < pface->nloops; i++) {
      ...
      for (j = 0; j < ploop->nedges; j++) {
        ...
        if (pedge->nodes[0] == node) {
          stat = EGlite_evaluate(edge, &pedge->trange[0], edata);
          if (stat != EGADS_SUCCESS) return stat;
        ...
        if (pedge->nodes[1] == node) {
          stat = EGlite_evaluate(edge, &pedge->trange[1], edata);
          if (stat != EGADS_SUCCESS) return stat;
          tng[0] = ploop->senses[j]*edata[3];
          tng[1] = ploop->senses[j]*edata[4];
          tng[2] = ploop->senses[j]*edata[5];
  ...
  tang[2] /= b;
  CROSS(nrm, pt, tang);
  stat     = EGlite_evaluate(surface, param, data);
  if (stat != EGADS_SUCCESS) return stat;
  pt[0]    = data[3];
  pt[1]    = data[4];
  pt[2]    = data[5];
...
int
EGlite_inFaceX(const egObject *face, const double *uva, /*@null@*/ double *pt,
           /*@null@*/ double *uvx)
{
  ...
  surface = pface->surface;
  if (surface == NULL)            return EGADS_NULLOBJ;
  if (surface->mtype == PLANE)    return EGlite_inFace3D(face, uva, pt, uvx);
  ...
  uv[0] = uva[0];
  uv[1] = uva[1];
  stat  = EGlite_getRange(surface, srange, &sper);
  if (stat != EGADS_SUCCESS) {
    printf(" EGlite_getRange PC = %d in EGlite_inFace!\n", stat);
    return stat;
  }
  /* is uv in range? */
  if ((sper&1) != 0) {
  ...
  stat = EGlite_evaluate(surface, uv, data);
  if (stat != EGADS_SUCCESS) {
    printf(" EGlite_evaluate S = %d in EGlite_inFace!\n", stat);
    return stat;
  }
  xyz[0] = data[0];
  xyz[1] = data[1];
  ...
  for (i = 0; i < pface->nloops; i++) {
    ...
    for (j = 0; j < ploop->nedges; j++) {
      ...
      stat   = EGlite_invEvaGeomLimits(pcurve, pedge->trange, uv, &ts, 0.0, uvs);
      if (stat != EGADS_SUCCESS) {
        printf(" EGlite_invEvaluate = %d in EGlite_inFace!\n", stat);
        return stat;
      }
      /* are we as close as the relative UV tolerance? */
      if ((fabs(uv[0]-uvs[0]) < utol) && (fabs(uv[1]-uvs[1]) < vtol)) {
        if (pt != NULL) {
          stat = EGlite_evaluate(edge, &ts, eval);
          if (stat != EGADS_SUCCESS) {
            printf(" EGlite_evaluate e = %d in EGlite_inFace!\n", stat);
            return stat;
          }
          pt[0]  = eval[0];
          pt[1]  = eval[1];
  ...
  if (pcurv == NULL) {
    printf(" No Edge Found in EGlite_inFace!\n");
...
#endif
    if (pnode == NULL) {
      printf(" NULL Node in EGlite_inFace!\n");
      return EGADS_NODATA;
    }
    ...
    for (i = 0; i < pface->nloops; i++) {
      ...
      for (j = 0; j < ploop->nedges; j++) {
        ...
        if (pedge->nodes[0] == node) {
          ...
          if (pcurve != NULL) {
            ts   = pedge->trange[0];
            stat = EGlite_evaluate(pcurve, &ts, data);
            if (stat != EGADS_SUCCESS) {
              printf("  EGADS Internal: EGlite_evaluate = %d in EGlite_inFace!\n", stat);
              return stat;
            }
...
/*          printf("   hit last  node for %d/%d  %lf\n", j+1, i+1, d);  */
  ...
  stat = EGlite_evaluate(pcurv, &t, data);
  if (stat != EGADS_SUCCESS) {
    printf("  EGADS Internal: EGlite_evaluate = %d in EGlite_inFace!\n", stat);
    return stat;
  }
  ...
  if (d == 0.0) {
    printf(" EGADS Internal: Cross = 0 (EGlite_inFace)!\n");
    printf("                 %le %le  %le %le\n",
...
/*  printf("  d = %le  sense = %d  face or = %d\n", d, sense, face->mtype);
    ...
    if (pt != NULL) {
      ...
      uvs[0] = data[0];
      uvs[1] = data[1];
      stat   = EGlite_evaluate(surface, uvs, data);
      if (stat != EGADS_SUCCESS)
...
static int
EGlite_getPerpDir(double *norm, egObject *edge, int sense, double t, double *dir)
{
  int    stat;
  double dist, result[9], *dt;
  
  dt   = &result[3];
  stat = EGlite_evaluate(edge, &t, result);
  if (stat != EGADS_SUCCESS) return stat;
  CROSS(dir, norm, dt);
...
int
EGlite_getWindingAngle(egObject *edge, double t, double *angle)
{
  ...
  *angle = -90.0;
  ...
  stat = EGlite_getBody(edge, &body);
  if (stat != EGADS_SUCCESS)      return stat;
  if (body == NULL)               return EGADS_NODATA;
  
  stat = EGlite_getBodyTopos(body, edge, FACE, &nface, &faces);
  if (stat != EGADS_SUCCESS) return stat;
  if (nface == 1) {
    *angle = 180.0;
    EGlite_free(faces);
  ...
  for (i = 0; i < nface; i++) {
    /* find the sense */
    stat = EGlite_getTopology(faces[i], &sur, &oclass, &mtype, lims, &nloop, &loops,
                          &senses);
    if (stat != EGADS_SUCCESS) {
      EGlite_free(faces);
      return stat;
    ...
    for (sense = j = 0; j < nloop; j++) {
      stat = EGlite_getTopology(loops[j], &ref, &oclass, &mtype, lims, &n, &edges,
                            &senses);
      if (stat != EGADS_SUCCESS) {
        EGlite_free(faces);
        return stat;
    ...
    /* get the normals */
    stat = EGlite_getEdgeUV(faces[i], edge, sense, t, uv);
    if (stat != EGADS_SUCCESS) {
      EGlite_free(faces);
      return stat;
    }
    ...
    stat = EGlite_getPerpDir(norms[i], edge, sense, t, dir[i]);
    if (stat != EGADS_SUCCESS) {
      EGlite_free(faces);
      return stat;
    }
  ...
  if ((dist < -1.0000001) || (dist > 1.0000001))
    printf(" EGlite_getWindingAngle: dot = %20.12lf\n", dist);
