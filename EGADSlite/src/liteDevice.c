...
__HOST_AND_DEVICE__ extern int EGlite_evaluate_lite.const egObject *geom,
                                           const double *param, double *ev);
__HOST_AND_DEVICE__ extern int EGlite_attributeRet(const egObject *obj,
                                               const char *name, int *atype,
                                               int *len,
                                               /*@null@*/ const int **ints,
                                               /*@null@*/ const double **reals,
                                               /*@null@*/ const char **str);
__HOST_AND_DEVICE__ extern int EGlite_addStrAttr(egObject *obj, const char *name,
                                             const char *str);

int EGlite_attributeRetDev(const egObject *obj_d, const char *name, int *atype,
                       int *len, int **ints, double **reals, char **str);
int EGlite_addStrAttrDev(egObject *obj_d, const char *name, const char *str);
int EGlite_evaluateDev(const egObject *geom_d, const double *param, double *ev);
...
__device__
size_t EGlite_strlen(const char *s)
{
  size_t i;
  for (i = 0; ; ++i)
    if (s[i] == '\0') break;
...
__device__
size_t EGlite_strcmp(const char *s1, const char *s2)
{
  size_t i = 0;
  do {
    if (s1[i] != s2[i]) {
...
__device__
static int EGlite_addStrAttr_d(egObject *obj, const char *name, const char *str)
{
  int     i, length, find = -1;
  egAttr  *attr;
  egAttrs *attrs;
  ...
  if (attrs != NULL)
    for (i = 0; i < attrs->nattrs; i++)
      if (EGlite_strcmp(attrs->attrs[i].name,name) == 0) {
        find = i;
        break;
      }
  ...
  if ((find != -1) && (attrs != NULL)) {
    ...
    if (attrs->attrs[find].type == ATTRINT) {
      if (attrs->attrs[find].length != 1)
        EGlite_free(attrs->attrs[find].vals.integers);
    } else if (attrs->attrs[find].type == ATTRREAL) {
      if (attrs->attrs[find].length != 1)
        EGlite_free(attrs->attrs[find].vals.reals);
    } else if (attrs->attrs[find].type == ATTRCSYS) {
      EGlite_free(attrs->attrs[find].vals.reals);
    } else if (attrs->attrs[find].type == ATTRSTRING) {
      EGlite_free(attrs->attrs[find].vals.string);
    }
    
  } else {
    
    if (attrs == NULL) {
      attrs = (egAttrs *) EGlite_alloc(sizeof(egAttrs));
      if (attrs == NULL) {
    ...
    if (attrs->attrs == NULL) {
      attr = (egAttr *) EGlite_alloc((attrs->nattrs+1)*sizeof(egAttr));
    } else {
      attr = (egAttr *) EGlite_reall(attrs->attrs,
    ...
    attrs->attrs[find].name        = EGlite_strdup(name);
  ...
  attrs->attrs[find].vals.string = EGlite_strdup(str);
  if (attrs->attrs[find].vals.string != NULL)
    attrs->attrs[find].length = EGlite_strlen(attrs->attrs[find].vals.string);
...
{
  attr_d->status = EGlite_attributeRet(obj_d, attr_d->name, &attr_d->atype,
...
{
  attr_d->status = EGlite_addStrAttr_d(obj_d, attr_d->name, attr_d->str);
}
...
{
  eval_d->status = EGlite_evaluate(geom_d, eval_d->param, eval_d->ev);
                       ...
                       int *len, int **ints, double **reals, char **str)
{
  ...
  EGlite_SET_STR(&(attr_d->name), name);
  ...
  *len = attr_h->len;
  if (*atype == ATTRINT) {
    if (attr_h->ints != NULL)
      if (attr_h->len <= 1) {
        *ints = (int *) EGlite_alloc(attr_h->len*sizeof(int));
    ...
    if (reals != NULL)
      if (attr_h->len <= 1) {
        *reals = (double *) EGlite_alloc(attr_h->len*sizeof(double));
    ...
    *len = attr_h->len - 12;
    ...
    if (str != NULL) {
      *str = (char *) EGlite_alloc((attr_h->len+1)*sizeof(char));
      *str[0] = '\0';
      if( 0 < attr_h->len )
...
int EGlite_evaluateDev(const egObject *geom_d, const double *param, double *ev)
{
  ...
  EGlite_GET(&oclass, &(geom_d->oclass), sizeof(short), 1);
  switch(oclass) {
    case(PCURVE):
    case(CURVE):
    case(EDGE):
  ...
  EGlite_COPY(&(eval_d->param), param, sizeof(double), nparam);
