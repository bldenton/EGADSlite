...
__PROTO_H_AND_D__ int  EGlite_evaluateGeom( const egObject *geom,
                                        const double *param, double *result );
__PROTO_H_AND_D__ int  EGlite_invEvaGeomLimits( const egObject *geom,
                                            /*@null@*/ const double *limits,
                                            const double *xyz, double *param,
                                            double toler, double *result );
__PROTO_H_AND_D__ int  EGlite_invEvaluateGeomGuess( const egObject *geom,
                                                /*@null@*/ const double *lmts,
                                                double *xyz, double *param,
                                                double *result );

__PROTO_H_AND_D__ int  EGlite_inFaceX( const egObject *face, const double *uv,
                                   /*@null@*/ double *pt,
                                   /*@null@*/ double *uvx );
...
EGlite_getGeometry(const egObject     *geom, int *oclass, int *type,
                                         ...
                                         /*@null@*/ double **rvec)
{
  ...
  *refGeom = lgeom->ref;
  ...
  if (nreal == 0) {
    printf(" EGlite_getGeometry: OCLASS = %d, MTYPE = %d not found!\n",
           geom->oclass, geom->mtype);
    return EGADS_GEOMERR;
  }

  if (nint != 0) {
    ints = (int *) EGlite_alloc(nint*sizeof(int));
    if (ints == NULL) return EGADS_MALLOC;
    for (i = 0; i < nint; i++) ints[i] = lgeom->header[i];
    *ivec = ints;
  }
  reals = (double *) EGlite_alloc(nreal*sizeof(double));
  if (reals == NULL) {
    if (*ivec != NULL) {
      EGlite_free(*ivec);
      *ivec = NULL;
...
EGlite_getRangeCurve(const egObject *geomx, double *range, int *periodic)
{
  int            stat, mtype;
  egObject       *ref;
  liteEdge       *ledge;
...
__HOST_AND_DEVICE__ static int
EGlite_getRangeSurface(const egObject *geomx, double *range, int *periodic)
{
  int            stat, mtype;
  egObject       *ref;
  liteFace       *lface;
...
surRecurse:
  ...
  switch (mtype) {
    ...
    case REVOLUTION:
      stat = EGlite_getRangeCurve(lgeom->ref, &range[2], &mtype);
    ...
    case EXTRUSION:
      stat = EGlite_getRangeCurve(lgeom->ref, range, periodic);
      if (stat == EGADS_SUCCESS) {
        range[2] = -2.e100;
        range[3] =  2.e100;
      }
...
__HOST_AND_DEVICE__ int
EGlite_getRange(const egObject *geom, double *range, int *periodic)
{
  *periodic = 0;
  if  (geom == NULL)               return EGADS_NULLOBJ;
  if  (geom->magicnumber != MAGIC) return EGADS_NOTOBJ;
  ...
  if ((geom->oclass == SURFACE) || (geom->oclass == FACE)) {
    return EGlite_getRangeSurface(geom, range, periodic);
  } else {
    return EGlite_getRangeCurve(geom, range, periodic);
  }
}
  ...
  *periodic = 0;
  ...
  if (geom->oclass == PCURVE) {
    ...
    switch (geom->mtype) {
      ...
      case TRIMMED:
        stat = EGlite_getRange(lgeom->ref, range, periodic);
        if (stat == EGADS_SUCCESS) {
          range[0] = lgeom->data[0];
    ...
    switch (mtype) {
      ...
      case REVOLUTION:
        stat = EGlite_getRange(lgeom->ref, &range[2], &mtype);
        if (stat == EGADS_SUCCESS) {
          range[0]  = 0.0;
          range[1]  = 2.0*PI;
          *periodic = 1 + 2*mtype;
...
__HOST_AND_DEVICE__ int
EGlite_evaluate(const egObject *geom, /*@null@*/ const double *param, double *result)
{
  const egObject *ref;
  liteNode       *lnode;
  liteEdge       *ledge;
  ...
  if ((geom->oclass == PCURVE) || (geom->oclass == CURVE) ||
      (geom->oclass == SURFACE)) return EGlite_evaluateGeom(geom, param, result);
  ...
  if (ref == NULL)        return EGADS_NULLOBJ;
  if (ref->blind == NULL) return EGADS_NODATA;
  return EGlite_evaluateGeom(ref, param, result);
}


__HOST_AND_DEVICE__ int
EGlite_invEvaLimits(const egObject *geom, /*@null@*/ const double *limits,
                const double *xyz, double *param, double *result)
{
  ...
  if ((geom->oclass == PCURVE) || (geom->oclass == CURVE) ||
      (geom->oclass == SURFACE))
    return EGlite_invEvaGeomLimits(geom, limits, xyz, param, 0.0, result);
  ...
  if (geom->oclass == EDGE) {
    ...
    if (ref == NULL)        return EGADS_NULLOBJ;
    if (ref->blind == NULL) return EGADS_NODATA;
    return EGlite_invEvaGeomLimits(ref, range, xyz, param, 0.0, result);
  }
  ...
  stat = EGlite_invEvaGeomLimits(ref, range, xyz, param, pface->tol, result);
  if (stat != EGADS_SUCCESS) return stat;
  
  stat = EGlite_getRange(ref, srange, &per);
  if (stat != EGADS_SUCCESS) return stat;

  stat = EGlite_inFaceX(geom, param, pt, uvs);
  if (stat < EGADS_SUCCESS) return stat;
...
__HOST_AND_DEVICE__ int
EGlite_invEvaluate(const egObject *geom, const double *xyz, double *param,
               double *result)
{
  return EGlite_invEvaLimits(geom, NULL, xyz, param, result);
}


__HOST_AND_DEVICE__ int
EGlite_invEvaluateGuess(const egObject *geom, double *xyz,
                    double *param, double *result)
{
  ...
  if ((geom->oclass == PCURVE) || (geom->oclass == CURVE) ||
      (geom->oclass == SURFACE))
    return EGlite_invEvaluateGeomGuess(geom, NULL, xyz, param, result);
  ...
  if (geom->oclass == EDGE) {
    ...
    if (ref == NULL)        return EGADS_NULLOBJ;
    if (ref->blind == NULL) return EGADS_NODATA;
    stat = EGlite_invEvaluateGeomGuess(ref, range, xyz, param, result);
    ...
    if (ref == NULL)        return EGADS_NULLOBJ;
    if (ref->blind == NULL) return EGADS_NODATA;
    stat = EGlite_invEvaluateGeomGuess(ref, NULL, xyz, param, result);
  ...
  *alen = 0.0;
  ...
  if (geom->oclass == PCURVE) {
    ...
    for (i = 0; i < ngauss; i++) {
      ...
      stat = EGlite_evaluate(geom, &t, result);
      if (stat != EGADS_SUCCESS) return stat;
    ...
    *alen *= 0.5*ur;
    ...
    for (i = 0; i < ngauss; i++) {
      t    = t1 + 0.5*wg[2*i+1]*ur + mid;
      stat = EGlite_evaluate(ref, &t, result);
      if (stat != EGADS_SUCCESS) return stat;
...
__HOST_AND_DEVICE__ int
EGlite_curvature(const egObject *geom, const double *param, double *result)
{
  int    i, stat;
  ...
  stat = EGlite_evaluate(geom, param, data);
  if (stat != EGADS_SUCCESS) return stat;
...
__HOST_AND_DEVICE__ int
EGlite_relPosTs(egObject *geom, int n, /*@null@*/ const double *rel,
            double *ts, double *xyzs)
{
  ...
  stat = EGlite_arcLength(geom, ts[0], ts[n-1], &alen);
  if (stat != EGADS_SUCCESS) return stat;
  ...
  for (i = 1; i < n-1; i++) {
    ...
    t    = 0.5*(tbeg + tend);
    for (j = 0; j < 20; j++) {
      stat = EGlite_arcLength(geom, ts[0], t, &len);
      if (stat != EGADS_SUCCESS) return stat;
