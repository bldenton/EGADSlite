...
EGlite_calloc(size_t nele, size_t size)
{
#ifndef __CUDA_ARCH__
  if (nele*size == 0) return NULL;
  return calloc(nele, size);
#else
  void *ptr = EGlite_alloc(nele*size);
...
__HOST_AND_DEVICE__ /*@null@*/ /*@only@*/ void *
EGlite_reall(/*@null@*/ /*@only@*/ /*@returned@*/ void *ptr, size_t nbytes)
{
#ifndef __CUDA_ARCH__
  if (nbytes == 0) return NULL;
...
EGlite_free(/*@null@*/ /*@only@*/ void *ptr)
{
  if (ptr == NULL) return;
  free(ptr);
}
...
EGlite_strdup(/*@null@*/ const char *str)
{
#ifndef __CUDA_ARCH__
  int  i, len;
  char *dup;
  ...
  len = strlen(str) + 1;
  dup = (char *) EGlite_alloc(len*sizeof(char));
  if (dup != NULL)
