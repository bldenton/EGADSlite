...
attrOut(int level, ego object)
{
  ...
  stat = EGlite_attributeNum(object, &nattr);
  if (stat  != EGADS_SUCCESS) return;
  if (nattr == 0)             return;
  
  for (i = 1; i <= nattr; i++) {
    stat = EGlite_attributeGet(object, i, &name, &atype, &alen, &ints, &reals, &str);
...
parseOut(int level, ego object, /*@null@*/ ego body, int sense)
{
  ...
  if ((oclass >= PCURVE) && (oclass <= SURFACE)) {
    stat = EGlite_getGeometry(object, &oclass, &mtype, &geom, &ivec, &rvec);
    if (stat != EGADS_SUCCESS) {
      printf(" parseOut: %d EGlite_getGeometry return = %d\n", level, stat);
      return;
    }
    stat = EGlite_getRange(object, limits, &periodic);
...
/*        for (i = 0; i < 2*level+2; i++) printf(" ");
    ...
    if (ivec != NULL) EGlite_free(ivec);
    if (rvec != NULL) EGlite_free(rvec);
  ...
  if ((oclass >= NODE) && (oclass < BODY))
    if (body != NULL) index = EGlite_indexBodyTopo(body, object);
#ifdef WIN32
  if (sense == 0) {
  ...
  if ((oclass >= NODE) && (oclass <= MODEL)) {
    stat = EGlite_getTopology(object, &geom, &oclass, &mtype, limits, &nobjs,
                          &objs, &senses);
    if (stat != EGADS_SUCCESS) {
      printf(" parseOut: %d EGlite_getTopology return = %d\n", level, stat);
      return;
    }
    if (oclass == NODE) {
      for (i = 0; i < 2*level+2; i++) printf(" ");
    ...
    if (oclass != NODE) {
      stat = EGlite_getBoundingBox(object, bbox);
      if (stat == EGADS_SUCCESS) {
        for (i = 0; i < 2*level+2; i++) printf(" ");
...
int main(int argc, char *argv[])
{
  ...
  printf(" EGlite_loadModel     = %d  %s\n", EGlite_loadModel(context, 0, argv[1],
                                                      &model), argv[1]);
  
  /* test bodyTopo functions */
  stat = EGlite_getTopology(model, &geom, &oclass, &mtype, NULL, &nbodies,
                        &bodies, &senses);
  if (stat == EGADS_SUCCESS)
    for (i = 0; i < nbodies; i++) {
      stat = EGlite_getBodyTopos(bodies[i], NULL, NODE, &n, &objs);
      if (stat != EGADS_SUCCESS) {
        printf(" ERROR: %d Node EGlite_getBodyTopos = %d!\n", i, stat);
        continue;
      }
      for (j = 0; j < n; j++) {
        k = EGlite_indexBodyTopo(bodies[i], objs[j]);
        if (k != j+1) printf("  Node Index = %d but should be %d!\n", k, j+1);
      }
      EGlite_free(objs);
      stat = EGlite_getBodyTopos(bodies[i], NULL, EDGE, &n, &objs);
      if (stat != EGADS_SUCCESS) {
        printf(" ERROR: %d Edge EGlite_getBodyTopos = %d!\n", i, stat);
        continue;
      }
      for (j = 0; j < n; j++) {
        k = EGlite_indexBodyTopo(bodies[i], objs[j]);
      ...
      EGlite_free(objs);
      stat = EGlite_getBodyTopos(bodies[i], NULL, LOOP, &n, &objs);
      if (stat != EGADS_SUCCESS) {
        printf(" ERROR: %d Loop EGlite_getBodyTopos = %d!\n", i, stat);
        continue;
      }
      for (j = 0; j < n; j++) {
        k = EGlite_indexBodyTopo(bodies[i], objs[j]);
        if (k != j+1) printf("  Loop Index = %d but should be %d!\n", k, j+1);
        stat = EGlite_getBodyTopos(bodies[i], objs[j], NODE, &nn, &nobjs);
        if (stat != EGADS_SUCCESS) {
        ...
        EGlite_free(nobjs);
        stat = EGlite_getBodyTopos(bodies[i], objs[j], FACE, &nn, &nobjs);
        if (stat != EGADS_SUCCESS) {
      ...
      EGlite_free(objs);
      stat = EGlite_getBodyTopos(bodies[i], NULL, FACE, &n, &objs);
      if (stat != EGADS_SUCCESS) {
        printf(" ERROR: Face %d EGlite_getBodyTopos = %d!\n", i, stat);
        continue;
      }
      for (j = 0; j < n; j++) {
        k = EGlite_indexBodyTopo(bodies[i], objs[j]);
      ...
      stat = EGlite_getBodyTopos(bodies[i], NULL, SHELL, &n, &objs);
      if (stat != EGADS_SUCCESS) {
        printf(" ERROR: Shell %d EGlite_getBodyTopos = %d!\n", i, stat);
        continue;
      }
      if (objs != NULL) {
        for (j = 0; j < n; j++) {
