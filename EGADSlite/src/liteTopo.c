...
__PROTO_H_AND_D__ int EGlite_evaluate( const egObject *geom, const double *param,
                                   double *result );
__PROTO_H_AND_D__ int EGlite_invEvaluate( const egObject *geom, const double *xyz,
                                      double *param, double *result );
__PROTO_H_AND_D__ int EGlite_invEvaLimits( const egObject *geom,
                                       /*@null@*/ const double *lim,
                                       const double *xyz, double *param,
                                       double *result );
__PROTO_H_AND_D__ int EGlite_getRange( const egObject *geom, double *range,
                                   int *periodic );
__PROTO_H_AND_D__ int EGlite_inFaceX( const egObject *face, const double *uva,
                                  /*@null@*/ double *pt,
                                  /*@null@*/ double *uvx );


__HOST_AND_DEVICE__ int
EGlite_getTopology(const egObject *topo, egObject **geom, int *oclass,
...
__HOST_AND_DEVICE__ static int EGlite_containedEdge(egObject *obj, egObject *src)
{
  int      i;
  liteEdge *pedge;
...
__HOST_AND_DEVICE__ static int EGlite_containedLoop(egObject *obj, egObject *src)
{
  int      i, stat;
  liteLoop *ploop;
  ...
  if (obj->oclass == EDGE) {
    ...
    for (i = 0; i < ploop->nedges; i++) {
      stat = EGlite_containedEdge(obj, ploop->edges[i]);
...
__HOST_AND_DEVICE__ static int EGlite_containedFace(egObject *obj, egObject *src)
{
  int      i, stat;
  liteFace *pface;
  ...
  if (obj->oclass == LOOP) {
    ...
    for (i = 0; i < pface->nloops; i++) {
      stat = EGlite_containedLoop(obj, pface->loops[i]);
      if (stat == EGADS_SUCCESS) return stat;
    }
  }
...
__HOST_AND_DEVICE__ static int EGlite_containedShel(egObject *obj, egObject *src)
{
  int       i, stat;
  liteShell *pshell;
  ...
  if (obj->oclass == FACE) {
    ...
    for (i = 0; i < pshell->nfaces; i++) {
      stat = EGlite_containedFace(obj, pshell->faces[i]);
      if (stat == EGADS_SUCCESS) return stat;
    }
  }
...
__HOST_AND_DEVICE__ static int EGlite_contained(egObject *obj, egObject *src)
{
  if (src->oclass == EDGE) {
    return EGlite_containedEdge(obj, src);
  } else if (src->oclass == LOOP) {
    return EGlite_containedLoop(obj, src);
  } else if (src->oclass == FACE) {
    return EGlite_containedFace(obj, src);
  } else if (src->oclass == SHELL) {
    return EGlite_containedShel(obj, src);
  }
...
static int
EGlite_contained(egObject *obj, egObject *src)
{
  int i, stat;
  ...
  if (src->oclass == EDGE) {
    ...
    if (obj->oclass == EDGE) {
      ...
      for (i = 0; i < ploop->nedges; i++) {
        stat = EGlite_contained(obj, ploop->edges[i]);
        if (stat == EGADS_SUCCESS) return stat;
      }
    }
  } else if (src->oclass == FACE) {
    ...
    if (obj->oclass == LOOP) {
      ...
      for (i = 0; i < pface->nloops; i++) {
        stat = EGlite_contained(obj, pface->loops[i]);
        if (stat == EGADS_SUCCESS) return stat;
      }
    }
  } else if (src->oclass == SHELL) {
    ...
    if (obj->oclass == FACE) {
      ...
      for (i = 0; i < pshell->nfaces; i++) {
        stat = EGlite_contained(obj, pshell->faces[i]);
        if (stat == EGADS_SUCCESS) return stat;
      }
    }
  }
...
__HOST_AND_DEVICE__ int
EGlite_getTolerance(const egObject *topo, double *toler)
{
  if  (topo == NULL)               return EGADS_NULLOBJ;
  if  (topo->magicnumber != MAGIC) return EGADS_NOTOBJ;
...
__HOST_AND_DEVICE__ int
EGlite_tolerance(const egObject *topo, double *toler)
{
  int    i, stat;
  double tol;
  ...
  *toler = 0.0;
  if (topo->oclass == NODE) {
    ...
    *toler = pnode->tol;
    ...
    pedge = (liteEdge *) topo->blind;
    *toler = pedge->tol;
    stat   = EGlite_tolerance(pedge->nodes[0], &tol);
    if (stat != EGADS_SUCCESS) return stat;
    if (tol > *toler) *toler = tol;
    stat   = EGlite_tolerance(pedge->nodes[1], &tol);
    if (stat != EGADS_SUCCESS) return stat;
    if (tol > *toler) *toler = tol;
  } else if (topo->oclass == LOOP) {
    liteLoop *ploop;
    ploop = (liteLoop *) topo->blind;
    for (i = 0; i < ploop->nedges; i++) {
      stat = EGlite_tolerance(ploop->edges[i], &tol);
      if (stat != EGADS_SUCCESS) return stat;
      if (tol > *toler) *toler = tol;
    }
  } else if (topo->oclass == FACE) {
    ...
    *toler = pface->tol;
    for (i = 0; i < pface->nloops; i++) {
      stat = EGlite_tolerance(pface->loops[i], &tol);
      if (stat != EGADS_SUCCESS) return stat;
      if (tol > *toler) *toler = tol;
    }
  } else if (topo->oclass == SHELL) {
    ...
    pshell = (liteShell *) topo->blind;
    for (i = 0; i < pshell->nfaces; i++) {
      stat = EGlite_tolerance(pshell->faces[i], &tol);
      if (stat != EGADS_SUCCESS) return stat;
      if (tol > *toler) *toler = tol;
    }
  } else {
    ...
    for (i = 0; i < pbody->shells.nobjs; i++) {
      stat = EGlite_tolerance(pbody->shells.objs[i], &tol);
      if (stat != EGADS_SUCCESS) return stat;
      if (tol > *toler) *toler = tol;
    }
  }
...
__HOST_AND_DEVICE__ int
EGlite_getBodyTopos(const egObject *body, /*@null@*/ egObject *src,
                int oclass, int *ntopo, /*@null@*/ egObject ***topos)
{
  ...
  *ntopo = 0;
  ...
  if (src == NULL) {
    ...
    objs = (egObject **) EGlite_alloc(map.nobjs*sizeof(egObject *));
  ...
  if (src->oclass > oclass) {
    for (n = i = 0; i < map.nobjs; i++)
      if (EGlite_contained(map.objs[i], src) == EGADS_SUCCESS) n++;
    if (n == 0) return EGADS_SUCCESS;
    if (topos == NULL) {
      *ntopo = n;
      return EGADS_SUCCESS;
    }
    objs = (egObject **) EGlite_alloc(n*sizeof(egObject *));
    if (objs == NULL) return EGADS_MALLOC;
    for (n = i = 0; i < map.nobjs; i++)
      if (EGlite_contained(map.objs[i], src) == EGADS_SUCCESS) {
        objs[n] = map.objs[i];
        n++;
      }
    *ntopo = n;
  ...
  /* look up the tree */
  for (n = i = 0; i < map.nobjs; i++)
    if (EGlite_contained(src, map.objs[i]) == EGADS_SUCCESS) n++;
  if (n == 0) return EGADS_SUCCESS;
  if (topos == NULL) {
    *ntopo = n;
    return EGADS_SUCCESS;
  ...
  if (objs == NULL) return EGADS_MALLOC;
  for (n = i = 0; i < map.nobjs; i++)
    if (EGlite_contained(src, map.objs[i]) == EGADS_SUCCESS) {
      objs[n] = map.objs[i];
      n++;
    }
  *ntopo = n;
...
__HOST_AND_DEVICE__ int
EGlite_indexBodyTopo(const egObject *body, const egObject *src)
{
  int      i;
  liteBody *pbody;
...
__HOST_AND_DEVICE__ int
EGlite_objectBodyTopo(const egObject *body, int oclass, int index, egObject **obj)
{
  liteBody *pbody;
  liteMap  map;
...
__HOST_AND_DEVICE__ int
EGlite_getBoundingBox(const egObject *topo, double *bbox)
{
  int i;
...
__HOST_AND_DEVICE__ int
EGlite_getEdgeUV(const egObject *face, const egObject *edge, int sense, double t,
             double *result)
{
  int      i, j, stat, found;
  double   data[9], xyz[3];
  ...
  if (surface->mtype == PLANE) {
    stat = EGlite_evaluate(edge, &t, data);
    if (stat != EGADS_SUCCESS) return stat;
    return EGlite_invEvaluate(pface->surface, data, result, xyz);
  }
  ...
  if (sense == 0) {
    ...
    if (found > 1) {
      printf(" EGADS Warning: Edge in Face twice & sense=0 (EGlite_getEdgeUV)!\n");
  ...
  for (i = 0; i < pface->nloops; i++) {
    ...
    for (j = 0; j < ploop->nedges; j++)
      if (ploop->edges[j] == edge) {
        ...
        stat   = EGlite_evaluate(pcurve, &t, data);
...
__HOST_AND_DEVICE__ int
EGlite_getEdgeUVs(const egObject *face, const egObject *edge, int sense, int nt,
              const double *t, double *uvs)
{
  ...
  if (surface->mtype == PLANE) {
    for (k = 0; k < nt; k++) {
      stat = EGlite_evaluate(edge, &t[k], data);
      if (stat != EGADS_SUCCESS) return stat;
      stat = EGlite_invEvaluate(pface->surface, data, &uvs[2*k], xyz);
      if (stat != EGADS_SUCCESS) return stat;
    }
    return EGADS_SUCCESS;
  }
  ...
  for (i = 0; i < pface->nloops; i++) {
    ...
    for (j = 0; j < ploop->nedges; j++)
      if (ploop->edges[j] == edge) {
        ...
        pcurve = ploop->edges[j+ploop->nedges];
        for (k = 0; k < nt; k++) {
          stat = EGlite_evaluate(pcurve, &t[k], data);
          if (stat != EGADS_SUCCESS) return stat;
          uvs[2*k  ] = data[0];
          uvs[2*k+1] = data[1];
        }
...
__HOST_AND_DEVICE__ int
EGlite_getEdgeUVeval(const egObject *face, const egObject *edge, int sense,
                 double t, double *result)
{
  ...
  if (surface->mtype == PLANE) {
    ...
    stat = EGlite_invEvaluate(pface->surface, data, result, xyz);
    if (stat != EGADS_SUCCESS) return stat;
    stat = EGlite_evaluate(face, result, eval);
    if (stat != EGADS_SUCCESS) return stat;
    ...
    for (j = 0; j < ploop->nedges; j++)
      if (ploop->edges[j] == edge) {
        if (sense != 0)
          if (sense != ploop->senses[j]) continue;
        pcurve = ploop->edges[j+ploop->nedges];
        return EGlite_evaluate(pcurve, &t, result);
      }
  }
...
__HOST_AND_DEVICE__ int
EGlite_getBody(const egObject *obj, egObject **body)
{
  int       i;
  liteModel *pmodel;
  egObject  *topObj, *bod;
    ...
    *body = topObj;
    ...
    if (pmodel != NULL) {
      for (i = 0; i < pmodel->nbody; i++) {
        ...
        if (EGlite_indexBodyTopo(bod, obj) <= 0) continue;
        *body = bod;
        break;
      }
    }
...
__HOST_AND_DEVICE__ int
EGlite_inFace(const egObject *face, const double *uv)
{
  return EGlite_inFaceX(face, uv, NULL, NULL);
}


__HOST_AND_DEVICE__ int
EGlite_inTopology(const egObject *topo, const double *xyz)
{
  int       i, j, stat;
  ...
  if (topo->oclass == EDGE) {
    pedge = (liteEdge *) topo->blind;
    stat  = EGlite_invEvaluate(pedge->curve, xyz, param, coord);
    if (stat != EGADS_SUCCESS) return stat;
    ...
    pface = (liteFace *) topo->blind;
    stat  = EGlite_invEvaluate(pface->surface, xyz, param, coord);
    if (stat != EGADS_SUCCESS) return stat;
    dist  = sqrt((xyz[0]-coord[0])*(xyz[0]-coord[0]) +
                 (xyz[1]-coord[1])*(xyz[1]-coord[1]) +
                 (xyz[2]-coord[2])*(xyz[2]-coord[2]));
    if (dist > pface->tol) return EGADS_OUTSIDE;
    return EGlite_inFace(topo, param);
    ...
    for (i = 0; i < pshell->nfaces; i++) {
      ...
      stat  = EGlite_invEvaluate(pshell->faces[i], xyz, param, coord);
      if (stat != EGADS_SUCCESS) return stat;
