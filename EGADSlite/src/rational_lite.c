...
void EGlite_EvaluateQuotientRule(int dim, int der_count, int v_stride, DOUBLE *v)
{
  /*
...
__HOST_AND_DEVICE__ TEMPLATE
void EGlite_EvaluateQuotientRule2(int dim, int der_count, int v_stride, DOUBLE *v)
{
...
__HOST_AND_DEVICE__ TEMPLATE
static void
EGlite_IncreaseBezierDegree(int dim, int order, int cv_stride, DOUBLE* cv)
...
__HOST_AND_DEVICE__ TEMPLATE
static bool
EGlite_RemoveBezierSingAt0(int dim, int order, int cv_stride, DOUBLE* cv)
{
  const int cvdim = dim+1;
  int j,k,ord0;
  ...
  while (order < ord0)
    EGlite_IncreaseBezierDegree(dim, order++, cv_stride, cv);
  return true;
}
...
__HOST_AND_DEVICE__ TEMPLATE
bool EGlite_Bezier1DRat(int dim,              // dimension
                    int order,            // order
                    ...
                    DOUBLE *v           ) // v[(der_count+1)*v_stride] array
{
...
#ifndef __clang_analyzer__
  if (CV[dim] == 0.0) {
    if ( !EGlite_RemoveBezierSingAt0(dim,order,cvdim,CV) ) {
  ...
  EGlite_EvaluateQuotientRule(dim, der_count, cvdim, CV);
...
#ifndef LITE
template void EGlite_EvaluateQuotientRule( int dim, int der_count, int v_stride,
                                       SurrealS<1> *v );
template void EGlite_EvaluateQuotientRule( int dim, int der_count, int v_stride,
                                       double *v );

template void EGlite_EvaluateQuotientRule2( int dim, int der_count, int v_stride,
                                        SurrealS<1> *v );
template void EGlite_EvaluateQuotientRule2( int dim, int der_count, int v_stride,
                                        double *v );
