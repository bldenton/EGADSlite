...
EGlite_getSidepoint(const ego face, double fac, const double *uvm,
...
static void meshStats(meshMap *qm, int sweep)
{
  ...
  valptr = (int*)EGlite_alloc(2 * (mv + 1) * sizeof(int));
  if (valptr == NULL) {
    fclose(fout);
...
static void gnuData(meshMap *qm, /*@null@*/ char *name)
{
  ...
  for (i = 0; i < qm->totQ; ++i) {
      ...
      for (k = 0; k < 4; ++k) {
          ...
          dist = 0.0;
          EGlite_evaluate(qm->face, &qm->uvs[2*v], eval);
          for (d = 0; d < 3; ++d)
      ...
      EGlite_invEvaluateGuess(qm->face, &eval[18], average, eval);
...
__HOST_AND_DEVICE__ static void printQuad (meshMap *qm, int id)
{
  ...
  i = EGlite_invEvaluate(qm->face, uv, uva, xyz);
  printf("\n\n\n %d  %lf %lf %lf %d %lf %lf \n",
...
__HOST_AND_DEVICE__ static void updateVertex(meshMap *qm, int vID, double *uv)
{
  ...
  if (EGlite_evaluate(qm->face, uv, eval) != EGADS_SUCCESS ||
...
__HOST_AND_DEVICE__ static int EGlite_regBoundQuad(meshMap *qm, int qID)
{
...
EGlite_adjQtoPair(meshMap *qm, int qID, int v1, int v2, int *adj)
{
  int i, aux = -1;
...
__HOST_AND_DEVICE__ static int EGlite_quadVertIdx(meshMap *qm, int q, int v)
{
  int i = 0;
...
__HOST_AND_DEVICE__ static void EGlite_freeStar(vStar **star)
{
  if ((*star) == NULL) return;
  EGlite_free((*star)->verts);
  EGlite_free((*star)->quads);
  EGlite_free((*star)->idxQ);
  EGlite_free((*star)->idxV);
  EGlite_free((*star)->area);
  EGlite_free((*star)->angle);
  EGlite_free((*star)->ratio);
  EGlite_free (*star);
  *star = NULL;
}


__HOST_AND_DEVICE__ static int EGlite_buildStar(meshMap *qm, vStar **star, int vID)
{
  int i = 0, id0 = -1, q = 0, auxV, auxQ, v = 0, quadID, prevQuad,
      it = 0, it2 = 0, adj[2], *vertex = NULL, *quads = NULL;
  int qLoop[8] = {0, 1, 2, 3, 0, 1, 2, 3};

  vertex = (int *) EGlite_alloc(qm->totV * sizeof(int));
  quads  = (int *) EGlite_alloc(qm->totQ * sizeof(int));
  if (vertex == NULL || quads == NULL) {
      printf(" EGlite_buildStar MALLOC at quads & verts!!\n ");
      if (vertex != NULL) EGlite_free(vertex);
      if (quads  != NULL) EGlite_free(quads);
      return EGADS_MALLOC;
  }
  // quads are -1 bias
  ...
  it          = 0;
  do {
      id0 = EGlite_quadVertIdx(qm, quadID + 1, vID);
      if (id0 < 0) {
          printf(" In EGlite_buildStar id for Vert %d in Quad %d is %d !!\n",
                 vID, quadID + 1, id0);
#ifdef DEBUG
      ...
      if (quadID  < 0) { //make a "ghost" quad
          ...
          it2         = 0;
          do {
              i    = EGlite_adjQtoPair(qm, auxQ + 1, vID, auxV, adj);
              if (adj[1] == -1 || i != EGADS_SUCCESS) break;
              auxQ = adj[1] - 1;
              i    = EGlite_quadVertIdx(qm, auxQ + 1, vID);
              if (i < 0) {
      ...
      it++;
      if (it > 200) {
          printf(" EGlite_buildStar:: stuck in outer loop of build star!!!!!!!!\n");
          EGlite_free(vertex);
          EGlite_free(quads);
          return EGADS_RANGERR;
      }
  ...
  if (*star != NULL) EGlite_freeStar(&(*star));
  *star = (vStar *) EGlite_alloc(sizeof(vStar));
  if ((*star) == NULL) {
      EGlite_free(vertex);
      EGlite_free(quads);
      return EGADS_MALLOC;
  ...
  (*star)->nV    = v;
  (*star)->type  = -1;
  (*star)->verts = (int    *) EGlite_alloc(    v * sizeof(int));
  (*star)->quads = (int    *) EGlite_alloc(    q * sizeof(int));
  (*star)->idxV  = (int    *) EGlite_alloc(2 * v * sizeof(int));
  (*star)->idxQ  = (int    *) EGlite_alloc(2 * q * sizeof(int));
  (*star)->area  = (int    *) EGlite_alloc(    q * sizeof(int));
  (*star)->ratio = (double *) EGlite_alloc(    q * sizeof(double));
  (*star)->angle = (double *) EGlite_alloc(    q * sizeof(double));
...
__HOST_AND_DEVICE__ static int EGlite_setValence(meshMap *qm, int vID)
{
  ...
  si = vID - 1;
  i  = EGlite_buildStar(qm, &qm->star[si], vID);
  if (i != EGADS_SUCCESS || qm->star[si] == NULL) {
      printf(" In setValence for vertex %d stat from EGlite_buildStar = %d\n",
             vID, i);
      return i;
  }
  if (qm->valence[si][2] < qm->star[si]->nQ) {
      qm->valence[si] = (int *) EGlite_reall(qm->valence[si],
  ...
  n  =  qm->star[si]->nQ;
  iv = (int *) EGlite_alloc(3 * n * sizeof(int));
  if (iv == NULL) {
    printf(" EGlite_setValence iv pointer is NULL \n");
    return EGADS_MALLOC;
  }
...
__HOST_AND_DEVICE__ static int
EGlite_backupQuads(meshMap *qm, int *nq, int *qlist, Quad **quad)
{
  int   i, j, q, v, qcount, *qaux = NULL, k, add = 0;

  qaux      = (int *) EGlite_alloc(qm->totQ * sizeof(int));
  ...
  (*quad)      = (Quad *) EGlite_alloc(qcount * sizeof(Quad));
  if ((*quad) == NULL) {
      EGlite_free(qaux);
      return EGADS_MALLOC;
  }
...
__HOST_AND_DEVICE__ static int EGlite_restoreQuads(meshMap *qm, Quad *quad, int nq)
{
  int i, j, *vid = NULL, k, stat;

  vid      = (int *) EGlite_alloc(qm->totV * sizeof(int));
  ...
  for (i   = 0; i < k; i++) {
      stat = EGlite_setValence(qm, vid[i]);
      if (stat != EGADS_SUCCESS) {
          printf(" EGlite_restoreQuads failed at setting valence for %d \n",
                 vid[i]);
          EGlite_free(vid);
          return stat;
      }
...
EGlite_centroid(meshMap *qm, int n, int *list, double *uvOUT, int usequv)
{
  ...
  if (usequv == 1) {
      uv[0] = uvOUT[0];
      uv[1] = uvOUT[1];
      if (EGlite_evaluate(qm->face, uv, xyz0) != EGADS_SUCCESS ) return ;
      sumuv++; sumxyz++;
  } else {
      uv  [0] =  uv [1] = 0.0;
  ...
  for (i = 0; i < n; i++) {
      ...
      if (qm->vType [list[i]-1]  > 0 &&
          qm-> degen[list[i]-1] == 1) {
#ifdef DEBUG
              printf(" EGlite_centroid SKIP %d --> DEGENERATE\n",list[i]);
#endif
              continue;
      }
  ...
  uvOUT[0] = uv[0];
  uvOUT[1] = uv[1];
  i        = EGlite_invEvaluateGuess(qm->face, xyz0, uv, xyz1);
  if (i   != EGADS_SUCCESS  || uv[0] < qm->range[0] ||
      ...
      i       = EGlite_invEvaluate(qm->face, xyz0, uv, xyz1);
...
__HOST_AND_DEVICE__ static int
EGlite_angAtNPlane(meshMap *qm, int vC, int v1, int v2, double *normal, double *angle)
{
  ...
  *angle = 0.0;
  ...
  stat      = EGlite_evaluate(qm->face, &qm->uvs[2 * (vC - 1)], xyz);
  if (stat != EGADS_SUCCESS) {
      printf(" EGlite_angAtNPlane EGlite_evaluate %d --> %d!!\n", vC, stat);
      return stat;
  }
  ...
  if (norm1 < qEPS || tc == 1) {
    ...
    stat = EGlite_evaluate(qm->face, uv, xyz);
    if (stat != EGADS_SUCCESS) {
#ifdef DEBUG
    printf(" INSIDE NORM1 < EPS EGlite_angAtNPlane EGlite_evaluate %d --> %d!!\n", vC, stat);
#endif
      return stat;
    }
    ...
    norm1 = sqrt(norm1 * norm2);
    if (norm1 < qEPS) {
#ifdef DEBUG  // NOSE CASE HAS DEGENERATE QUAD EVEN AFTER EGlite_CENTROID. IT IS ENTERING HERE SOMETIMES
    // NEED TO FIX IT AT SOME POINT !!!!
    printf("EGlite_angleAtVnormalPlane VERT %d TYPE %d LINK TO %d %d "
...
__HOST_AND_DEVICE__ static int
EGlite_angAtBdVert(meshMap *qm, int v, int *links, double *size)
{
  int i, j, k;
  ...
  *size = 0.0;
  ...
  if (k >= 3) {
      *size = PI * 0.5; // boundary vertex is connected to more than two bounds (?)
      ...
      printf(" EGlite_angAtBdVert:: vertex %d is at surface bounds and connected only to another boundary vertex !!\n ", v);
      return EGADS_GEOMERR;
  } else {
    ...
    return EGlite_angAtNPlane(qm, v, links[0], links[1], normal, &(*size));
  }
}


__HOST_AND_DEVICE__ static int EGlite_nValenceCount(meshMap *qm, int q, int n)
{
  int i, count, val;

  if (q <= 0 || q > qm->totQ ) {
      printf("EGlite_nValenceCount for a wrong index!! %d != [%d,%d]\n", q, 1, qm->totQ);
      return -1;
  }
  for (count = i = 0; i < 4; i++) {
...
EGlite_quadSize(meshMap *qm, int qID, /*@null@*/ double *tris)
{
  int iA, iB, iC, iD;
...
__HOST_AND_DEVICE__ static int EGlite_vertexArea(meshMap *qm, int vID)
  {
   ...
   for (s = 0; s < qm->star[vID-1]->nQ; s++) {
     ...
     (void) EGlite_quadSize(qm, qm->star[vID-1]->quads[s], tria);
 #ifdef DEBUG2
     ...
     for (doublet = count = k = 0; k < 4; ++k) {
       ...
       if (qm->valence[qV[k]][2] * qm->vType[qV[k]] == -2) {
         ...
         if (EGlite_angAtNPlane(qm, qV[vA] + 1, qV[vB] + 1, qV[vD] + 1,
...
__HOST_AND_DEVICE__ static int
EGlite_placeVertex(meshMap *qm, int vID, int pass, int full)
{
  ...
  qInfo  = (int*) EGlite_alloc(2 * qm->star[v]->nQ * sizeof(int));
  vl     = (int*) EGlite_alloc(    qm->star[v]->nV * sizeof(int));
      ...
      for (q = 0; q <= nt; q++) {
          if (i0 == 1) {
              ...
              if (q == 0) {
                uv[2] = uv[0];
                uv[3] = uv[1];
                EGlite_centroid (qm, qm->star[v]->nQ, &qm->valence[v][3], &uv[2], (round+1)%2);
                if (la != -1 && ta[0] == QA0 && qm->bdAng[vla] > 3.9 &&
                  ...
                  if ((qm->vType[vl[0]] > 0 && qm->degen[vl[0]] == 1) ||
                      (qm->vType[vl[1]] > 0 && qm->degen[vl[1]] == 1)) {
                        vl[0]++; vl[1]++;
                        EGlite_centroid(qm, 2, vl, &uv[2], 0);
                   } else
                      EGlite_getSidepoint(qm->face, 0.5, &qm->uvs[2 * vl[0]],
                  ...
                  if ((qm->vType[vl[0]] > 0 && qm->degen[vl[0]] == 1) ||
                   ...
                   else EGlite_getSidepoint(qm->face, 0.5, &qm->uvs[2 * vl[0]],
                                       &qm->uvs[2 * vl[1]], NULL, NULL, &uv[2]);
              } else {
                  ...
                  if (i == 2) continue;
                  EGlite_centroid(qm, i, vl, &uv[2], 0);
#ifdef DEBUG2
          ...
          ta[i0]          = EGlite_vertexArea(qm, vID);
          angpen          = 0.0;
          res[3 * i0    ] = 1.0;
          res[3 * i0 + 1] = 1.0;
          res[3 * i0 + 2] = 1.0;
          for (j = 0 ;j < qm->star[v]->nQ; j++ ) {
              ...
              if (qm->star[v]->ratio[j] < 0.0)
                  ...
                  if (corner == j) {
                   ...
                   EGlite_angAtNPlane (qm, vla + 1, qm->star[v]->verts[la + 1], v + 1,
  ...
  printf (" LA = 2j+1 %d %d %d --> ANGOPT %lf ANG %lf\t",
            vla + 1, qm->star[v]->verts[la + 1], v + 1, angopt, auxd);
                      ...
                      if (EGlite_angAtNPlane(qm, vla + 1, v + 1, qm->star[v]->verts[v1],
                                         normal, &auxd) == EGADS_SUCCESS) {
                      errbd = fabs(angopt - auxd);
#ifdef DEBUG2
...
__HOST_AND_DEVICE__ int EGlite_createMeshMap(bodyQuad *bodydata)
{
  ...
  bodydata->qm = (meshMap **) EGlite_alloc(bodydata->nfaces * sizeof(meshMap *));
  if (bodydata->qm == NULL) {
    printf(" EGlite_createMeshMap Failed to allocate quad map pointer!\n");
    return  EGADS_MALLOC;
  }
  for (f = 0; f < bodydata->nfaces; ++f) {
      bodydata->qm[f] = (meshMap *) EGlite_alloc(sizeof(meshMap));
      if (bodydata->qm[f] == NULL) {
          printf("EGlite_createMeshMap face %d VOID ptr\n ", f + 1);
          continue;
      }
      bodydata->qm[f]->fID        = f + 1;
      bodydata->qm[f]->plotcount  = 0;
      /* Edges associated to face */
      stat = EGlite_getTessFace(bodydata->tess, f + 1, &len,
                            &xyzs, &uvs, &ptype, &pindex, &ntri,
                            &tris, &tric);
      if (stat != EGADS_SUCCESS) {
          printf("EGlite_createMeshMap :: EGlite_getTessFace %d = %d !!\n",f + 1, stat);
          bodydata->qm[f]->fID   = 0;
      }
      ...
      for (q = 0; q < ntri; q++) {
         ...
         if ( norm1 == 0. ) {
            printf(" \n EGlite_createMeshMap FACE %d TRI %d, zero area !!!\n",
            f +1, q + 1 );
            bodydata->qm[f]->fID       = 0;
         }
         uC[0] /= norm1;
         ...
         for (kk = 0; kk < 3; kk++) {
           ...
           if (DOT(uC, vC) < -EPS08 ) {
              printf(" \n EGlite_createMeshMap FACE %d TRI %d, %d %.12f wrong OR !!!\n",
      ...
      bodydata->qm[f]->xyzs    = (double *) EGlite_alloc(3*(2 * len  )*sizeof(double));
      bodydata->qm[f]->uvs     = (double *) EGlite_alloc(2*(2 * len  )*sizeof(double));
      bodydata->qm[f]->vType   = (int    *) EGlite_alloc(  (2 * len  )*sizeof(  int ));
      bodydata->qm[f]->qIdx    = (int    *) EGlite_alloc(4*(2 * nquad)*sizeof(  int ));
      bodydata->qm[f]->qAdj    = (int    *) EGlite_alloc(4*(2 * nquad)*sizeof(  int ));
      bodydata->qm[f]->remQ    = (int    *) EGlite_alloc(  (2 * nquad)*sizeof(  int ));
      bodydata->qm[f]->remV    = (int    *) EGlite_alloc(  (2 * len  )*sizeof(  int ));
      bodydata->qm[f]->valence = (int   **) EGlite_alloc(  (2 * len  )*sizeof(  int*));
      bodydata->qm[f]->star    = (vStar **) EGlite_alloc(  (2 * len  )*sizeof(vStar*));
      ...
      for (j = 0; j < 2 * len; j++) {
          ...
          bodydata->qm[f]->valence[j] = (int *) EGlite_alloc(3 * sizeof(int));
      ...
      stat = EGlite_getRange(bodydata->qm[f]->face, bodydata->qm[f]->range, &j);
      if (stat != EGADS_SUCCESS)
          printf(" EGlite_createMeshMap :: getRange = %d !! \n", stat);
      ...
      for (k = j = 0; j < len; j++) {
          ...
          if (uvs[2 * j    ] < bodydata->qm[f]->range[0] ||
              ...
              printf(" EGlite_createMeshMap :: vertex %d = %f  %f  is out of range !! \n ",
      ...
      bodydata->qm[f]->bdAng = (double *) EGlite_alloc(k * sizeof(double));
      bodydata->qm[f]->degen = (int    *) EGlite_alloc(k * sizeof(   int));
      ...
      for (j = 0; j < len; j++) {
          ...
          stat  = EGlite_evaluate(bodydata->qm[f]->face, &bodydata->qm[f]->uvs[2*j],
                              xyz);
          if (stat != EGADS_SUCCESS)
              printf(" EGlite_createMeshMap :: evaluate = %d !! \n", stat);
          norm1 = xyz[3] * xyz[3] + xyz[4] * xyz[4] + xyz[5] * xyz[5];
          norm2 = xyz[6] * xyz[6] + xyz[7] * xyz[7] + xyz[8] * xyz[8];
      ...
      for (j = 0; j < len; j++) {
          stat = EGlite_setValence (bodydata->qm[f], j + 1);
          if (stat != EGADS_SUCCESS) {
              printf("In EGlite_createMeshMap :: set valence at %d is %d!!\n ",
                     j + 1, stat);
              bodydata->qm[f]->fID = 0;
              break;
          }
      ...
      for (e4[0]= e4[1] = j = 0; j < len; j++) {
          if (bodydata->qm[f]->vType[j] == -1) break;
          stat     = EGlite_angAtBdVert(bodydata->qm[f], j + 1, e4, &angle);
          if (stat != EGADS_SUCCESS || angle < EPS08 ) {
#ifdef REPORT
              printf(" FACE %d EGlite_angAtBdVert %d angle %f\n ",f + 1, stat, angle);
...
__HOST_AND_DEVICE__ static int
EGlite_makeValidMesh(meshMap *qm, int nP, /*@null@*/ int *pList, int fullReg)
{
  ...
  mv      = (int   *)EGlite_alloc(qm->totV * sizeof(int));
  qlist   = (int   *)EGlite_alloc(qm->totQ * sizeof(int));
  ...
  if (fullReg == 0) { // move around only affected vertices
      ...
      itMax = 5;
      uvxyz = (double*)EGlite_alloc(5 * kv * sizeof(double));
      if ( uvxyz == NULL ) {
          EGlite_free(mv);
          EGlite_free(qlist);
          return EGADS_MALLOC;
  ...
  area = (int *)EGlite_alloc(kv * sizeof(int));
  if (area == NULL ) {
      EGlite_free(mv);
      EGlite_free(uvxyz);
      EGlite_free(qlist);
...
#ifdef DEBUG2
  printf(" EGlite_makeValid mesh for %d regular  ---- \n ", fullReg);
  ...
  if (fout != NULL ) {
      for (i = 0 ; i < kq; i++ ) {
          ...
          fprintf(fout,"\n\n");
          EGlite_evaluate(qm->face, uv, pos);
          fprintf(fout, "%lf %lf %lf %d\n", pos[0], pos[1], pos[2],
          qlist[i] );
          fprintf(fout,"\n\n");
      }
  ...
  for (it = 0 ; it < itMax; it++) {
      ...
      for (sum = k = 0 ; k < kv; k++) {
          ...
          area[k] = EGlite_placeVertex(qm, mv[k] + 1, pass, fr);
          sum     = MAX(sum,area[k]);
#ifdef DEBUG2
...
__HOST_AND_DEVICE__ static int
EGlite_createQuadGroup(meshMap *qm, quadGroup *qg, int q0, int q1)
{
  ...
  if (q0 < 0 || q1 < 0 || q0 > qm->totQ || q1 > qm->totQ ||
      ...
      qm->qIdx[4 * ( q1 - 1 )] == -2 ) {
#ifdef DEBUG
      printf(" EGlite_commonVerts wrong quad pair %d %d\n", q0, q1);
      if (q0 > 0)
        printQuad(qm, q0);
      if (q1 > 0)
        printQuad(qm, q1);
  ...
  qg->q[1] = q1;
  if (q0 == q1) {
      printf("EGlite_createQuadGroup :: Quads %d are the same!!!\n", q0);
      return EGADS_INDEXERR;
  }
  for (k = i = 0; i < 4; i++) {
      for (j = 0; j < 4; j++) {
  ...
  if (k < 2) {
      printf("EGlite_createQuadGroup: Quad %d %d are not adjacent!!\n", q0, q1);
#ifdef DEBUG
      printQuad(qm, q0);
      printQuad(qm, q1);
#endif
...
__HOST_AND_DEVICE__ static int EGlite_validCollapse(meshMap *qm, int qID, int v)
{
  ...
  for (j   = 0; j < 2; j++) {
      ...
      for (k  = 0; k < 3; k++) {
          ...
          if (EGlite_quadVertIdx(qm, qID,  aux + 1) >= 0) continue;
...
__HOST_AND_DEVICE__ static int EGlite_validSwap(meshMap *qm, int v1, int v2)
{
  int i, vs[2], type, val;
...
__HOST_AND_DEVICE__ static int
EGlite_swappingOperation(meshMap *qm, quadGroup qg, int swap, int *activity)
{
  ...
  *activity = 0;
  ...
  nq         = 2;
  stat       = EGlite_backupQuads(qm, &nq, qID, &quad);
  if (stat != EGADS_SUCCESS || quad == NULL) {
      EGlite_free(quad);
      printf(" In EGlite_swappingOperation :: EGlite_backupQuad %d !!\n ", stat);
      return stat;
  }
  i0 = EGlite_quadVertIdx(qm, qID[0], qg.verts[0]); // centre
  i1 = EGlite_quadVertIdx(qm, qID[1], qg.verts[3]); // opposite
  ...
  list = (int *) EGlite_alloc (qm->totV * sizeof(int));
  if (list == NULL) {
      EGlite_free(quad);
      return EGADS_MALLOC;
  }
  ...
  for (i = 0; i < i0; i++) {
      stat = EGlite_setValence(qm, list[i]);
      if (stat != EGADS_SUCCESS) {
#ifndef __CUDA_ARCH__
          fprintf(stderr," EGlite_swappingOperation :: ERROR SETTING VALENCE %d \n ",
                  list[i]);
#else
          printf(" EGlite_swappingOperation :: ERROR SETTING VALENCE %d \n ",
                 list[i]);
#endif
          EGlite_free(list);
          return stat;
  ...
  EGlite_free(list);
  *activity = 1;
  i     = EGlite_vertexArea(qm,qg.verts[0]);
  j     = EGlite_vertexArea(qm,qg.verts[3]);
  ...
  if (i != QACB && j != QACB) {
      if (i != QA0 || j != QA0) {
          if (qm->vType[qg.verts[0]-1] == -1 &&
              ...
              EGlite_centroid(qm, qm->valence[qg.verts[0] -1][2],
          ...
          if (qm->vType[qg.verts[3]-1] == -1 &&
              ...
              EGlite_centroid(qm, qm->valence[qg.verts[3] -1][2],
                          &qm->valence[qg.verts[3] -1][3],
                          &qm->uvs[2 * (qg.verts[3] - 1)], 1);
              updateVertex(qm, qg.verts[3], &qm->uvs[2 * (qg.verts[3] - 1)] );
          }
          i0 = EGlite_vertexArea(qm, qg.verts[0]);
          i1 = EGlite_vertexArea(qm, qg.verts[3]);
          if (i0 + i1 > i + j) {
      ...
      if (EGlite_makeValidMesh(qm, 6, qg.verts, 0) == EGADS_SUCCESS) {
          EGlite_free(quad);
          return EGADS_SUCCESS;
      }
  } else {
#ifdef DEBUG
    printf(" EGlite_swappingOperation : reject swap. It crosses the bounds %d  %d  \n ",
           i, j);
    gnuData(qm, NULL);
#endif
  }
#ifdef DEBUG
  printf(" EGlite_swapping operation didn't work well: restore quads\n ");
#endif
  if(qm->vType[qg.verts[0]-1])
    updateVertex(qm, qg.verts[0], uv);
  if(qm->vType[qg.verts[3]-1])
    updateVertex(qm, qg.verts[3], &uv[2]);
  stat = EGlite_restoreQuads(qm, quad, nq);
  EGlite_free(quad);
  if (stat != EGADS_SUCCESS) {
      printf(" EGlite_swappingOperation :: restore quads %d !!\n ", stat);
      return stat;
  }
  *activity = 0;
  return EGADS_SUCCESS;
...
__HOST_AND_DEVICE__ static int
EGlite_splittingOperation(meshMap *qm, int vC, int vL, int vR, int *activity)
{
  ...
  *activity   = 0;
  ...
  nq        = qm->star[si]->nQ;
  stat      = EGlite_backupQuads(qm, &nq, qm->star[si]->quads, &quad);
  if (stat != EGADS_SUCCESS || quad == NULL) {
      EGlite_free(quad);
      printf("In EGlite_splittingOperation :: EGlite_backupQuads %d !!\n", stat);
      return stat;
  }
  if (qm->remQ[0] > 0) {
...
/*    dist = 4;
  ...
  if (id0 == -1 || id1 == -1) {
#ifdef DEBUG
    printf("EGlite_splittingOperation  Problem finding vertices %d !!\n ",
           EGADS_INDEXERR);
    printf(" vL %d vR %d \n ", vL, vR);
  ...
  for (i = 0; i < 4; ++i) {
      ...
      (void) EGlite_adjQtoPair(qm, modQ[i], poly[0], verts[i], adj);
  ...
  for (i = 0 ; i < n; i++) {
      ...
      if (stat != EGADS_SUCCESS) {
#ifndef __CUDA_ARCH__
          fprintf(stderr," EGlite_splittingOperation :: ERROR SETTING VALENCE %d \n ",
                  poly[i]);
#else
          printf(" EGlite_splittingOperation :: ERROR SETTING VALENCE %d \n ",
                 poly[i]);
#endif
          EGlite_free(quad);
          EGlite_free(list);
  ...
  if (qm->star[si] == NULL) {
#ifndef __CUDA_ARCH__
      fprintf(stderr," EGlite_splittingOperation star for new vertex %d is NULL\n",
              poly[3]);
#else
      printf(" EGlite_splittingOperation star for new vertex %d is NULL\n",
             poly[3]);
#endif
      return EGADS_MALLOC;
  }
  EGlite_centroid(qm, qm->star[si]->nQ, &qm->valence[si][3], uv, 0);
  updateVertex(qm, poly[3], uv);
  if (qm->vType[poly[0]-1] == -1) {
      EGlite_centroid(qm, qm->star[poly[0]-1]->nQ, &qm->valence[poly[0]-1][3], uv, 0);
...
#endif
  id1 = 0;
  id0 = EGlite_placeVertex(qm, poly[3], 1, 0);
  if (qm->vType[poly[0]-1] == -1)
    id1 = EGlite_placeVertex(qm, poly[0], 1, 0);
  if (id0 >= QA2) {
      if (id0 >= QA3 ) id0 = 1;
      else {
  ...
  if (id0 == 0 && id1 == 0) {
    ...
    gnuData(qm, NULL);
#endif
    if (EGlite_makeValidMesh(qm, 4, poly, 0) == EGADS_SUCCESS) {
      EGlite_free(quad);
      return EGADS_SUCCESS;
    }
  }
...
#ifdef DEBUG
  printf("EGlite_splittingOperation mesh valid failed: restore quads\n");
  gnuData(qm, NULL);
#endif
  stat = EGlite_restoreQuads(qm, quad, nq);
  EGlite_free(quad);
  if (stat != EGADS_SUCCESS) {
      printf(" EGlite_splittingOperation :: EGlite_restoreQuads %d !!\n ", stat);
      return stat;
  }
  if (addedV == 1) {
      --qm->totV;
...
__HOST_AND_DEVICE__ static int
EGlite_mergeVertices(meshMap *qm, int qC, int centre, int *activity)
{
  ...
  nq        = 1;
  stat      = EGlite_backupQuads(qm, &nq, &qC, &quad);
  if (stat != EGADS_SUCCESS || quad == NULL) {
      printf(" In EGlite_mergeVertices stat in EGlite_backpQuads %d !!\n ", stat);
      return stat;
  }
  EGlite_centroid(qm, 4, &qm->qIdx[4 * (qC - 1)], uv, 0);
  *activity  = 0;
  i          = EGlite_quadVertIdx(qm, qC, centre);
  if ( qm->vType[centre - 1] != -1 ) {
      i = (i+ 2)%4;
      centre = qm->qIdx[4 * (qC - 1) + i];
  }
#ifdef DEBUG
  printf(" EGlite_mergeVertices collapsing %d through %d = %d \n ",
         qC, centre, i);
  gnuData(qm, NULL);
#endif

  if (qm->vType[centre - 1] != -1 || i < 0) {
      printf(" EGlite_mergeVertices collapse through %d is type %d and id in quad %d->%d !!\n",
...
/*    centre = qm->qIdx[4 * (qC - 1) + i];    */
  ...
  for (i = 0; i < 4; i++) {
      ...
      stat = EGlite_adjQtoPair(qm, q, oldQ[i], oldQ[(i + 1)%4], adjPair);
      if (stat != EGADS_SUCCESS || adjPair[1] != qC) {
          printf("EGlite_mergeVertices stat %d  adj %d to pair %d %d is %d \n ",
  ...
  for (i = 1; i < 4; i++) {
      ...
      for (q = 0; q < 4; q++) {
          auxQ = oldQ[4 + q];
          if (auxQ < 0) continue;
          if (EGlite_quadVertIdx(qm, auxQ, oldQ[i]) >= 0) {
              qm->valence[oldQ[i] - 1][0] = auxQ;
              break;
          }
      }
      if (qm->valence[oldQ[i] - 1][0] == qC) {
          printf(" EGlite_mergeVertices opposite vertex still points at collapsed quad %d !!\n",
                 EGADS_INDEXERR);
          EGlite_free(list);
          return EGADS_INDEXERR;
      }
  }
  EGlite_setValence(qm, oldQ[2]);
  EGlite_setValence(qm, oldQ[1]);
  EGlite_setValence(qm, oldQ[3]);
  list = (int *) EGlite_alloc(qm->totV * sizeof(int));
  if (list == NULL) {
      EGlite_free(quad);
      return EGADS_MALLOC;
  ...
  for ( i = 0 ; i < n; i++ ) {
      stat = EGlite_setValence(qm, list[i]);
      if (stat != EGADS_SUCCESS) {
          printf(" EGlite_mergeVertices error setting valence %d \n ", list[i]);
          EGlite_free(quad);
          EGlite_free(list);
          return stat;
      }
  ...
  if (qm->vType[oldQ[2] - 1] == -1 && doublet == 0 &&
      ...
      EGlite_placeVertex(qm, oldQ[2], 1,  0);
  }
  ...
  *activity  = 1;
  ...
  if (doublet == 1) {
      ...
      if (qm->vType[oldQ[1] -1] == -1)
        EGlite_placeVertex(qm, oldQ[1],1, 0);
      if (qm->vType[oldQ[3] -1] == -1)
        EGlite_placeVertex(qm, oldQ[3], 1, 0);
      return EGADS_SUCCESS;
  }
  if (EGlite_makeValidMesh(qm, 3, &oldQ[1], 0) == EGADS_SUCCESS) {
      EGlite_free(quad);
      return EGADS_SUCCESS;
  }
#ifdef DEBUG
  printf("EGlite_mergeVertices collapse quad %d thru vertex %d -->%d !!\n",
         qC, oldQ[0], stat);
#endif
  qm->vType[oldQ[0] - 1]  = -1;
  stat = EGlite_restoreQuads(qm, quad, nq);
  EGlite_free(quad);
  if (stat != EGADS_SUCCESS) {
      printf("EGlite_mergeVertices :: EGlite_restoreQuads stat %d \n", stat);
      return stat;
  }
  i = -1;
  if (qm->remQ[0] > 0)
...
__HOST_AND_DEVICE__ static int EGlite_swap(meshMap *qm, int qIn, int *activity)
{
  ...
  *activity  = 0;
...
#ifdef DEBUG
      printf(" EGlite_swap for quad %d is void \n ", qIn );
#endif
      return EGADS_SUCCESS;
  }
  if (qm->regBd == 1 && EGlite_regBoundQuad(qm, qIn) == 0) bq1 = 1;
  for (swap = q = 0; q < 4; q++) {
      qg.q[0]        = qIn;
      qg.q[1]        = qm->qAdj[4 * (qIn - 1) + q];
      if (qg.q[1]   == -1) continue;
      if (qm->regBd ==  1 && EGlite_regBoundQuad(qm, qg.q[1]) == 0) bq2 = 1;
      stat           = EGlite_createQuadGroup(qm, &qg, qg.q[0], qg.q[1]);
      if (stat      != EGADS_SUCCESS) {
#ifdef DEBUG
          printf(" EGlite_swap->EGlite_createQuadGroup  %d !! \n", stat);
      ...
      if ( EGlite_validSwap(qm, qg.verts[0], qg.verts[3]) != 1) continue;
      corner = 0;
      if ((bq1 == 1 || bq2 == 1) &&
  ...
  return EGlite_swappingOperation(qm, qg, swap, &(*activity));
}


__HOST_AND_DEVICE__ static int
EGlite_doubleSwap(meshMap *qm, quadGroup qg, int forcing, int *activity)
{
  int piv5 = -1, piv3 = -1, q5, i, adjPiv5, stat, adj[2], swap = 0, vopp3;

  *activity = 0;
  if      (EGlite_validSwap(qm, qg.verts[0], qg.verts[3]) == 0) return EGADS_SUCCESS;
  ...
  q5   = 0;
  if (vopp3 > 3) q5 = 1;
  stat = EGlite_adjQtoPair(qm, qg.q[q5], qg.verts[vopp3], qg.verts[piv5], adj);
#ifdef DEBUG
  ...
  if (stat != EGADS_SUCCESS || adj[1] == -1 ||
      (qm->regBd == 1 && EGlite_regBoundQuad(qm, adj[1]) == 1)) {
#ifdef DEBUG
    if (stat != EGADS_SUCCESS)
      printf(" EGlite_doubleSwap adjToPair %d -->%d !!\n", adj[1], stat);
#endif
    return stat;
  }
  i       = EGlite_quadVertIdx(qm, adj[1], qg.verts[vopp3]);
  ...
  piv5      = qg.verts[0];
  stat      = EGlite_swappingOperation(qm, qg, piv3, &(*activity));
  if (stat != EGADS_SUCCESS || *activity == 0) {
#ifdef DEBUG
    printf(" EGlite_doubleSwap: at first swap activity %d, stat -->%d !!\n ",
           *activity, stat);
#endif
    return stat;
  }
  stat = EGlite_swap(qm, adj[1], &i);
  if (i == 0 && stat == EGADS_SUCCESS) {
    (void) EGlite_createQuadGroup(qm, &qg, qg.q[0], qg.q[1]);
    for (swap = 0; swap < 6; swap++) if (qg.verts[swap] == piv5) break;
    stat = EGlite_swappingOperation(qm, qg, swap, &i);
    if (stat == EGADS_SUCCESS && i == 1) *activity = 0;
    else {
#ifdef DEBUG
      printf("In EGlite_doubleSwap undoing first swap activity %d stat %d \n!!",
             i, stat);
#endif
      return stat;
    }
...
__HOST_AND_DEVICE__ static int
EGlite_collapse(meshMap *qm, int qID, int *activity, int forcing, int cID)
{
  ...
  *activity = 0;
  if (qID <=0 || qID > qm -> totQ || qm->qIdx[4 * (qID - 1)] == -2 ||
      (qm->regBd == 1 && EGlite_regBoundQuad (qm, qID) == 1)) {
#ifdef DEBUG
      printf(" EGlite_collapse for quad %d is empty or touches bounds\n ", qID);
#endif
      return EGADS_SUCCESS;
  }
  ...
  for (v3[0] = v5[0] = i = 0; i < 4; i++) {
      ...
      if      (val[0] == 3) v3[++v3[0]] = i;
          ...
          stat = EGlite_mergeVertices(qm, qID, qb[i], &(*activity));
          if (stat != EGADS_SUCCESS || *activity == 0) {
              printf("EGlite_collapse :: I can't fail whilst removing doublets !!!!!!!! \n ");
#ifdef DEBUG
              gnuData(qm, NULL);
              printQuad (qm, qID );
#endif
  ...
  if (forcing == 0 && *activity == 0) {
      ...
      if (v5[0] == 1 || (v5[0] == 2 && abs(v5[1] - v5[2])%2 == 0)) {
          vC   = qb[ (v5[1] + 1)%4];
          if (EGlite_validCollapse(qm, qID, vC) == 1) {
              stat      = EGlite_mergeVertices(qm, qID, vC, &(*activity));
              if (stat != EGADS_SUCCESS) {
                  printf(" EGlite_collapse :: EGlite_mergeVertices %d activity %d \n",
                         stat, *activity);
                  return stat;
              }
          }
  ...
  if (*activity == 0 && forcing > 0) {
      if (cID > 0) {
          if(EGlite_validCollapse(qm, qID, cID) == 1)
            stat = EGlite_mergeVertices(qm, qID, cID, &(*activity));
      } else {
          if (EGlite_nValenceCount(qm, qID, 3) == 0 && forcing == 1) return EGADS_SUCCESS;
          i = 0; i3 = 0;
          while (i < 2) {
              ...
              if ( qm->vType  [vC - 1]    == -1 && ( forcing == 3 ||
                  (qm->valence[vO - 1][2] <  10 && qm->valence[vC - 1][2] ==  3)) )
                {
                  ...
                  if ((qm->valence[links[0] - 1][1] >= 5 ||
                      ...
                      EGlite_validCollapse(qm, qID, vC) == 1) {
                      stat      = EGlite_mergeVertices(qm, qID, vC, &(*activity));
                      if (stat != EGADS_SUCCESS) {
                          printf("EGlite_collapse :: forcing EGlite_mergeVertices quad %d !! \n",
                                 stat);
#ifdef DEBUG
                          printQuad(qm, qID);
#endif
  ...
  for (i = 0; i < 4; i++) {
      ...
      for (i3  = 0; i3 < 3; i3++) {
          ...
          stat = EGlite_collapse(qm, qm->valence[links[i3]][0], &act, 0, 0);
          if (stat != EGADS_SUCCESS || act == 0) {
              printf(" EGlite_collapse removing doublet in doublet loop : V %d stat %d act %d !!!!\n ",
                     links[i3] + 1, stat, act );
              return EGADS_GEOMERR;
          }
      }
...
__HOST_AND_DEVICE__ static int
EGlite_swapCollapse(meshMap *qm, quadGroup qg, int forcing, int *activity)
{
  ...
  *activity = 0;
  ...
  //    forcing = 0;
    ...
    //  forcing = 0;
      ...
      if (i3 == 1) {
          q5   = 0;
          stat = EGlite_adjQtoPair(qm, qg.q[0], qg.verts[2], qg.verts[3], adj);
      }
      else {
          q5   = 1;
          stat = EGlite_adjQtoPair(qm, qg.q[1], qg.verts[4], qg.verts[3], adj);
      }
      if (stat != EGADS_SUCCESS || adj[1] == -1) {
          if (stat != EGADS_SUCCESS)
            printf("EGlite_swapCollapse centre 4-5 adj to pair %d !!\n", stat);
          return stat;
      }
      stat = EGlite_createQuadGroup(qm, &qg, qg.q[q5], adj[1]);
      if (stat != EGADS_SUCCESS) {
          printf("Inside EGlite_swapCollapse :: EGlite_createGroup %d !!\n ", stat);
          return stat;
      }
      return EGlite_swapCollapse(qm, qg, forcing, &(*activity));
  }
  else if (forcing == 1) {
      if (qg.vals[1] * qg.vals[5] == 25 || qg.vals[2] * qg.vals[4] == 25) {
          ...
          if (qm->regBd == 0 || (
              ...
              stat      = EGlite_swappingOperation(qm, qg, 1, &(*activity));
              if (stat != EGADS_SUCCESS || *activity == 0) {
#ifdef DEBUG
              ...
              qC = qg.q[0];
              if (EGlite_nValenceCount(qm, qC, 3) < 2) qC = qg.q[1];
              stat = EGlite_collapse(qm, qC, &(*activity), 1, 0);
              if (stat  != EGADS_SUCCESS || *activity == 0) {
                  (void) EGlite_createQuadGroup(qm, &qg, qg.q[0], qg.q[1]);
  ...
  qC    = qg.q[(q5 + 1)%2];
  if (EGlite_validCollapse(qm, qC, qg.verts[i3]) == 0) return EGADS_SUCCESS;
  v03   = (vOpp3 + 1)%6;
  if (v03%3 != 0) v03 = (vOpp3 + 5)%6;
  if (qg.vals[vOpp3] == 3) return EGADS_SUCCESS;
  stat = EGlite_adjQtoPair(qm, qg.q[q5], qg.verts[vOpp3], qg.verts[v03], adj);
  ...
  if (stat != EGADS_SUCCESS) {
#ifdef DEBUG
      printf("EGlite_swapCollapse before swap: EGlite_createQuadGroup -->%d !!\n",
             stat);
      printQuadGroup(qm, qg);
#endif
      return stat;
  ...
  v0        = qg.verts[0];
  if (qm->regBd == 1 && (
      EGlite_regBoundQuad(qm, qC)             == 1 ||
      qm->vType[qg.verts[ swap     ] -1] != -1 ||
      qm->vType[qg.verts[(swap+3)%6] -1] != -1)) return EGADS_SUCCESS;
#ifdef DEBUG
  printf(" SWAP IS VALID -- SWAP THRU %d \n ", swap);
#endif
  stat      = EGlite_swappingOperation(qm, qg, swap, &(*activity));
  if (stat != EGADS_SUCCESS || *activity == 0) {
#ifdef DEBUG
      printf("EGlite_swapCollapse after swapping %d activity %d !!\n",
             swap, *activity);
#endif
      return stat;
  }
  ...
  if (stat != EGADS_SUCCESS || *activity == 0) {
#ifdef DEBUG
      printf("EGlite_swapCollapse after collapsing %d activity %d  !!\n",
             stat, *activity);
#endif
      (void) EGlite_createQuadGroup(qm, &qg, qg.q[0], qg.q[1]);
      for (swap = 0; swap < 6; swap++) if (qg.verts[swap] == v0) break;
...
__HOST_AND_DEVICE__ static int
EGlite_doubleCollapse(meshMap *qm, quadGroup qg, int forcing, int *activity)
{
  int i, stat, i3, i5, q3;

  *activity  = 0;
  if (qm->regBd == 1 && (
      EGlite_regBoundQuad(qm, qg.q[0]) == 1 ||
      EGlite_regBoundQuad(qm, qg.q[1]) == 1 )) return EGADS_SUCCESS;
  ...
  for (i = 0; i < 2; i++) {
      ...
      stat        = EGlite_collapse(qm, qg.q[(q3 + i)%2], &i3, 1, 0);
      if (stat   == EGADS_SUCCESS) {
          if (i3 == 0) break;
          (*activity)++;
      }
      if (stat != EGADS_SUCCESS) {
          printf(" EGlite_doubleCollapse after EGlite_forceColapse quad %d->%d !!\n ",
                 qg.q[(q3 + i)%2], stat);
          return stat;
      }
  }
...
__HOST_AND_DEVICE__ static int
EGlite_swapDoubleCollapse(meshMap *qm, quadGroup qg, int *activity)
{
  ...
  *activity = 0;
  if (qg.vals[0] * qg.vals[3] != 20 ||
      ...
      EGlite_validSwap(qm, qg.verts[0], qg.verts[3]) == 0) return EGADS_SUCCESS;
  ...
  if (stat != EGADS_SUCCESS || *activity == 0) {
#ifdef DEBUG
      printf(" In EGlite_swapDoubleCollapse: Swapping went %d activity %d \n",
             stat, *activity);
#endif
      return stat;
  }
  *activity = 1;
  for (k = 0; k < 2; k++) {
      id = EGlite_quadVertIdx(qm, qg.q[k], qg.verts[3]);
      if (id >= 0) break;
  }
  if (id < 0) {
...
#endif
  stat      = EGlite_collapse(qm, qg.q[k], &(*activity), 1, 0);
  if (stat == EGADS_SUCCESS && *activity > 0) return stat;
  if (*activity == 0 ) {
      j = qg.q[k] - 1;
#ifdef DEBUG
  ...
  stat      = EGlite_createQuadGroup(qm, &qg, qg.q[0], qg.q[1]);
  if (stat != EGADS_SUCCESS) {
      printf("EGlite_swapDoubleCollapse after invalid force collapse :: EGlite_createQuadGroup is %d \n ",
             stat);
#ifdef DEBUG
      printQuadGroup(qm, qg);
#endif
  ...
  for (swap = 0; swap < 6; swap++) if (qg.verts[swap] == v0) break;
  stat = EGlite_swappingOperation(qm, qg, swap, &j);
...
__HOST_AND_DEVICE__ static int EGlite_split(meshMap *qm, int qID, int *activity)
{
...
#ifdef DEBUG
      printf(" EGlite_split for quad %d is -2\n ", qID );
#endif
      return EGADS_SUCCESS;
  }
  for (v = 0; v < 4; v++) {
      ...
      si = poly[0] - 1;
      if (qm->star[si] == NULL ) {
          printf("EGlite_split star at vertex %d is NULL !!\n ", si + 1);
          return EGADS_MALLOC;
      }
      id6[0]  = -1;
      id6[1]  = -1;
    ...
    if (validSplit == 1)  {
      stat = EGlite_splittingOperation(qm, qm->star[si]->verts[0], qm->star[si]->verts[id0],
...
__HOST_AND_DEVICE__ static int
EGlite_doubleSplit(meshMap *qm, quadGroup qg, int forcing, int *activity)
{
  ...
  *activity = 0;
  ...
  stat      = EGlite_splittingOperation(qm, qg.verts[0], qg.verts[piv[0]],
                                    qg.verts[piv[1]], &(*activity));
  if (stat != EGADS_SUCCESS || *activity == 0) {
#ifdef DEBUG
    printf(" EGlite_doubleSplit: force 1st split through %d - %d -->%d activity %d !!\n ",
           qg.verts[0], qg.verts[piv[0]], stat, *activity);
#endif
    return stat;
  }
  for (j    = 0; j < 2; j++)
    if (EGlite_quadVertIdx(qm, qg.q[j], qg.verts[piv[1]]) >= 0) break;
  stat      = EGlite_split(qm, qg.q[j], &i);
  if (stat == EGADS_SUCCESS && i == 0) {
    j     = qm->valence[qg.verts[0] - 1][0];
    if ( forcing == 0) {
      stat  = EGlite_collapse(qm, j, &i, 1, 0);
      if (stat == EGADS_SUCCESS && i == 1) *activity = 0;
    }
    return stat;
  }
...
__HOST_AND_DEVICE__ static int
EGlite_swapDoubleSplit(meshMap *qm, quadGroup qg, int *activity)
{
  ...
  *activity = 0;
  ...
  q = 0;
  if (EGlite_quadVertIdx(qm, qg.q[q], qg.verts[i5]) < 0) q = 1;
  stat = EGlite_adjQtoPair(qm, qg.q[q], qg.verts[i5], qg.verts[i55], adj);
  if (stat != EGADS_SUCCESS || adj[1] == -1) {
      if (stat != EGADS_SUCCESS)
        printf("In EGlite_swapDoubleSplit: adjacent to pair %d -->!!\n ", stat);
      return stat;
  }
  ...
  q0[1]  = qg.q[1];
  val3   = qg.verts[i3];
  stat   = EGlite_createQuadGroup(qm, &qg, qg.q[q], adj[1]);
  if (stat != EGADS_SUCCESS) {
      printf("Inside EGlite_swapDoubleSplit: before swapping EGlite_createQuadGroup stat %d\n ",
             stat);
#ifdef DEBUG
      printQuadGroup(qm, qg);
#endif
  ...
  stat          = EGlite_swappingOperation(qm, qg, i0, &(*activity));
  if (stat     != EGADS_SUCCESS || *activity == 0) {
      if (stat != EGADS_SUCCESS)
        printf(" EGlite_swapDoubleSplit error at swap: %d activity %d!!\n ",
               stat, *activity);
      return stat;
  }
  i = 0;
  if (EGlite_quadVertIdx(qm, q0[0], val3) < 0) i = 1;
  stat = EGlite_adjQtoPair(qm, q0[i], v30[0], v30[1], adj);
  if (stat != EGADS_SUCCESS || adj[1] == -1) {
      if (stat != EGADS_SUCCESS)
        printf(" EGlite_swapDoubleSplit after swapping adjacent to pair %d !!\n ",
               stat);
      return stat;
  }
  stat = EGlite_createQuadGroup(qm, &sqg, q0[i], adj[1]);
  if (stat != EGADS_SUCCESS) {
      printf("Inside EGlite_swapDoubleSplit: before splitting EGlite_createQuadGroup stat %d\n ",
             stat);
#ifdef DEBUG
      printQuadGroup(qm, qg);
#endif
  ...
  stat   = EGlite_doubleSplit(qm, sqg, 0, &i);
  if (i == 0 && stat == EGADS_SUCCESS) {
      stat      = EGlite_createQuadGroup(qm, &qg, qg.q[0], qg.q[1]);
      if (stat != EGADS_SUCCESS) {
          printf(" In EGlite_swapDoubleSplit: recover swap create group %d\n ",
                 stat);
#ifdef DEBUG
          printQuadGroup(qm, qg);
#endif
      ...
      stat = EGlite_swappingOperation(qm, qg, i, &q);
...
__HOST_AND_DEVICE__ static int
EGlite_swapSplit(meshMap *qm, quadGroup qg, int forcing, int *activity)
{
  ...
  *activity = 0;
  ...
  if (v3opp == -1) {
      for (i = 0; i < 2; i++) {
          ...
          stat = EGlite_adjQtoPair(qm, qg.q[i], qg.verts[j],
                               qg.verts[vL5], adj);
          if (stat != EGADS_SUCCESS || adj[1] == -1) continue;
          q5 = EGlite_quadVertIdx(qm, adj[1], qg.verts[j]);
      ...
      q5 = 0;
      if (EGlite_quadVertIdx(qm, qg.q[q5], qg.verts[vL5]) < 0) q5 = 1;
      stat = EGlite_adjQtoPair(qm, qg.q[q5], qg.verts[v3opp],
                           qg.verts[vL5], adj);
      if (stat     != EGADS_SUCCESS || adj[1] == -1) {
          if (stat != EGADS_SUCCESS)
            printf("EGlite_swapSplit: EGlite_adjQtoPair from quad %d is %d\n!!",
                   qg.q[q5], stat);
          return stat;
      }
      i           = EGlite_quadVertIdx(qm, adj[1], qg.verts[v3opp]);
      vL5adj      = qm->qIdx[4 * (adj[1] - 1) + (i + 1)%4];
      if (vL5adj == qg.verts[vL5])
  ...
  if (stat    != EGADS_SUCCESS || *activity == 0) {
#ifdef DEBUG
      printf(" In swapSplit thru %d : EGlite_swappingOperation went %d activity %d !!\n ",
             swap, stat, *activity);
      printQuadGroup(qm, qg);
#endif
      return stat;
  ...
  if (qm->regBd ==  0 ||
      (qm->vType[qg.verts[3] -1] == -1 &&
          qm->vType[adj[0] -     1] == -1 ))
    stat     = EGlite_splittingOperation(qm, qg.verts[i5], qg.verts[3], adj[0], &i);
  if (stat    != EGADS_SUCCESS || i == 0) {
#ifdef DEBUG
...
__HOST_AND_DEVICE__ static int
EGlite_basicOperation(meshMap *qm, int qID, int type, int *activity)
{
  int stat = EGADS_SUCCESS;
  switch (type) {
    case SWAP:
      stat = EGlite_swap(qm, qID, &(*activity));
      break;
    case COLLAPSE:
      stat = EGlite_collapse(qm, qID, &(*activity), 0, 0);
      break;
    case SPLIT:
      stat = EGlite_split(qm, qID, &(*activity));
      break;
  }
...
__HOST_AND_DEVICE__ static int
EGlite_composeOperation(meshMap *qm, quadGroup qg, int type, int forcing,
                    int *activity)
{
  int stat = EGADS_SUCCESS;
  switch (type) {
    case DOUBLESWAP:
      stat = EGlite_doubleSwap(qm, qg, forcing, &(*activity));
      break;
    case SWAPCOLLAPSE:
      stat = EGlite_swapCollapse(qm, qg,forcing, &(*activity));
      break;
    case DOUBLECOLLAPSE:
      stat = EGlite_doubleCollapse(qm, qg, forcing,  &(*activity));
      break;
    case SWAPDOUBLECOLLAPSE:
      stat = EGlite_swapDoubleCollapse(qm, qg, &(*activity));
      break;
    case SWAPSPLIT:
      stat = EGlite_swapSplit(qm, qg, forcing, &(*activity));
      break;
    case DOUBLESPLIT:
      stat = EGlite_doubleSplit(qm, qg, forcing, &(*activity));
      break;
    case SWAPDOUBLESPLIT:
      stat = EGlite_swapDoubleSplit(qm, qg, &(*activity));
      break;
  }
  return stat;
}
...
__HOST_AND_DEVICE__ static int
EGlite_cleanQuad(meshMap *qm, int qID, int useAdj, int transfer, int forcing,
             int *activity)
{
  ...
  *activity = 0;
  if (qm->qIdx[4 * (qID - 1)] == -2) {
#ifdef DEBUG
    printf(" EGlite_cleanQuad %d is empty\n", qID);
#endif
    return EGADS_SUCCESS;
  }
  ...
  if (transfer <= 0) {
      ...
      for (i = 0; i < 3; i++) {
          stat       = EGlite_basicOperation(qm, qID, opBasic[i], &act);
          *activity += act;
          if (stat != EGADS_SUCCESS) {
              printf(" EGlite_cleanQuad Basic operation %d !!\n ", stat);
              return stat;
          }
  ...
  for (i = 0; i < 7; i++) {
      ...
      for (q   = 0; q < 4; q++) {
          ...
          if (qadj == -1 ) continue;
          stat = EGlite_createQuadGroup(qm, &qg, qID, qadj);
          if (stat != EGADS_SUCCESS) {
              printf("Inside EGlite_cleanQuad: EGlite_createQuadGroup stat -->%d !!\n ",
                     stat);
#ifdef DEBUG
              printQuadGroup(qm, qg);
#endif
          ...
          stat       = EGlite_composeOperation(qm, qg, opComp[i], aux, &act);
          *activity += act;
          if (stat != EGADS_SUCCESS) {
              printf("In EGlite_cleanQuads restor mesh after EGlite_composeOperation stat -->%d !!\n ",
                     stat);
              return stat;
          }
          if (act > 0 || qID < 0 || qm->qIdx[4 * (qID - 1)] == -2) return EGADS_SUCCESS;
  ...
  if (forcing == 1 && EGlite_nValenceCount(qm, qID, 3) > 1) {
#ifdef DEBUG
      printf("EGlite_cleanQuad FORCE COLLAPSE AROUND QUAD %d \n ", qID);
#endif

      stat = EGlite_collapse(qm, qID, &(*activity), 1, 0);
      if (stat != EGADS_SUCCESS || *activity > 0) {
          if (stat != EGADS_SUCCESS)
            printf("EGlite_cleanQuad force Collapse went %d!1\n ", stat);
          return stat;
      }
  }
...
__HOST_AND_DEVICE__ static int
EGlite_transferValences(meshMap *qm, int *qID, int try5533, int *transfering,
                    int *activity)
{
  ...
  *activity = 0;
...
#ifdef DEBUG
      printf(" EGlite_transferValences for quad %d is out of bounds or quad is regular!!\n ",
             qID[0]);
#endif
      return EGADS_SUCCESS;
  }
  bdq = EGlite_regBoundQuad(qm, qID[0]);
  if (*transfering == 0) {
      stat          = EGlite_cleanQuad(qm, qID[0], 1, 0, try5533, &(*activity));
      if (stat     != EGADS_SUCCESS || *activity > 0) {
          qID[0]    = -1;
          if (stat != EGADS_SUCCESS)
              printf("EGlite_transferValences: EGlite_cleanQuad %d -->%d!!\n",
              qID[0], stat);
          return stat;
      }
  }
  ...
  for (q   = 0; q < 4; q++) {
      ...
      if (adj == -1 || adj == qID[1] ||
         (qm->regBd == 1 && bdq == -1 &&  EGlite_regBoundQuad(qm, adj) ==  1 )) {
          if (*transfering == 0 && qv35 == 1 ) {
              EGlite_collapse(qm, qID[0], &(*activity), 1, 0);
              if (stat     != EGADS_SUCCESS || *activity > 0) {
                  qID[0]    = -1;
                  if (stat != EGADS_SUCCESS)
                      printf("EGlite_transferValences: EGlite_cleanQuad %d -->%d!!\n",
      ...
      if (*transfering == 0) {
          stat      = EGlite_cleanQuad(qm, adj, 1, 0, try5533, &(*activity));
          if (stat != EGADS_SUCCESS || *activity > 0) {
              if (stat != EGADS_SUCCESS)
                  printf(" EGlite_TransferValence EGlite_cleanQuad -->%d !!\n ", stat);
              qID[0] = -1;
              return stat;
          }
      }
      if (qm->qAdj[4 * (qID[0] - 1) + q] != adj) continue;
      stat = EGlite_createQuadGroup(qm, &qg, qID[0], adj);
      if (stat != EGADS_SUCCESS) {
          printf(" Inside EGlite_transferValences EGlite_createQuadGroup %d !!\n", stat);
#ifdef DEBUG
          printQuadGroup(qm, qg);
#endif
          return stat;
      ...
      if (*transfering == 0 && qg.vals[0] * qg.vals[3] == 15 && qID[1] != -1) {
          ...
          for (i = 0 ; i < 4; i++) {
              ...
              if (qm->vType[qm->valence[j][3] - 1] > 0) {
                  stat      = EGlite_collapse(qm, qID[0], &(*activity), 1, 0);
                  if (stat != EGADS_SUCCESS)
                    printf("EGlite_transferValences: forceCollapse gave %d !!\n",
                           stat);
                  qID[0]    = qID[1];
                  qID[1]    = -1;
                  return stat;
          ...
          return EGlite_transferValences(qm, qID, 0, &(*transfering), &(*activity));
      }
      ...
      if (EGlite_validSwap(qm, qg.verts[0], qg.verts[3]) == 0 || swap == 0) continue;
      stat          = EGlite_swappingOperation(qm, qg, swap, &(*activity));
      if (stat     != EGADS_SUCCESS || *activity == 0) {
          if (stat != EGADS_SUCCESS)
#ifdef DEBUG
          printf(" EGlite_transferValences EGlite_swappingOperation stat %d activity %d !!\n",
                 stat, *activity);
#endif
          return stat;
      }
      ...
      if (qID[1] > 0 && qm->qIdx[4 * (qID[1] - 1)] != -1 &&
          qg.q[0] == qID[1]) i = 1;
      else if (EGlite_nValenceCount(qm,qg.q[0], 5) == 2 &&
               EGlite_nValenceCount(qm,qg.q[1], 5) >= 1 &&
               EGlite_nValenceCount(qm,qg.q[1], 3) == 1) i = 1;
      else if (EGlite_nValenceCount(qm,qg.q[1], 4) <
               EGlite_nValenceCount(qm,qg.q[0], 4)) i = 1;
      qID[0] = qg.q[ i       ];
      qID[1] = qg.q[(i + 1)%2];
#ifdef DEBUG
      ...
      if (*transfering == 1) {
          for (j = 0; j < 2; j++) {
              ...
              qAux[0]   = qID[ j       ];
              qAux[1]   = qID[(j + 1)%2];
              stat      = EGlite_transferValences(qm, qAux, 0, &i, &min);
              if (stat != EGADS_SUCCESS) {
                  printf(" EGlite_transferValences: separating valences after forcing %d!!\n",
                  stat);
                  return stat;
              }
          }
...
__HOST_AND_DEVICE__ void EGlite_destroyMeshMap(bodyQuad *bodydata)
{
  int i, j;
      ...
      if (bodydata->qm[i]) {
          for (j = 0; j < bodydata->qm[i]->sizeV; ++j) {
              if (bodydata->qm[i]->valence)
                EGlite_free(bodydata->qm[i]->valence[j]);
              if (bodydata->qm[i]->star)
                EGlite_freeStar(&bodydata->qm[i]->star[j]);
          }
          EGlite_free(bodydata->qm[i]->valence);
          EGlite_free(bodydata->qm[i]->qIdx);
          EGlite_free(bodydata->qm[i]->qAdj);
          EGlite_free(bodydata->qm[i]->uvs);
          EGlite_free(bodydata->qm[i]->xyzs);
          EGlite_free(bodydata->qm[i]->remQ);
          EGlite_free(bodydata->qm[i]->remV);
          EGlite_free(bodydata->qm[i]->vType);
          EGlite_free(bodydata->qm[i]->star);
          EGlite_free(bodydata->qm[i]->vInv);
          EGlite_free(bodydata->qm[i]->bdAng);
          EGlite_free(bodydata->qm[i]->degen);
          EGlite_free(bodydata->qm[i]);
      }
  }
  EGlite_free(bodydata->qm);
  bodydata->qm = NULL;
}
...
__HOST_AND_DEVICE__ static int resizeQm(meshMap *qm)
{
  ...
  vpiv = (int *) EGlite_alloc(qm->totV * sizeof(int));
  qpiv = (int *) EGlite_alloc(qm->totQ * sizeof(int));
  ...
  bV    = qm->totV;
  bQ    = qm->totQ;
  uvxyz = (double *) EGlite_alloc(5 * bV *sizeof(double));
  type  = (int    *) EGlite_alloc(    bV *sizeof(int));
  verts = (int    *) EGlite_alloc(4 * bQ *sizeof(int));
  adjs  = (int    *) EGlite_alloc(4 * bQ *sizeof(int));
...
__HOST_AND_DEVICE__ static int
EGlite_cleanNeighborhood(meshMap *qm, int qID,  int transfer, int *activity)
{
  ...
  *activity = 0;
  ...
  stat      = EGlite_cleanQuad(qm, qID, 1, transfer, 0, &act);
  if (stat != EGADS_SUCCESS) return stat;
  *activity += act;
  for (i = 0; i < 4; i++) {
      ...
      si = v[i] - 1;
      if ( qm->star[si] == NULL ) {
          printf(" EGlite_cleanNeighborhood star at %d is NULL !!\n", v[i]);
          return EGADS_MALLOC;
      }
      for (j = 0; j < qm->star[si]->nQ; j++) {
          if (qm->star[si]->quads[j] == -1) continue;
          if (EGlite_cleanQuad(qm, qm->star[si]->quads[j], 1, transfer, 0, &act) != EGADS_SUCCESS) {
              return stat;
          }
          count += act;
      }
...
__HOST_AND_DEVICE__ static void meshProperties(meshMap *qm, int sweep)
{
  ...
  for (k = 0 ; k < qm->totQ; k++) {
      ...
      for (n = 0; n <= Nx; n++) {
          ...
          if (EGlite_evaluate(qm->face, uv3, p) != EGADS_SUCCESS) continue;
...
__HOST_AND_DEVICE__ int EGlite_meshRegularization(meshMap *qm)
{
...
#endif
  skipQuad = (int *)    EGlite_alloc (qm->totQ * sizeof(int   ));
  qArea    = (double *) EGlite_alloc (qm->totQ * sizeof(double));
  ...
  for (n0 = sq = loopact = it = 0 ; it < ITMAX; it++) {
      ...
      for (j = i = 0; i < qm->totQ; i++) {
          qArea[i] = 0.0;
          if (qm->qIdx[ 4 * i ] == -2) continue;
          qArea[i] = EGlite_quadSize(qm, i + 1, NULL);
          if (it == 0) {
...
break;
      ...
      if (n0 == 0) {
          ...
          for (ni = i = 0; i < qm->totV; i++) {
              ...
              if (EGlite_vertexArea(qm, i + 1) > QA2) ni++;
          }
          if (ni > 0 ) {
              //#ifdef REPORT
              printf("\n\n Attention! face %d has initally %d / %d INVALID vertex positions \n",
                     qm->fID, ni, qm->totV);
              //#endif
              qm->vInv = (int *) EGlite_alloc ((ni + 1) * sizeof(int));
              if (qm->vInv == NULL) {
                  EGlite_free (skipQuad);
                  EGlite_free (qArea   );
                  qm->fID = 0;
                  return EGADS_MALLOC;
              }
      ...
      if (qm->vInv != NULL && qm->vInv[0] > 0) {
          for (j = 0; j < qm->vInv[0]; j++) {
              ...
              for (i = 0; i < qm->star[k]->nQ; i++) {
                  ...
                  stat  = EGlite_collapse(qm, qm->star[k]->quads[i], &activity, 3, 0);
                  if (stat != EGADS_SUCCESS) {
                      printf(" EGlite_meshRegularization :: preprocess EGlite_collapse for quad %d -->%d!!\n ",
                             qm->vInv[1 + j], stat);
                      EGlite_free (skipQuad);
                      EGlite_free (qArea   );
                      EGlite_free(qm->vInv);
      ...
      for (k = i = 0; i < qm->totQ; i++) {
          ...
          if (qArea[i] <= avArea ) {
              stat  = EGlite_collapse(qm, i+1, &activity, 2, 0);
              if (stat != EGADS_SUCCESS) {
                  printf(" EGlite_meshRegularization :: preprocess EGlite_collapse for quad %d -->%d!!\n ",
                         k, stat);
              }
  ...
  stat = resizeQm(qm);
  if (stat != EGADS_SUCCESS) {
      printf(" EGlite_meshRegularization resizing after preprocess %d !!\n", stat);
      qm->fID = 0;
      return stat;
  }
  ITMAX = 100;
  for (it = 0; it < ITMAX; it++) { //deactivae regular bound:: bring it back during transfer
      ...
      for (q = 0; q < qm->totQ; q++) {
          ...
          stat = EGlite_cleanNeighborhood(qm, q + 1, 0, &activity);
          if (stat != EGADS_SUCCESS) {
              printf(" In EGlite_cleanMesh: EGlite_CleanNeighborhood for quad %d -->%d!!\n ",
                     q + 1, stat);
              qm->fID = 0;
              return stat;
          }
          ...
          if (it == -1) {
              stat = resizeQm(qm);
              if (stat != EGADS_SUCCESS)
                printf(" In EGlite_cleanMesh: resizeQm = %d\n", stat);
      ...
      for (k  = 0; k <= 1; k++) {
          ...
          if (qm->regBd == 1) {
              for (q = 0; q < qm->totQ; q++) {
                  if (qm->qIdx[4 * q] == -2) continue; //can be a deleted quad
                  j = i = 0;
                  if (EGlite_regBoundQuad(qm, q + 1) != 0) {
                      for (i = 0; i < 4; i++) {
                      ...
                      if (j == 1) {
                        i = qm->qIdx[4 * q + i];
                        if (EGlite_quadSize(qm, q + 1, NULL) <= 2.0 * qm->minArea) j = 1;
                        else j = 0;
                      }
                  } else {
                      j = -1;
                      stat = EGlite_swap(qm, q + 1, &activity);
                      if ( stat != EGADS_SUCCESS || activity > 0) {
#ifdef DEBUG
                  ...
                  if (j == 1 && EGlite_validCollapse(qm, q + 1, i) == 1) {
#ifdef DEBUG
                      printf(" FORCE COLLAPSE: EITHER TOO SMALL OR IRREGULARBOUNDARY \n");
#endif
                      stat         = EGlite_collapse(qm, q + 1, &activity, 1, i);
                      totActivity += activity;
                      if (stat != EGADS_SUCCESS) {
                          printf(" EGlite_meshRegularization :: preprocess EGlite_collapse for quad %d -->%d!!\n ",
                                 q + 1, stat);
          ...
          for ( it = 0 ; it < ITMAX; it++) {
              ...
              for (q = 0; q < qm->totQ; q++) {
                  ...
                  qPair[0]  = q + 1;
                  qPair[1]  =    -1;
                  stat      = EGlite_transferValences(qm, qPair, k, &transfer, &activity);
                  if (stat     != EGADS_SUCCESS) {
                      printf("EGlite_meshRegularization:: EGlite_transferValences %d\n", stat);
                      qm->fID = 0;
                      return stat;
                  }
                  totActivity += activity;
                  ...
                  for (j = 0; j < 20; j++) {
                      ...
                      for (i  = 0; i < 2; i++) {
                          ...
                          stat = EGlite_cleanNeighborhood(qm, qPair[i], transfer, &loopact);
                          if (stat != EGADS_SUCCESS) {
                              printf("EGlite_meshRegularization:: EGlite_cleanNeighborhood in transferValences %d\n", stat);
                              qm->fID = 0;
                              return stat;
                          }
                          activity +=loopact;
                      ...
                      if (activity > 0) break;
                      stat      = EGlite_transferValences(qm, qPair, k,
                                                      &transfer, &activity);
                      if (stat != EGADS_SUCCESS) {
                          printf("EGlite_meshRegularization:: EGlite_transferValences after j loop %d\n", stat);
                          qm->fID = 0;
                          return stat;
                      }
                      if (activity == 0 || qPair[0] < 0 ||
              ...
              for (q = 0; q < qm->totQ; q++) {
                  ...
                  stat = EGlite_cleanQuad(qm, q + 1, 1, 0, 0, &i);
                  if (stat != EGADS_SUCCESS) {
                      printf(" In EGlite_meshRegularization clean quad %d !!\n ", stat);
                      qm->fID = 0;
                      return stat;
                  }
                  totActivity += i;
  ...
  for (q = 0; q < qm->totQ; q++) {
      ...
      if ((qm->vType[qm->qIdx[4 * q    ]-1] * qm->valence[qm->qIdx[4 * q    ]-1][2] == -3 &&
          ...
          (qm->vType[qm->qIdx[4 * q + 1]-1] * qm->valence[qm->qIdx[4 * q + 1]-1][2] == -3 &&
           ...
           stat = EGlite_collapse (qm, q + 1, &i, 1, -1);
      else stat = EGlite_cleanNeighborhood(qm, q + 1, 0, &activity);
      if (stat    != EGADS_SUCCESS) {
          printf(" In EGlite_cleanMesh: EGlite_CleanNeighborhood for quad %d -->%d!!\n ",
                 q + 1, stat);
          qm->fID = 0;
  ...
  for (q = 0; q < qm->totQ; q++) {
      if (qm->qIdx[4 * q] == -2) continue; //can be a deleted quad
      stat         = EGlite_cleanQuad(qm, q + 1, 0, 0, 0, &activity);
      if (stat    != EGADS_SUCCESS) {
          printf(" In EGlite_cleanMesh: EGlite_CleanNeighborhood for quad %d -->%d!!\n ",
                 q + 1, stat);
          qm->fID = 0;
  ...
  stat      = resizeQm(qm);
  if (stat != EGADS_SUCCESS) {
      printf("EGlite_meshRegularization final resize %d !!\n", stat);
      qm->fID = 0;
      return stat;
  }
#ifdef REPORT
  ...
  printf("*******************************************************************\n");
#endif
  stat = EGlite_makeValidMesh(qm, 0, NULL, 1);
#ifdef REPORT
  printf("__________________________IO FILES ______________________________\n");
...
__HOST_AND_DEVICE__ int EGlite_makeQuadTess(bodyQuad bodydata, ego *quadTess)
{
  ...
  *quadTess = NULL;
  ...
  stat = EGlite_initTessBody(btess->src, &newTess);
  if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_initTessBody = %d (EGlite_makeQuadTess)!\n", stat);
      return stat;
  }
  ntess = (egTessel *) newTess->blind;
  if (btess->src->oclass == EBODY) {
    stat  = EGlite_getBodyTopos(btess->src, NULL, EEDGE, &nedges, &edges);
  } else {
    stat  = EGlite_getBodyTopos(btess->src, NULL,  EDGE, &nedges, &edges);
  }
  if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_getBodyTopos E = %d (EGlite_makeQuadTess)!\n",
               stat);
      EGlite_deleteObject(newTess);
      return stat;
  }
  ...
  for (i = 0; i < nedges; i++) {
      ...
      stat = EGlite_getTessEdge(tess, i+1, &npts, &xyzs, &ts);
      if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Error: EGlite_getTessEdge %d = %d (EGlite_makeQuadTess)!\n",
                   i+1, stat);
          EGlite_free(edges);
          EGlite_deleteObject(newTess);
          return stat;
      ...
      if (npts == 0) {
          if (outLevel > 0)
            printf(" EGADS Error: EGlite_getTessEdge %d -- no points (EGlite_makeQuadTess)!\n",
                   i+1);
          EGlite_free(edges);
          EGlite_deleteObject(newTess);
          return EGADS_INDEXERR;
      ...
      stat = EGlite_setTessEdge(newTess, i+1, npts, xyzs, ts);
      if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Error: EGlite_setTessEdge %d = %d (EGlite_makeQuadTess)!\n",
                   i+1, stat);
          EGlite_free(edges);
          EGlite_deleteObject(newTess);
          return stat;
  ...
  for (mQ = i = 0; i < bodydata.nfaces; i++) {
      ...
      stat = EGlite_getTessFace(tess, i+1, &npts, &xyzs, &uvs, &ptype, &pindex,
                            &nt, &trs, &trc);
      if ((stat != EGADS_SUCCESS) || (nt == 0)) {
          if (outLevel > 0)
            if (stat != EGADS_SUCCESS) {
                printf(" EGADS Error: EGlite_getTessFace %d = %d (EGlite_quadTess)!\n",
                       i+1, stat);
            } else {
                printf(" EGADS Error: Face %d has no tessellation (EGlite_quadTess)!\n",
                       i+1);
            }
          EGlite_deleteObject(newTess);
          return stat;
      }
      stat = EGlite_setTessFace(newTess, i+1, npts, xyzs, uvs, nt, trs);
      if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            EGlite_deleteObject(newTess);
          return stat;
  ...
  printf(" Max Quads = %d\n", mQ);
#endif
  tris = (int *) EGlite_alloc(6*mQ*sizeof(int));
  if (tris == NULL) {
      if (outLevel > 0)
        printf(" EGADS Error: Quad Allocation = %d (EGlite_makeQuadTess)!\n", mQ);
      EGlite_deleteObject(newTess);
      return EGADS_MALLOC;
  }
  ...
  for (i = 0; i < bodydata.nfaces; i++) {
      ...
      stat = EGlite_setTessFace(newTess, i+1, bodydata.qm[i]->totV,
                            bodydata.qm[i]->xyzs,
                            bodydata.qm[i]->uvs,
                            bodydata.qm[i]->totQ*2, tris);
      if (stat != EGADS_SUCCESS) {
          if (outLevel > 0)
            printf(" EGADS Error: EGlite_setTessFace %d = %d (EGlite_makeQuadTess)!\n",
                   i+1, stat);
          EGlite_free(tris);
          EGlite_deleteObject(newTess);
          return stat;
  ...
  stat = EGlite_statusTessBody(newTess, &obj, &i, &npts);
  if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_statusTessBody = %d (EGlite_makeQuadTess)!\n", stat);
      EGlite_deleteObject(newTess);
      return stat;
  }
  if (i != 1) {
      if (outLevel > 0)
        printf(" EGADS Error: New Tessellation Object is Open (EGlite_makeQuadTess)!\n");
      EGlite_deleteObject(newTess);
      return EGADS_TESSTATE;
  }

#ifndef LITE
  iq = (int *) EGlite_alloc(bodydata.nfaces*sizeof(int));
  if (iq != NULL) {
    for (i = 0; i < bodydata.nfaces; i++) iq[i] = ntess->tess2d[i].ntris/2;
    stat = EGlite_attributeAdd(newTess, ".mixed", ATTRINT, bodydata.nfaces,
                           iq, NULL, NULL);
    if (stat != EGADS_SUCCESS)
      printf(" EGADS Warning: EGlite_attributeAdd M = %d (EGlite_makeQuadTess)!\n",
             stat);
    EGlite_free(iq);
  }
  stat = EGlite_attributeAdd(newTess, ".tessType", ATTRSTRING, 4,
                         NULL, NULL, "Quad");
  if (stat != EGADS_SUCCESS)
    if (outLevel > 0)
      printf(" EGADS Warning: EGlite_attributeAdd Q = %d (EGlite_makeQuadTess)!\n",
             stat);
#endif
...
int main (int argc, char *argv[])
{
  ...
  /* initialize */
  stat = EGlite_open(&context);
  if (stat != EGADS_SUCCESS) return 1;
  stat = EGlite_loadModel(context, 0, argv[1], &model);
  ...
  for (iBody = 0; iBody < nbody; iBody++) {
    ...
    /*stat = EGlite_attributeAdd(bodies[iBody], ".qParams",
                           ATTRSTRING, 4, NULL, NULL, "off");
    if (stat != EGADS_SUCCESS)
        printf(" Body %d: attributeAdd = %d\n", iBody, stat);*/
      EGlite_getTopology(bodies[iBody], &geom, &oclass,
      ...
      stat = EGlite_getBodyTopos(bodies[iBody], NULL, FACE,
                             &bodydata[iBody].nfaces, &bodydata[iBody].faces);
      i    = EGlite_getBodyTopos(bodies[iBody], NULL, EDGE,
      ...
      stat = EGlite_makeTessBody(bodies[iBody], params, &bodydata[iBody].tess);
      if (stat != EGADS_SUCCESS) {
          printf(" EGlite_makeTessBody %d = %d\n", iBody, stat);
          continue;
      }
      tess = bodydata[iBody].tess;
      /* disable regularization in EGADS */
      stat = EGlite_attributeAdd(tess, ".qRegular", ATTRSTRING, 3, NULL, NULL, "Off");
      if (stat != EGADS_SUCCESS)
        printf(" EGlite_attributeAdd qRegular %d = %d\n", iBody, stat);
      stat = EGlite_quadTess(tess, &bodydata[iBody].tess);
      if (stat != EGADS_SUCCESS) {
          printf(" EGlite_quadTess %d = %d  -- reverting...\n", iBody, stat);
  ...
  for (iBody = 0; iBody < nbody; iBody++) {
      stat = EGlite_attributeRet(bodydata[iBody].tess, ".tessType", &atype,
      ...
      if (EGlite_createMeshMap(&bodydata[iBody]) == EGADS_SUCCESS) {
        for (f = 0; f < bodydata[iBody].nfaces; ++f) {
            if (FACECHOICE >= 0) f = FACECHOICE - 1;
            stat      = EGlite_meshRegularization(bodydata[iBody].qm[f]);
            if (stat != EGADS_SUCCESS) {
#ifndef __CUDA_ARCH__
                fprintf(stderr, " EGlite_meshRegularization face %d / %d = %d DEACTIVATE !!!\n ",
                        f + 1, bodydata[iBody].nfaces,  stat);
#else
                printf(" EGlite_meshRegularization face %d / %d = %d DEACTIVATE !!!\n ",
                        f + 1, bodydata[iBody].nfaces,  stat);
#endif
      ...
      if (FACECHOICE == -1) {
          stat = EGlite_makeQuadTess(bodydata[iBody], &newTess);
          if (stat != EGADS_SUCCESS) {
#ifndef __CUDA_ARCH__
              fprintf(stderr, " EGlite_makeQuadTess = %d!\n ", stat);
#else
              printf(" EGlite_makeQuadTess = %d!\n ", stat);
#endif
              goto cleanup;
          }
          stat = EGlite_deleteObject(newTess);
          if (stat != EGADS_SUCCESS) {
#ifndef __CUDA_ARCH__
              fprintf(stderr, " EGlite_deleteObject = %d!\n ", stat);
#else
  ...
  cleanup:
  if (stat != EGADS_SUCCESS ) printf(" EGlite_main stat in cleanup %d !!\n", stat);
  for (iBody = 0; iBody < nbody; iBody++) {
      EGlite_destroyMeshMap(&bodydata[iBody]);
      EGlite_free(bodydata[iBody].faces);
      EGlite_deleteObject(bodydata[iBody].tess);
  }
  EGlite_free(bodydata);
#ifdef REPORT
  end_t   = clock();
