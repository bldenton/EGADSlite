...
#define PI               3.14159265358979324
  ...
  /*** internal function not in egads_lite.h ***/
  extern int EGlite_outLevel(const egObject *object);


static int
EGlite_retessEdge(egObject *tess, int eIndex, egObject *edge, double *params)
{
  int    i, j, k, stat, npts;
  ...
  if (params[0] > 0.0)
    for (;;) {
      stat = EGlite_getTessEdge(tess, eIndex, &npts, &xyz, &t);
      ...
      newts = (double *) EGlite_alloc(j*sizeof(double));
      if (newts == NULL) return EGADS_MALLOC;
      for (k = 0; k < j; k++)
        newts[k] = t[i] + (k+1.0)*(t[i+1]-t[i])/(j+1.0);
      stat = EGlite_insertEdgeVerts(tess, eIndex, i+1, j, newts);
      EGlite_free(newts);
      if (stat != EGADS_SUCCESS) return stat;
    }
  ...
  if (params[1] > 0.0)
    for (;;) {
      ...
      d    = 0.5*(t[i]+t[i+1]);
      stat = EGlite_insertEdgeVerts(tess, eIndex, i+1, 1, &d);
      if (stat != EGADS_SUCCESS) return stat;
    }
  ...
  if (params[2] > 0.0) {
    ...
    for (;;) {
      ...
      i       = 0;
      for (j = 0; j < npts-1; j++) {
        stat = EGlite_evaluate(edge, &t[j+1], result);
        if (stat != EGADS_SUCCESS) return stat;
        d = sqrt(result[3]*result[3] + result[4]*result[4] +
                 result[5]*result[5]);
        if (d == 0.0) d = 1.0;
...
int
EGlite_retessFaces(egObject *tess, int nf, int *iface, double *params)
{
  ...
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_getBodyTopos(F) = %d (EGlite_retessFaces)!\n",
             stat);
    return stat;
  }
  ...
  for (i = 0; i < nf; i++) {
    if ((iface[i] < 1) || (iface[i] > nface)) {
      if (outLevel > 0)
        printf(" EGADS Error: iface %d = %d [1-%d] (EGlite_retessFaces)!\n",
               i+1, iface[i], nface);
      EGlite_free(faces);
    ...
    for (j = 0; j < i; j++)
      if (iface[i] == iface[j]) {
        if (outLevel > 0)
          printf(" EGADS Error: iface %d == iface %d (EGlite_retessFaces)!\n",
                 i+1, j+1);
        EGlite_free(faces);
        return EGADS_INDEXERR;
      }
  ...
  if (stat != EGADS_SUCCESS) {
    if (outLevel > 0)
      printf(" EGADS Error: EGlite_getBodyTopos(E) = %d (EGlite_retessFaces)!\n",
             stat);
    EGlite_free(faces);
    return stat;
  }
  ...
  iedge = (int *) EGlite_alloc(nedge*sizeof(int));
  if (iedge == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Allocation on %d indices (EGlite_retessFaces)!\n",
             nedge);
    EGlite_free(edges);
    EGlite_free(faces);
    return EGADS_MALLOC;
  ...
  for (i = 0; i < nedge; i++) {
    iedge[i] = 0;
    stat = EGlite_getInfo(edges[i], &oclass, &mtype, &topRef, &prev, &next);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_getInfo Edge %d = %d (EGlite_retessFaces)!\n",
               i+1, stat);
      EGlite_free(iedge);
      EGlite_free(edges);
      EGlite_free(faces);
  ...
  for (i = 0; i < nf; i++) {
    stat = EGlite_getBodyTopos(object, faces[iface[i]-1], EDGE, &nef, &edgf);
    if (stat != EGADS_SUCCESS) {
      if (outLevel > 0)
        printf(" EGADS Error: EGlite_getBodyTopos(EF) %d = %d (EGlite_retessFaces)!\n",
               iface[i], stat);
      EGlite_free(iedge);
      EGlite_free(edges);
      EGlite_free(faces);
    ...
    for (j = 0; j < nef; j++) {
      k = EGlite_indexBodyTopo(object, edgf[j]);
      if (k <= EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: EGlite_indexBodyTopo %d/%d=%d (EGlite_retessFaces)!\n",
                 i+1, j+1, stat);
        EGlite_free(iedge);
        EGlite_free(edges);
        EGlite_free(faces);
  ...
  facedg = (egObject **) EGlite_alloc((nf+nedge)*sizeof(egObject *));
  if (facedg == NULL) {
    if (outLevel > 0)
      printf(" EGADS Error: Allocation on %d objects (EGlite_retessFaces)!\n",
             nf);
    EGlite_free(iedge);
    EGlite_free(edges);
    EGlite_free(faces);
  ...
  for (k = 0; k < nedge; k++)
    if (iedge[k] == 1) {
      ...
      stat = EGlite_retessEdge(tess, k+1, edges[k], params);
      if (stat != EGADS_SUCCESS) {
        if (outLevel > 0)
          printf(" EGADS Error: EGlite_retessEdge %d=%d (EGlite_retessFaces)!\n",
                 k+1, stat);
        EGlite_free(facedg);
        EGlite_free(iedge);
        EGlite_free(edges);
      ...
      for (i = 0; i < nf; i++) {
        EGlite_getBodyTopos(object, faces[iface[i]-1], EDGE, &nef, &edgf);
        for (j = 0; j < nef; j++) {
          stat = EGlite_indexBodyTopo(object, edgf[j]);
  ...
  stat = EGlite_remakeTess(tess, k, facedg, params);
  EGlite_free(facedg);
  return stat;
}
