...
PETSC_EXTERN PetscErrorCode DMPlexGenerate_Tetgen(DM boundary, PetscBool interpolate, DM *dm)
{
  ...
  ::tetgenio     out;
  DMLabel        label;
  PetscInt       vStart, vEnd, v, fStart, fEnd, f, numLabels;
  PetscMPIInt    rank;
  PetscErrorCode ierr;
  ...
  ierr = PetscPrintf(PETSC_COMM_SELF, "    numLabels = %d \n", numLabels); CHKERRQ(ierr);
  ...
  if (in.numberofpoints > 0) {
    ...
    in.pointmarkerlist = new int[in.numberofpoints * numLabels];
    ...
    for (v = vStart; v < vEnd; ++v) {
      ...
      for (PetscInt ii = 0; ii < numLabels; ++ii){
        //ierr = PetscPrintf(PETSC_COMM_SELF, "  ii = %d \n", ii); CHKERRQ(ierr);
        ierr = DMGetLabelName(boundary, ii, &labelName); CHKERRQ(ierr);
        //ierr = PetscPrintf(PETSC_COMM_SELF, "  Current Label = %s \n", labelName); CHKERRQ(ierr);
        ierr = DMGetLabel(boundary, labelName, &label); CHKERRQ(ierr);
        if (label) {
          PetscInt val;
  
          ierr = DMLabelGetValue(label, v, &val);CHKERRQ(ierr);
          //ierr = PetscPrintf(PETSC_COMM_SELF, "  Curent Label Value = %d \n", val); CHKERRQ(ierr);
          //ierr = PetscPrintf(PETSC_COMM_SELF, "  idx + in.numberofpoints*ii = %d \n", idx + in.numberofpoints*ii); CHKERRQ(ierr);
          in.pointmarkerlist[idx + in.numberofpoints*ii] = (int) val;
          ierr = PetscPrintf(PETSC_COMM_SELF, "  in.pointmarkerlist[%d] = %d \n", idx + in.numberofpoints*ii, in.pointmarkerlist[idx + in.numberofpoints*ii] ); CHKERRQ(ierr);
        }
      }
    }
  ...
  if (in.numberoffacets > 0) {
    ...
    in.facetmarkerlist = new int[in.numberoffacets*numLabels];
    ...
    for (f = fStart; f < fEnd; ++f) {
      ...
      for (PetscInt ii = 0; ii < numLabels; ++ii){
        ...
        if (label) {
          ...
          ierr = DMLabelGetValue(label, f, &val);CHKERRQ(ierr);
          in.facetmarkerlist[idx + in.numberofpoints*ii] = (int) val;
        } 
      }
  ...
  }
  {
    ...
    for (PetscInt ii = 0; ii < numLabels; ++ii){
      ...
      ierr = DMGetLabel(boundary, labelName, &label); CHKERRQ(ierr); 
      if (label) {ierr = DMCreateLabel(*dm, labelName); ierr = DMGetLabel(*dm, labelName, &glabel);}
      ierr = PetscPrintf(PETSC_COMM_SELF, "  Curent Label = %s \n", labelName); CHKERRQ(ierr);
      /* Set labels */
      for (v = 0; v < numVertices; ++v) {
        if (out.pointmarkerlist[v + out.numberofpoints*ii]) {
          ierr = PetscPrintf(PETSC_COMM_SELF, "  Curent Label Value = %d \n", out.pointmarkerlist[v + out.numberofpoints*ii]); CHKERRQ(ierr);
          ierr = PetscPrintf(PETSC_COMM_SELF, "  out.pointmarkerlist[%d] = %d \n", v + out.numberofpoints*ii, out.pointmarkerlist[v + out.numberofpoints*ii] ); CHKERRQ(ierr);
          if (glabel) {ierr = DMLabelSetValue(glabel, v+numCells, out.pointmarkerlist[v + out.numberofpoints*ii]);CHKERRQ(ierr);}
        }
      }  
    }
    ...
    if (interpolate) {
      ...
      for (f = 0; f < out.numberoftrifaces; f++) {
        if (out.trifacemarkerlist[f]) {
          ...
          ierr = PetscPrintf(PETSC_COMM_SELF, "  Here \n"); CHKERRQ(ierr);
          ...
          for (PetscInt ii = 0; ii < numLabels; ++ii){
            ...
            if (label) {ierr = DMCreateLabel(*dm, labelName); ierr = DMGetLabel(*dm, labelName, &glabel);}
            /* Set labels */
            if (glabel) {ierr = DMLabelSetValue(glabel, faces[0], out.trifacemarkerlist[f + out.numberofpoints*ii]); CHKERRQ(ierr);}
          }
