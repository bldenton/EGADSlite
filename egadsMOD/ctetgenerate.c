...
PETSC_EXTERN PetscErrorCode DMPlexGenerate_CTetgen(DM boundary, PetscBool interpolate, DM *dm)
{
  ...
  //ierr = DMGetLabel(boundary, labelName, &label);CHKERRQ(ierr);        // Moved to other locations to allow handling of other labels besides "marker"
  ...
  if (in->numberofpoints > 0) {
    ...
    //ierr = PetscMalloc1(in->numberofpoints,       &in->pointmarkerlist);CHKERRQ(ierr);    // Replaced with line below for multiple labels
    ierr = PetscMalloc1(in->numberofpoints*numLabels,  &in->pointmarkerlist);CHKERRQ(ierr);
    ...
    for (v = vStart; v < vEnd; ++v) {
      ...
      //if (label) {ierr = DMLabelGetValue(label, v, &m);CHKERRQ(ierr);}  // Original Line of Code. Replace by for loop below 
      //
      //in->pointmarkerlist[idx] = (int) m;        // Original Line of Coad. Replaced as part of the for loop below
      ...
      for (PetscInt ii = 0; ii < numLabels; ++ii){
        ...
        in->pointmarkerlist[idx + (in->numberofpoints*ii)] = (int) m;      // Not Sure this is right!!!!!!
      }
    }
  ...
  if (in->numberoffacets > 0) {
    ...
    ierr = PetscMalloc1(in->numberoffacets*numLabels,   &in->facetmarkerlist);CHKERRQ(ierr);
    for (f = fStart; f < fEnd; ++f) {
      ...
      //if (label) {ierr = DMLabelGetValue(label, f, &m);CHKERRQ(ierr);}      // Original Code Replaced by for loop below
      ...
      for (PetscInt ii = 0; ii < numLabels; ++ii){
        ...
        in->facetmarkerlist[idx + (in->numberoffacets*ii)] = (int) m;      // Not Sure this is right!!!!!!
      }
  ...
  }
  {
    ...
    for (PetscInt ii = 0; ii < numLabels; ++ii){
      ...
      if (label) {ierr = DMCreateLabel(*dm, labelName); ierr = DMGetLabel(*dm, labelName, &glabel);}
      //if (label) {ierr = DMLabelGetValue(label, f, &m);CHKERRQ(ierr);}
      //in->facemarkerlist[idx + (in->numberoffacets*(ii)] = (int) m;      // Not Sure this is right!!!!!!
