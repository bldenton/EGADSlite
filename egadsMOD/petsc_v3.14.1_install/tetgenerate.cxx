...
PETSC_EXTERN PetscErrorCode DMPlexGenerate_Tetgen(DM boundary, PetscBool interpolate, DM *dm)
{
  ...
  ierr = DMUniveralLabelCreate(boundary, &universal);CHKERRQ(ierr);
  ...
  }
  {
    ...
    /* Set labels */
    ierr = DMUniversalLabelCreateLabels(universal, *dm);CHKERRQ(ierr);
    for (v = 0; v < numVertices; ++v) {
...
PETSC_EXTERN PetscErrorCode DMPlexRefine_Tetgen(DM dm, double *maxVolumes, DM *dmRefined)
{
  ...
  ierr = DMUniveralLabelCreate(dm, &universal);CHKERRQ(ierr);
  ...
  in.tetrahedronvolumelist = NULL;
  {
    ...
    /* Set labels */
    ierr = DMUniversalLabelCreateLabels(universal, *dmRefined);CHKERRQ(ierr);
    for (v = 0; v < numVertices; ++v) {
      if (out.pointmarkerlist[v]) {
