    ...
    for (e = eStart; e < eMax; ++e) {
      ...
      if (offA != -1 && offB != -1) {
        ...
        for (d = 0; d < spaceDim; ++d) {
          ...
          //ierr = DMPlexSnapToGeomModel(dm, e, &coordsNew[offnew], &coordsNew[offnew]);CHKERRQ(ierr);    // Old Position
        }
        ierr = DMPlexSnapToGeomModel(dm, e, &coordsNew[offnew], &coordsNew[offnew]);CHKERRQ(ierr);        // New Position
      } else {
        for (d = 0; d < spaceDim; ++d) coordsNew[offnew+d] = PETSC_MIN_REAL;
      }
    }
