...
{
  const PetscInt *perm;
  PetscInt        Nv, v, Np;	// q was also a variable (unused)
  //PetscErrorCode  ierr;

  PetscFunctionBegin;
  PetscValidHeaderSpecific(label, DMLABEL_CLASSID, 1);
  PetscValidHeaderSpecific(permutation, IS_CLASSID, 2);
  PetscCall(DMLabelMakeAllValid_Private(label));	// was wrapped in PetscCall()
  PetscCall(DMLabelGetNumValues(label, &Nv));	// was wrapped in PetscCall()
  PetscCall(ISGetLocalSize(permutation, &Np));	// was wrapped in PetscCall()
  PetscCall(ISGetIndices(permutation, &perm));	// was wrapped in PetscCall()
  for (v = 0; v < Nv; ++v) {
  ...
  PetscCall(ISRestoreIndices(permutation, &perm));	// was wrapped in PetscCall()
  PetscCall(PetscHMapIReset(label->hmap));		// was wrapped in PetscCall
  for (v = 0; v < Nv; ++v) {
    PetscCall(PetscHMapISet(label->hmap, label->stratumValues[v], v));	// was wrapped in PetscCall
  }
  PetscFunctionReturn(0);
}
